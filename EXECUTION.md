...
grep -r "class " src --include="_.ts" | grep -v "TaggedClass\|TaggedError\|Data.Class" # 0件
grep -rE " as | any | unknown |!" src --include="_.ts" | grep -v spec.ts # 0件
grep -r "new Date" src --include="_.ts" # 0件
grep -rE "if\s_\(|else|switch|try|catch|\.then\(|for\s*\(" src --include="*.ts" | grep -v spec.ts # 0件
