...
New file `packages/entities/test/mob/phase-13-acceptance.test.ts`, plain `it` + `Effect.runPromise(Effect.provide(EntityManagerLive)(program))` (deadlock-safe; compose the real Layer), deterministic, no Math.random/THREE:
...
Tick mapped boxes and append `→ <real monorepo home>` (e.g. `src/ai/stateMachine.ts → packages/entities/domain/mob/state-machine.ts`; rendering → `packages/rendering/infrastructure/entity/{mob-geometry,walk-cycle,entity-renderer}.ts`). Any task box describing a flag-don't-build behavior is labeled 将来作業/未実装 and **not** ticked. Add a `## 将来作業 (本Phaseの受け入れ条件外)` footer. Do **not** create files at literal `src/` paths.
...
pnpm vitest run packages/entities/test/mob/phase-13-acceptance.test.ts packages/rendering/test/entity/walk-cycle.test.ts
