...
import { DomainLayer, InfrastructureLayer, ApplicationServicesLayer, TestLayer } from '@/layers'
...
describe('Architecture Compliance', () => {
  describe('Domain Layer Purity', () => {
      ...
      const checkDomainPurity = (dirPath: string): boolean => {
        ...
        for (const item of items) {
          ...
          if (stat.isDirectory()) {
            ...
            const forbiddenImports = ['@infrastructure', '@application', '@presentation', 'three', 'three/', 'gl-matrix', 'vite', 'vitest']
            ...
            const allowedImports = ['effect', '@domain', '@shared', '../', './', 'uuid']
    ...
    it('should only export ports and domain logic from domain layer', async () => {
      ...
      const result = await Effect.runPromise(Effect.provide(domainTest, TestLayer))
    ...
    it('should use only Effect and domain types in domain services', async () => {
      ...
      const result = await Effect.runPromise(Effect.provide(typeTest, TestLayer))
  ...
  describe('Infrastructure Port Implementation', () => {
    it('should implement all domain ports correctly', async () => {
      ...
      const result = await Effect.runPromise(Effect.provide(portImplementationTest, TestLayer))
    ...
    it('should provide adapter switching capability', async () => {
      ...
      const result = await Effect.runPromise(Effect.provide(adapterTest, TestLayer))
    ...
    it('should maintain port contract compliance', async () => {
      ...
      const result = await Effect.runPromise(Effect.provide(contractTest, TestLayer))
  ...
  describe('Application Layer Orchestration', () => {
    it('should properly orchestrate domain services', async () => {
      ...
      const result = await Effect.runPromise(Effect.provide(orchestrationTest, TestLayer))
    ...
    it('should handle cross-cutting concerns properly', async () => {
      const crossCuttingTest = Effect.gen(function* () {
        ...
        const result = yield* Effect.either(worldService.validatePosition({ x: 0, y: 0, z: 0 }))
      ...
      const result = await Effect.runPromise(Effect.provide(crossCuttingTest, TestLayer))
    ...
    it('should manage transactions and consistency', async () => {
      ...
      const result = await Effect.runPromise(Effect.provide(transactionTest, TestLayer))
  ...
  describe('Dependency Injection with Effect Layers', () => {
    it('should provide proper dependency injection', async () => {
      ...
      const result = await Effect.runPromise(Effect.provide(diTest, TestLayer))
    ...
    it('should support layer composition and overrides', async () => {
      ...
      const mockMathLayer = Layer.succeed(MathPort, {
        vector3: {
          ...
          distance: (a, b) => Effect.succeed(Math.sqrt((a.x - b.x) ** 2 + (a.y - b.y) ** 2 + (a.z - b.z) ** 2)),
          lerp: (a, b, t) =>
            Effect.succeed({
              x: a.x + (b.x - a.x) * t,
              y: a.y + (b.y - a.y) * t,
      ...
      const result = await Effect.runPromise(Effect.provide(overrideTest, testLayer))
    ...
    it('should handle circular dependency prevention', async () => {
      ...
      const result = await Effect.runPromise(Effect.provide(circularTest, TestLayer))
  ...
  describe('Architectural Pattern Enforcement', () => {
      ...
      const checkHexagonalPattern = (dirPath: string): boolean => {
        ...
        for (const item of items) {
          ...
          if (stat.isDirectory()) {
            ...
            const hasPortImplementation = content.includes('implements ') || content.includes('Layer.succeed') || content.includes('Context.GenericTag')
    ...
    it('should enforce SOLID principles', async () => {
      ...
      const result = await Effect.runPromise(Effect.provide(solidTest, TestLayer))
    ...
    it('should support Open/Closed principle through ports', async () => {
      ...
      const result = await Effect.runPromise(Effect.provide(openClosedTest, TestLayer))
