...
describe('DDD Architecture Validation', () => {
  describe('Directory Structure Validation', () => {
    ...
    it('should have proper file naming conventions', () => {
      const checkNaming = (dir: string, expectedSuffixes: string[]) => {
        ...
        files.forEach((file) => {
          const hasValidSuffix = expectedSuffixes.some((suffix) => file.includes(suffix) || file === 'index.ts')
          if (!hasValidSuffix) {
      ...
      const checkApplicationDependencies = (dirPath: string) => {
        ...
        for (const item of items) {
          ...
          if (stat.isDirectory()) {
            ...
            const problematicInfraImports = [/from ['"]@infrastructure.*three/g, /from ['"]@infrastructure.*webgl/g, /from ['"]@infrastructure.*canvas/g]
  ...
  describe('Effect-TS Usage Patterns', () => {
    it('should use Effect-TS consistently in domain services', () => {
      ...
      const serviceFiles = fs.readdirSync(domainServicesPath).filter((f) => f.endsWith('.service.ts') || f.endsWith('.ts'))
    ...
    it('should have proper port definitions with Effect-TS', () => {
      ...
      const portFiles = fs.readdirSync(portsPath).filter((f) => f.endsWith('.port.ts') && f !== 'index.ts')
      ...
      const analyzeDirectory = (dirPath: string) => {
        ...
        for (const item of items) {
          ...
          if (stat.isDirectory()) {
            ...
            if (content.includes('const ') && content.includes('=>') && (content.includes('Effect.') || content.includes('pipe('))) {
  ...
  describe('Error Handling Patterns', () => {
    it('should have comprehensive error definitions', () => {
      ...
      const errorFiles = fs.readdirSync(errorsPath).filter((f) => f.endsWith('.ts') && f !== 'index.ts')
    ...
    it('should use tagged errors pattern', () => {
      ...
      errorFiles.forEach((file) => {
        ...
        const hasTaggedPattern = content.includes('_tag') || content.includes('extends Error') || content.includes('defineError')
  ...
  describe('Basic Effect-TS Integration', () => {
    ...
    it('should demonstrate Effect error handling works', async () => {
      ...
      const handledEffect = Effect.catchAll(errorEffect, (error) => Effect.succeed(`Handled: ${error}`))
