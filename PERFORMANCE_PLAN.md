...
**状態: 部分完了。CLI環境制約により P4.2 で中断。**

- [x] **P4.1** `frame-stage-executor.ts`: 外側 `Effect.gen` を pre-build して再利用 ✅ (commit 8621d1b)
...
- [x] **4ステージ Effect.gen → flatMap 変換**: chunk-sync-stage, input-stage, interaction-hotbar-handler, multiplayer-stage ✅ (commit 4d9cc08)

- [-] **P4.2** 残存ステージ Effect.gen 完全除去（DEFERRED — 測定環境待ち）
  - 未変換: physics-stage, entity-update-stage, lighting-stage, camera-stage, hud-stage, post-processing-stage, interaction-stage (+ interaction handler群)
  - 理由: 複雑な条件分岐を含むため、ブラウザ heap/FPS 測定なしでの機械変換は回帰リスクが高い
  - 再開条件: ブラウザ環境で `?debug=perf` HUD heap測定が可能になり次第、1ステージずつ変換・実測
  - 推定工数: Medium (1-2日)
