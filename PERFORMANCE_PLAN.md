...
- [x] **P4.1** `frame-stage-executor.ts`: 外側 `Effect.gen` を pre-build して再利用（部分改善）
  - ファイル: `packages/app/application/frame/frame-stage-executor.ts` + `frame-handler.ts`
  - 内容: 毎フレームの外側 `Effect.gen` ジェネレータクロージャ生成を排除。各ステージ内部の `Effect.gen` には未着手（完全な Effect 分離には全ステージ書き換えが必要で、ブラウザ測定なしでは安全に実施不可）。
  - 品質: 5669 tests, build OK _(done 2026-06-12, commit 8621d1b)_

- [-] **P4.2** 各ステージ内部の `Effect.gen` 完全除去（DEFERRED — 測定環境待ち）
  - 内容: `physicsStage`, `renderStage`, `lightingStage`, `hudStage` など全15ステージが毎フレーム `Effect.gen` を生成している。完全除去には全ステージを pre-built Effect + mutable input ref 方式に書き換える必要がある。
  - **ブロック**: CLI環境ではブラウザheap測定が不可能なため、回帰テストなしでの実施は危険。ブラウザ環境での `?debug=perf` HUD heap測定が可能になり次第、段階的に着手。
  - **状態**: `[-]` DEFERRED。P4.1 (外側pipeline) + game-loop + browser-runtime のEffect.genは除去済み。残存分は全ステージ内部の15個で、広範囲なためP4.2として分離。
...
- 2026-06-12: **P4.1 実行** — 外側frame pipeline pre-build化 (commit 8621d1b)。各ステージ内部Effect.genはP4.2として測定ゲート付き保留
- 2026-06-12: 品質ゲート: typecheck 0, lint 0/2w, 5669 tests, build OK
- 2026-06-12: **状況**: 4本柱 — データ指向設計✅、メッシュ最適化✅、ゼロアロケーション化✅（物理+LOD完全zero-alloc）、Effect-TS分離⚠️（外側pipeline pre-build化で部分改善、ステージ内部はP4.2保留）
