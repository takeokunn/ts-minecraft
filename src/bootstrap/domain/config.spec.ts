...
describe('bootstrap/domain/config', () => {
  ...
  it.prop('bootstrapConfigはSchema生成値をそのまま返す', [configArb], ([input]) => {
  ...
  it.prop('bootstrapConfigSnapshotは構造を保持する', [snapshotArb], ([input]) => {
    const snapshot = Effect.runSync(bootstrapConfigSnapshot(input))
    expect(snapshot).toStrictEqual(input)
    return true
  })

  it.prop('materializeConfigSnapshotは引数をそのまま保持する', [configArb, epochMillisArb], ([config, loadedAt]) => {
