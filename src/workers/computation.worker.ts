...
export const getTerrainHeight = (x: number, z: number, terrainNoise: Noise2D, amplitude: number) => {
  const frequency = 0.01
  return Math.floor(terrainNoise(x * frequency, z * frequency) * amplitude)
}

export const getBiome = (x: number, z: number, biomeNoise: Noise2D) => {
...
export const generateGreedyMesh = (blocks: ReadonlyArray<PlacedBlock>, chunkX: number, chunkZ: number): ChunkGenerationResult['mesh'] => {
