...
import { Effect, Exit, Duration, TestContext, TestClock, Ref, Either, Fiber, Cause, Match, pipe, Schema } from 'effect'
...
describe('ErrorHandlers', () => {
  ...
  it.effect('エラーをログに記録してフォールバック値を返す', () => Effect.gen(function* () {
    ...
    it.effect('ロガーがない場合はconsole.errorを使用', () => Effect.gen(function* () {
    ...
    it.effect('複数のエラータイプでフォールバック', () => Effect.gen(function* () {
    const loggedErrors = yield* Ref.make<unknown[]>([])
    const logger = (error: unknown) => {
    return Effect.runSync(Ref.update(loggedErrors, (errors) => [...errors, error]))
    const errors = [
    ...
    it.effect('異なるフォールバック値を返す', () => Effect.gen(function* () {
    const error = NetworkError({ message: 'Test error'
  })
)
    ...
    describe('mapError', () => {
  it.effect('エラーを変換する', () => Effect.gen(function* () {
    ...
    it.effect('複雑なエラー変換', () => Effect.gen(function* () {
  ...
  describe('partial', () => {
  it.effect('部分的な成功を許可する', () => Effect.gen(function* () {
  ...
  Effect.gen(function* () {
    ...
    it.effect('すべて成功の場合', () => Effect.gen(function* () {
    const effects: Array<Effect.Effect<number, Error, never>> = [
    Effect.succeed(1),
    Effect.succeed(2
    }),
    ...
    it.effect('デフォルトの最小成功数（1）', () => Effect.gen(function* () {
    const effects: Array<Effect.Effect<number, Error, never>> = [
    Effect.fail(new Error('Failed 1')),
    Effect.fail(new Error('Failed 2')
    }),
    ...
    it.effect('最小成功数が0の場合', () => Effect.gen(function* () {
    const effects: Array<Effect.Effect<number, Error, never>> = [
    Effect.fail(new Error('Failed 1')
    }),
    Effect.fail(new Error('Failed 2')),
  ...
  it.scoped('エフェクトが時間内に完了する場合は成功', () => Effect.gen(function* () {
    ...
    it.scoped('エフェクトがタイムアウトする場合は失敗', () => Effect.gen(function* () {
    ...
    it.scoped('カスタムタイムアウトエラー', () => Effect.gen(function* () {
    const effect: Effect.Effect<string, never, never> = Effect.sleep(Duration.millis(100)).pipe(
    Effect.map(() => 'success')
    )
    ...
    it.scoped('エフェクトが既に失敗している場合はタイムアウトではなく元のエラー', () => Effect.gen(function* () {
    ...
    it.scoped('長時間実行エフェクトのタイムアウト管理', () => Effect.gen(function* () {
  ...
  it.prop('logAndFallbackは任意のエラーと任意のフォールバック値で動作する', [
    Schema.String,
    Schema.String
    ], ({ string: errorMsg, string_2: fallback
})
    ...
    describe('統合テスト', () => {
  it.scoped('複数のエラーハンドラーを組み合わせて使用', () => Effect.gen(function* () {
    const loggedErrors = yield* Ref.make<unknown[]>([])
    const logger = (error: unknown) => {
    return Effect.runSync(Ref.update(loggedErrors, (errors) => [...errors, error]))
    }
    ...
    it.scoped('部分的成功とタイムアウトの組み合わせ', () => Effect.gen(function* () {
    const effects = [
    Effect.succeed(1),
  ...
  it.effect('logAndFallbackは常にフォールバック値を返す', () => Effect.gen(function* () {
    const errors = [NetworkError({ message: 'test'
}), new Error('test'), 'string error', 123, null, undefined]
    for (
    const handler = ErrorHandlers.logAndFallback('fallback')
    const program = Effect.fail(error).pipe(Effect.catchAll(handler))
    const result = yield* program
    ...
    it.effect('mapErrorは常にEffect.failを返す', () => Effect.gen(function* () {
    const errors = [NetworkError({ message: 'test'
    }), new Error('test'), 'string error']
  ...
  Effect.gen(function* () {
        ...
        [Effect.fail(new Error('fail1')
    }),
    Effect.fail(new Error('fail2'))],
        ]
