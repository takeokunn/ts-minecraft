...
describe('Effect-TS Configuration', () => {
  describe('GameError', () => {
  it.effect('should create a GameError with required fields', () => Effect.gen(function* () {
    ...
    it.effect('should validate GameError schema', () => Effect.gen(function* () {
    const error = createGameError('Test error', 'TEST_ERROR')
    expect(Schema.is(GameErrorSchema)(error)).toBe(true)
    )
    it.effect('should create GameError with default code', () => Effect.gen(function* () {
    ...
    it.effect('should create and run simple effects', () => Effect.gen(function* () {
    const effect = EffectConfig.succeed(42)
    const result = runSync(effect)
    expect(result).toBe(42)
    )
    it.effect('should handle failures with fail', () => Effect.gen(function* () {
    const effect = EffectConfig.fail('test error')
    expect(() => runSync(effect)).toThrow('test error')
    )
    it.effect('should handle sync operations', () => Effect.gen(function* () {
    const effect = sync(() => 'hello world')
    const result = runSync(effect)
    expect(result).toBe('hello world')
    )
    it.effect('should compose effects with pipe', () => Effect.gen(function* () {
    const effect = pipe(
    ...
    it.effect('should identify GameError correctly', () => Effect.gen(function* () {
  ...
  it.effect('should catch errors with catchAll', () => Effect.gen(function* () {
    const effect = pipe(
    ...
    it.effect('should map errors', () => Effect.gen(function* () {
    const effect = pipe(
  ...
  Effect.gen(function* () {
        const effect = pipe(EffectConfig.fail('first'
    }),
    orElse(EffectConfig.succeed('fallback')))
        const result = runSync(effect)
  ...
  it.effect('should validate data with schemas', () => Effect.gen(function* () {
    ...
    it.effect('should fail validation for invalid data', () => Effect.gen(function* () {
    const schema = Schema.Struct({
    name: Schema.String,
    age: Schema.Number,
    )
  ...
  it.effect('should log messages', () => Effect.gen(function* () {
    const effect = pipe(
    ...
    it.effect('should create services with makeService', () => Effect.gen(function* () {
  ...
  it.effect('should create layers', () => Effect.gen(function* () {
    interface Config {
    apiUrl: string
  ...
  it.effect('should create PlayerId safely', () => Effect.gen(function* () {
  ...
  Effect.gen(function* () {
    ...
    it.effect('should create ChunkId safely', () => Effect.gen(function* () {
    ...
    it.effect('should reject invalid BlockTypeId', () => Effect.gen(function* () {
  ...
  it.effect('should decode branded types using Schema', () => Effect.gen(function* () {
