...
export const EffectConfig = {
   ...
   * 成功値を持つEffectを作成
   */
  succeed: <T>(value: T): GameResult<T, never> =>
    Effect.succeed(value),

  /**
   * エラーを持つEffectを作成
   */
  fail: (message: string, code?: string): GameResult<never> =>
    Effect.fail(createGameError(message, code)),

  /**
   * GameErrorかどうかを判定
   */
  isGameError: (value: unknown): value is GameError =>
    Schema.is(GameErrorSchema)(value),

  /**
   * Schemaを使用したバリデーション
   */
  validate: <A, I>(schema: Schema.Schema<A, I>) => (input: I): GameResult<A> =>
    Effect.try({
      try: () => Schema.decodeSync(schema)(input),
      catch: (error) => createGameError(
