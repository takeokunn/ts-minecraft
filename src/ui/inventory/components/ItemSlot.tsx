...
export const ItemSlot: React.FC<ItemSlotProps & { animation?: SlotAnimationState | undefined }> = ({
  ...
  const slotStyle = {
    width: size,
    height: size,
    backgroundColor: slot.isDisabled ? theme.slotDisabled :
                    slot.isHighlighted ? theme.slotSelected :
                    isDragOver || isOver ? theme.slotHover :
    ...
    transform: animation ?
              `scale(${animation.scale}) rotate(${animation.rotation}deg)` :
              undefined,
    boxShadow: animation?.glow ?
               `0 0 ${animation.glow * 20}px rgba(255, 255, 0, ${animation.glow})` :
               undefined,
  ...
  return (
    <>
      <motion.div
        ...
        animate={animation && animation.shake ? {
