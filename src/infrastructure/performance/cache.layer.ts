...
export const MultiLevelCacheServiceLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const has = (key: string) =>
      Effect.succeed(
        l1Cache.get(key) !== undefined ||
        l2Cache.get(key) !== undefined ||
        l3Cache.get(key) !== undefined
      )
...
export const cached = <R, E, A>(
  ...
  Effect.gen(function* () {
    ...
    const cached = yield* cache.get<A>(cacheKey).pipe(
...
export const memoize = <Args extends ReadonlyArray<any>, R, E, A>(
  ...
  return (...args: Args) =>
    Effect.gen(function* () {
      ...
      return yield* cache.get<A>(key).pipe(
        Effect.catchTag('CacheMissError', () =>
          Effect.tap(fn(...args), value =>
...
export const createQueryCache = (config?: Partial<CacheConfig>) =>
  Effect.gen(function* () {
    ...
    return {
      ...
      set: <T>(queryId: string, result: T) => 
        cache.set(`query:${queryId}`, result, { ttl: Duration.seconds(5) }),
...
export const createEntityCache = () =>
  Effect.gen(function* () {
    ...
    return {
      ...
      set: <T>(entityId: string, entity: T) => 
        cache.set(`entity:${entityId}`, entity, { level: 'L1' }),
