...
const startAutoCollection = (): Effect.Effect<void, never, never> =>
  Effect.gen(function* () {
    ...
    // Schedule collection
    yield* collectSystemMetrics.pipe(Effect.repeat(Schedule.fixed(`${metricsConfig.collectInterval} millis`)), Effect.fork)
  })
 ...
 */
const recordMetric = (name: string, type: MetricType, value: number, unit: string, description: string, labels?: Record<string, string>): Effect.Effect<void, never, never> =>
  Effect.gen(function* () {
    if (!metricsState) return
    ...
    yield* Ref.update(metricsState, (state) => {
      ...
      // Trim to max length
      const trimmedValues = values.length > metricsConfig.maxSeriesLength ? values.slice(-metricsConfig.maxSeriesLength) : values
...
export const Metrics = {
   ...
   */
  recordGauge: (name: string, value: number, unit: string = 'value', labels?: Record<string, string>) => recordMetric(name, 'gauge', value, unit, `Gauge: ${name}`, labels),
   ...
   */
  recordTimer: (name: string, durationMs: number, labels?: Record<string, string>) => recordMetric(name, 'timer', durationMs, 'ms', `Timer: ${name}`, labels),

  /**
   * Increment a counter by 1
   */
  increment: (name: string, labels?: Record<string, string>) => Metrics.recordCounter(name, 1, 'count', labels),

  /**
   * Decrement a counter by 1
   */
  decrement: (name: string, labels?: Record<string, string>) => Metrics.recordCounter(name, -1, 'count', labels),
  ...
  getStatistics: (name: string, windowMs?: number) =>
    Effect.gen(function* () {
      ...
      const now = Date.now()
      const values = windowMs ? series.values.filter((v) => v.timestamp > now - windowMs) : series.values
  ...
  generateDashboard: (): Effect.Effect<string, never, never> =>
    Effect.gen(function* () {
      ...
      const sortedSeries = Array.from(snapshot.series.values()).sort((a, b) => a.name.localeCompare(b.name))
...
export const withMetrics =
  <Args extends ReadonlyArray<any>, Return>(name: string, fn: (...args: Args) => Return, labels?: Record<string, string>) =>
  (...args: Args): Return => {
    const start = performance.now()
