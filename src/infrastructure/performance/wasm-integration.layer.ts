...
export const WasmModuleType = S.Literal(
  'physics',
  'terrain',
  'pathfinding',
  'noise',
...
export class WasmCompilationError extends S.TaggedError<WasmCompilationError>()(
...
export interface WasmService {
  ...
  ) => Stream.Stream<R, WasmError>
  
  readonly batch: <T, R>(
    moduleType: WasmModuleType,
    functionName: string,
    items: ReadonlyArray<T>
  ) => Effect.Effect<ReadonlyArray<R>, WasmError>
...
export const WasmServiceLive = Layer.effect(
  ...
  Effect.gen(function* () {
      ...
      Effect.forever(
        Effect.gen(function* () {
          ...
          try {
            ...
            yield* Effect.Deferred.fail(deferred, new WasmError({
        ...
        try {
          ...
          const compiledModule = config.streaming
            ? yield* Effect.tryPromise({
                ...
                catch: () => new WasmCompilationError({
          ...
          yield* Ref.update(modules, map =>
            HashMap.set(map, config.type, loadedModule)
          )
          ...
          return yield* Effect.fail(new WasmError({
    ...
    const stream = <T, R>(
      ...
      input.pipe(
        Stream.mapEffect((item: T) =>
          execute<R>(moduleType, functionName, [item])
        )
      )
    ...
    const batch = <T, R>(
      ...
      Effect.gen(function* () {
        ...
        const chunkSize = 100
        const chunks = Chunk.fromIterable(items).pipe(
          Chunk.chunksOf(chunkSize)
        )
        ...
        const results = yield* Effect.forEach(
          ...
          chunk =>
            Effect.forEach(
              ...
              item => execute<R>(moduleType, functionName, [item])
            ),
          { concurrency: 4 }
        )
    ...
    const getStats = (moduleType?: WasmModuleType) =>
      Effect.gen(function* () {
        ...
        const entries = moduleType
          ? Option.match(
              HashMap.get(moduleMap, moduleType),
              {
                onNone: () => [],
        ...
        for (const [type, module] of entries) {
          stats.push({
            ...
            averageExecutionTime: module.stats.executionCount > 0
              ? module.stats.totalExecutionTime / module.stats.executionCount
              : 0,
...
export const offloadToWasm = <T, R>(
  moduleType: WasmModuleType,
  functionName: string,
  prepareArgs?: (input: T) => unknown[]
) => (input: T) =>
  Effect.gen(function* () {
...
export const createWasmWorker = (moduleType: WasmModuleType) =>
  Effect.gen(function* () {
    ...
    return {
      execute: <T>(functionName: string, args: unknown[]) =>
        wasm.execute<T>(moduleType, functionName, args),
      
      batch: <T, R>(functionName: string, items: ReadonlyArray<T>) =>
        wasm.batch<T, R>(moduleType, functionName, items),
      
      stream: <T, R>(functionName: string, input: Stream.Stream<T>) =>
        wasm.stream<T, R>(moduleType, functionName, input)
    }
  })
...
export const simdAccelerated = <T extends Float32Array | Float64Array>(
  operation: (data: T) => T
) => (data: T) =>
  Effect.gen(function* () {
    const wasm = yield* WasmService
