...
const detectFrameDrop = (frameTime: number, targetFrameTime: number): FrameDropEvent | null => {
  ...
  if (frameTime > threshold) {
    ...
    const severity: FrameDropEvent['severity'] = 
      frameTime > targetFrameTime * 3 ? 'severe' :
      frameTime > targetFrameTime * 2 ? 'moderate' : 'minor'
...
const updateFPS = (currentTime: number): Effect.Effect<void, never, never> =>
  Effect.gen(function* () {
    ...
    const trimmedFrames = frames.length > fpsConfig.sampleSize 
      ? frames.slice(-fpsConfig.sampleSize)
      : frames
...
export const FPSCounter = {
  ...
  getStats: (): Effect.Effect<FPSStats, never, never> =>
    Effect.gen(function* () {
      ...
      const unstableFrames = fpsList.filter(fps => 
        Math.abs(fps - targetFPS) > fpsConfig.unstableThreshold
      ).length
  ...
  generateReport: (): Effect.Effect<string, never, never> =>
    Effect.gen(function* () {
      ...
      const performanceRating = 
        stats.averageFPS >= fpsConfig.targetFPS * 0.95 && stats.unstableFrames < stats.totalFrames * 0.05 ? 'Excellent' :
        stats.averageFPS >= fpsConfig.targetFPS * 0.8 && stats.unstableFrames < stats.totalFrames * 0.1 ? 'Good' :
        stats.averageFPS >= fpsConfig.targetFPS * 0.6 ? 'Fair' : 'Poor'
 ...
 */
export const withFPSTracking = <T>(
  callback: () => T
 ...
 */
export const requestAnimationFrameWithFPS = (
  callback: () => void
...
export const createFrameLoop = (
  callback: () => Effect.Effect<void, never, never>,
  targetFPS: number = fpsConfig.targetFPS
): Effect.Effect<() => void, never, never> =>
  Effect.gen(function* () {
    let isRunning = true
