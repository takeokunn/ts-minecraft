...
export const ProfilerService = Context.GenericTag<{
  ...
  readonly measure: <R, E>(name: string, effect: Effect.Effect<R, E, never>, metadata?: Record<string, unknown>) => Effect.Effect<R, E, never>
 ...
 */
const createProfilerServiceImpl = (config: ProfilerConfig): Effect.Effect<Context.Tag.Service<typeof ProfilerService>, never, never> =>
  Effect.gen(function* () {
    const measurements = yield* Ref.make<ProfileMeasurement[]>([])
    const activeMeasurements = yield* Ref.make<
      Map<
        string,
        {
          name: string
    ...
    const addMeasurement = (measurement: ProfileMeasurement) =>
      Effect.gen(function* () {
        yield* Ref.update(measurements, (current) => {
          ...
          return updated.length > config.maxMeasurements ? updated.slice(-config.maxMeasurements) : updated
        })
        ...
        if (config.enableConsoleOutput && measurement.duration > config.slowThreshold) {
          yield* Effect.logWarning(`Slow operation detected: ${measurement.name} took ${measurement.duration.toFixed(2)}ms`)
        }

        // Log memory spikes
        if (config.enableMemoryTracking && measurement.memory && measurement.memory.delta > 1024 * 1024) {
          yield* Effect.logWarning(`Memory spike detected: ${measurement.name} allocated ${(measurement.memory.delta / 1024 / 1024).toFixed(2)}MB`)
        }
      })
    ...
    return {
      ...
      endMeasurement: (id: string) =>
        Effect.gen(function* () {
          ...
          const measurement: ProfileMeasurement = {
            ...
            memory:
              config.enableMemoryTracking && active.startMemory !== undefined && endMemory !== undefined
                ? {
      ...
      measure: <R, E>(name: string, effect: Effect.Effect<R, E, never>, metadata?: Record<string, unknown>) =>
        Effect.gen(function* () {
          // Sample rate check
          ...
          const startMemory = config.enableMemoryTracking ? yield* Effect.sync(() => getMemoryUsage()) : yield* Effect.succeed(undefined)

          const result = yield* effect

          const endTime = yield* Effect.sync(() => performance.now())
          const endMemory = config.enableMemoryTracking ? yield* Effect.sync(() => getMemoryUsage()) : yield* Effect.succeed(undefined)
          ...
          const measurement: ProfileMeasurement = {
            ...
            timestamp: startTime,
            memory:
              config.enableMemoryTracking && startMemory !== undefined && endMemory !== undefined
                ? {
                    before: startMemory,
                    after: endMemory,
      ...
      getStats: () =>
        Effect.gen(function* () {
          ...
          const memoryDeltas = allMeasurements.filter((m) => m.memory).map((m) => m.memory!.delta)
      ...
      exportData: () =>
        Effect.gen(function* () {
          ...
          const stats = yield* Effect.sync(() => ({
            totalMeasurements: allMeasurements.length,
            averageDuration: allMeasurements.length > 0 ? allMeasurements.reduce((sum, m) => sum + m.duration, 0) / allMeasurements.length : 0,
            ...
            slowOperations: allMeasurements.filter((m) => m.duration > config.slowThreshold).length,
            memoryAllocated: allMeasurements.filter((m) => m.memory).reduce((sum, m) => sum + m.memory!.delta, 0),
            memoryPeak: allMeasurements.filter((m) => m.memory).reduce((max, m) => Math.max(max, m.memory!.delta), 0),
          }))
 ...
 */
export const ProfilerServiceLive = (config: ProfilerConfig = defaultProfilerConfig) => Layer.effect(ProfilerService, createProfilerServiceImpl(config))
 ...
 */
export const withSession = <R, E>(sessionName: string, effect: Effect.Effect<R, E, ProfilerService>): Effect.Effect<R, E, ProfilerService> =>
  Effect.gen(function* () {
    const profiler = yield* ProfilerService
