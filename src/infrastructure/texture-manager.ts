...
const applyTextureSettings = (texture: THREE.Texture, metadata: TextureMetadata): THREE.Texture => {
  ...
  texture.minFilter = CONFIG.MAX_MIPMAPS > 1 
 ...
 */
const addTextureToAtlas = (
  atlas: TextureAtlas,
  name: string,
  imageData: ImageData
): TextureAtlasCoords | null => {
  ...
  atlas.usage = ((CONFIG.ATLAS_SIZE * CONFIG.ATLAS_SIZE) - 
    atlas.freeRegions.reduce((sum, region) => sum + region.width * region.height, 0)) / 
    (CONFIG.ATLAS_SIZE * CONFIG.ATLAS_SIZE) * 100
    ...
    return TextureManagerService.of({
      loadTexture: (name: string) =>
        Effect.gen(function* () {
          ...
          // Load image data
          const imageData = yield* _(loadImageData(textureConfig.path).pipe(
            Effect.catchAll(() => Effect.succeed(new ImageData(CONFIG.TILE_SIZE, CONFIG.TILE_SIZE)))
          ))
      ...
      preloadEssentials: () =>
        Effect.gen(function* () {
          ...
          yield* _(Effect.all(
            essentials.map(name => 
              Effect.gen(function* () {
                return yield* _(Effect.serviceRef.loadTexture(name))
              })
            ),
            { concurrency: 4 }
          ))
      ...
      createAnimatedTexture: (name: string, frames: string[], frameRate: number) =>
        Effect.gen(function* () {
          const frameTextures = yield* _(Effect.all(
            frames.map(frameName => Effect.serviceRef.loadTexture(frameName))
          ))
      ...
      getStats: () =>
        Effect.gen(function* () {
          ...
          const totalAtlasUsage = Array.from(state.atlases.values())
            .reduce((sum, atlas) => sum + atlas.usage, 0) / state.atlases.size
