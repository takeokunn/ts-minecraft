...
import { RendererServiceLive } from './renderer-service-live'

const provideRenderer = <A>(effect: Effect.Effect<A>) => effect.pipe(provideLayers(RendererServiceLive))
...
const parseFails = <A>(thunk: () => A) => parseSucceeds(thunk).pipe(Effect.map((result) => !result))
    ...
    fc.assert(
      fc.property(fc.integer(bounds), (value) => {
        return Match.value(value).pipe(
          ...
          Match.orElse((invalid) => Effect.runSync(parseFails(() => parseRgbColor(invalid))))
        )
      }),
      { verbose: false }
    )
    ...
    fc.assert(
      fc.property(fc.double({ min: -10, max: 10 }), (value) => {
        return Match.value(value).pipe(
          ...
          Match.orElse((invalid) => Effect.runSync(parseFails(() => parseAlpha(invalid))))
        )
      })
    )
  })
