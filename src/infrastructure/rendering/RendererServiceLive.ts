...
class RendererServiceImpl implements RendererService {
  constructor(private readonly rendererRef: Ref.Ref<THREE.WebGLRenderer | null>) {}
  ...
  readonly initialize = (canvas: HTMLCanvasElement): Effect.Effect<void, RenderError> =>
    Effect.gen(this, function* () {
      try {
        ...
        // レンダラーの保存
        yield* Ref.set(this.rendererRef, renderer)
  ...
  readonly render = (scene: THREE.Scene, camera: THREE.Camera): Effect.Effect<void, RenderError> =>
    Effect.gen(this, function* () {
      const renderer = yield* Ref.get(this.rendererRef)
  ...
  readonly setClearColor = (color: number, alpha = 1.0): Effect.Effect<void, never> =>
    Effect.gen(this, function* () {
      const renderer = yield* Ref.get(this.rendererRef)
...
export const RendererServiceLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    return new RendererServiceImpl(rendererRef)
  })
)