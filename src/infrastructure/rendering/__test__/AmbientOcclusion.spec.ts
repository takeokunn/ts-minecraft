...
describe('AmbientOcclusion', () => {
  ...
  describe('AmbientOcclusionService - calculateVertexAO', () => {
    const getService = () => Effect.gen(function* () {
      return yield* AmbientOcclusionService
    }).pipe(
      Effect.provide(AmbientOcclusionLive),
      Effect.runSync
    )
    ...
    it('should calculate AO for open space vertex', async () => {
      ...
      const ao = await pipe(
        getService().calculateVertexAO(chunk.blocks, 2, 2, 2, chunk.size),
        Effect.runPromise
      )
    ...
    it('should handle corner occlusion correctly', async () => {
      ...
      const ao = await pipe(
        getService().calculateVertexAO(chunk.blocks, 0, 0, 0, chunk.size),
        Effect.runPromise
      )
    ...
    it('should handle out of bounds coordinates', async () => {
      ...
      const ao = await pipe(
        getService().calculateVertexAO(chunk.blocks, -1, -1, -1, chunk.size),
        Effect.runPromise
      )
    ...
    it('should handle errors in vertex AO calculation', async () => {
      const result = runEffect(
        getService().calculateVertexAO(null as any, 0, 0, 0, 4)
      )
  ...
  describe('AmbientOcclusionService - calculateFaceAO', () => {
    ...
    it('should calculate AO for top face', async () => {
      ...
      const face = await pipe(
        getService().calculateFaceAO(chunk.blocks, mid, mid, mid, 'top', chunk.size),
        Effect.runPromise
      )
    ...
    it('should calculate different AO for different faces', async () => {
      ...
      const topFace = await pipe(
        getService().calculateFaceAO(chunk.blocks, 0, 0, 0, 'top', chunk.size),
        Effect.runPromise
      )

      const bottomFace = await pipe(
        getService().calculateFaceAO(chunk.blocks, 0, 0, 0, 'bottom', chunk.size),
        Effect.runPromise
      )
    ...
    it('should handle all face directions', async () => {
      ...
      for (const face of faces) {
        const aoFace = await pipe(
          getService().calculateFaceAO(chunk.blocks, mid, mid, mid, face, chunk.size),
          Effect.runPromise
        )
    ...
    it('should handle errors in face AO calculation', async () => {
      const result = runEffect(
        getService().calculateFaceAO(null as any, 0, 0, 0, 'top', 4)
      )
  ...
  describe('AmbientOcclusionService - applyAOToChunk', () => {
    ...
    it('should apply AO to empty chunk', async () => {
      ...
      const vertices = await pipe(
        getService().applyAOToChunk(chunk),
        Effect.runPromise
      )
    ...
    it('should respect disabled AO setting', async () => {
      ...
      const DisabledAOLive = Layer.succeed(
        ...
        AmbientOcclusionService.of({
          ...
          calculateFaceAO: () => Effect.succeed({
            vertices: [
              { x: 0, y: 0, z: 0, ao: 1.0 },
              { x: 1, y: 0, z: 0, ao: 1.0 },
              { x: 1, y: 1, z: 0, ao: 1.0 },
      ...
      const getService = () => Effect.gen(function* () {
        ...
        Effect.provide(DisabledAOLive),
        Effect.runSync
      )

      const ao = await pipe(
        getService().calculateVertexAO([], 0, 0, 0, 4),
        Effect.runPromise
      )
