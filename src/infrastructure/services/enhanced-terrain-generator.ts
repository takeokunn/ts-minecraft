 ...
 */
const calculateBiome = (_worldX: number, _worldZ: number, temperature: number, humidity: number, elevation: number): BiomeDefinition => {
 ...
 */
const generateStructures = (chunkX: number, chunkZ: number, biome: BiomeDefinition, seed: number): Array<{ type: string; position: [number, number, number] }> => {
 ...
 */
const generateCaves = (_chunkX: number, _chunkZ: number, _seed: number): Array<{ position: [number, number, number]; radius: number }> => {
 ...
 */
const generateOres = (_chunkX: number, _chunkZ: number, _seed: number): Array<{ type: string; position: [number, number, number] }> => {
...
export const EnhancedTerrainGeneratorLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    return {
        ...
        Effect.gen(function* () {
          ...
          if (CONFIG.USE_WASM) {
            try {
              ...
              const noiseData = yield* wasmService
                .callFunction('terrain_noise', 'generate_noise', coords.x, coords.z, state.params.seed, state.params.octaves)
      ...
      getSeed: () => Ref.get(stateRef).pipe(Effect.map((state) => state.params.seed)),

      getConfig: () => Ref.get(stateRef).pipe(Effect.map((state) => state.params)),

      // Enhanced methods
      prefetchChunk: (coords: ChunkCoordinates, priority: number = 1) =>
        Ref.update(stateRef, (state) => ({
          ...state,
          prefetchQueue: [...state.prefetchQueue, { coords, priority }].sort((a, b) => b.priority - a.priority).slice(0, 100), // Limit queue size
        })),

      getStats: () => Ref.get(stateRef).pipe(Effect.map((state) => state.stats)),

      optimizeCache: () =>
        Ref.update(stateRef, (state) => {
          // Compress least recently used entries
          const sortedEntries = Array.from(state.cache.entries()).sort(([, a], [, b]) => a.lastAccessed - b.lastAccessed)
  ...
  }).pipe(Effect.provide(TypedWorkerManager), Effect.provide(WASMIntegrationService)),
)
 ...
 */
const generateTerrainLocal = (coords: ChunkCoordinates, params: TerrainGenParams, biomes: Map<string, BiomeDefinition>) =>
  Effect.succeed({
...
export type { TerrainGeneratorState, TerrainGenParams, BiomeDefinition, EnhancedChunkData, TerrainCacheEntry }
