...
export const SpatialGridLive = Layer.effect(
  ...
  Effect.gen(function* (_) {
    ...
    return {
      ...
      clear: () => Effect.all([Ref.set(legacyGridRef, HashMap.empty()), Ref.update(gridRef, () => initialState)]).pipe(Effect.asVoid),

      add: (entityId: EntityId, aabb: AABB) =>
        Effect.all([Ref.update(legacyGridRef, (grid) => addPure(grid, entityId, aabb)), Ref.update(gridRef, (state) => addEntityPure(state, entityId, aabb))]).pipe(Effect.asVoid),

      query: (aabb: AABB) => Ref.get(gridRef).pipe(Effect.map((state) => queryEntitiesPure(state, aabb))),

      // Enhanced methods
      addEntity: (entityId: EntityId, aabb: AABB) => Ref.update(gridRef, (state) => addEntityPure(state, entityId, aabb)),

      removeEntity: (entityId: EntityId) => Ref.update(gridRef, (state) => removeEntityPure(state, entityId)),

      queryNearby: (center: { x: number; y: number; z: number }, radius: number) => Ref.get(gridRef).pipe(Effect.map((state) => queryNearbyPure(state, center, radius))),

      getMetrics: () => Ref.get(gridRef).pipe(Effect.map((state) => state.metrics)),

      optimize: () => Ref.update(gridRef, optimizePure),

      getCellCount: () => Ref.get(gridRef).pipe(Effect.map((state) => state.cells.size)),

      getEntityCount: () => Ref.get(gridRef).pipe(Effect.map((state) => state.metrics.totalEntities)),
