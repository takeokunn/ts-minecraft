 ...
 */
const createInstancedBatch = (geometry: THREE.BufferGeometry, material: THREE.Material, maxInstances: number = CONFIG.MAX_INSTANCES_PER_TYPE): InstancedRenderBatch => {
 ...
 */
const addInstanceToBatch = (batch: InstancedRenderBatch, entityId: EntityId, matrix: THREE.Matrix4, color?: THREE.Color): boolean => {
 ...
 */
const removeInstanceFromBatch = (batch: InstancedRenderBatch, entityId: EntityId): boolean => {
 ...
 */
const createLODObject = (geometries: THREE.BufferGeometry[], materials: THREE.Material[], distances: number[]): LODConfiguration => {
 ...
 */
const updateLODObjects = (lodObjects: Map<string, LODConfiguration>, camera: THREE.Camera): void => {
 ...
 */
const performFrustumCulling = (objects: THREE.Object3D[], camera: THREE.Camera, frustum: THREE.Frustum): FrustumCullingData => {
            ...
            Ref.update(stateRef, (state) => {
              ...
              for (const batch of state.instancedBatches.values()) {
                if (batch.instanceCount > 0) {
                  ...
                  const indexCount = geometry.index ? geometry.index.count : geometry.getAttribute('position')?.count || 0
...
export type { ThreeJSOptimizerState, InstancedRenderBatch, LODConfiguration, FrustumCullingData, OcclusionCullingData, GeometryBatch }
