...
const generateBlockGeometry = (
  ...
  faces.forEach((face) => {
    ...
    indices.push(startIndex, startIndex + 1, startIndex + 2, startIndex + 2, startIndex + 3, startIndex)
 ...
 */
const getFaceVisibility = (chunk: Chunk, neighborChunks: MeshGenerationRequest['neighborChunks'], x: number, y: number, z: number): FaceVisibility => {
...
export const MeshBuilderLive = Layer.effect(
  ...
  Effect.gen(function* (_) {
    ...
    const workerClient = CONFIG.WORKER_ENABLED ? yield* _(createTypedWorkerClient(new Worker('/infrastructure/workers/mesh-generation.worker.js'), workerClientConfig)) : null
    ...
    return {
      ...
      generateLOD: (chunk: Chunk, lodLevel: number) => Effect.succeed(generateLODMesh(chunk, lodLevel)),
        ...
        Effect.gen(function* () {
          ...
          for (let i = 0; i < vertexData.indices.length; i++) {
            ...
            if (!positionMap.has(key)) {
              ...
              optimizedNormals.push(vertexData.normals[originalIndex * 3], vertexData.normals[originalIndex * 3 + 1], vertexData.normals[originalIndex * 3 + 2])
              optimizedUvs.push(vertexData.uvs[originalIndex * 2], vertexData.uvs[originalIndex * 2 + 1])
              optimizedColors.push(vertexData.colors[originalIndex * 3], vertexData.colors[originalIndex * 3 + 1], vertexData.colors[originalIndex * 3 + 2])
