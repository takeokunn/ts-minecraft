 ...
 */
const endProfiling = (functionName: string, profilingData: Map<string, ProfilingData>): void => {
  if (!CONFIG.PROFILING_ENABLED) return
...
export const PerformanceMonitorLive = Layer.effect(
  ...
  Effect.gen(function* (_) {
        ...
        if (timeSinceLastSample >= 1000 / CONFIG.SAMPLE_RATE) {
          ...
          yield* _(
            Ref.update(stateRef, (s) => {
              ...
              // Check for alerts
              const newAlerts = checkPerformanceAlerts(s.frameTimings, newMemoryMetrics, newSystemMetrics, s.alerts)
    ...
    return {
      ...
      startProfiling: (functionName: string) => Effect.sync(() => startProfiling(functionName)),
...
export type { PerformanceMonitorState, FrameTiming, MemoryMetrics, SystemMetrics, RenderingMetrics, GameMetrics, PerformanceAlert, ProfilingData }
