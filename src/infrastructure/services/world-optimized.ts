 ...
 */
const invalidateQueryCache = (archetypes: OptimizedArchetypeStorage, componentName: ComponentName): void => {
 ...
 */
const getEntitiesInChunk = (state: OptimizedWorldState, chunkX: number, chunkZ: number): Set<EntityId> => {
 ...
 */
const updateSpatialIndex = (state: OptimizedWorldState, entityId: EntityId, oldPosition?: Vector3, newPosition?: Vector3): OptimizedWorldState => {
...
const removeEntityOptimized = (state: OptimizedWorldState, entityId: EntityId): OptimizedWorldState => {
  ...
  const position = Option.isSome(positionComponent) ? ([positionComponent.value.x, positionComponent.value.y, positionComponent.value.z] as Vector3) : undefined
...
const queryOptimized = <T extends ReadonlyArray<ComponentName>>(
  ...
  // Collect results
  const results = Array.from(matchingArchetypes)
...
const optimizeWorldState = (state: OptimizedWorldState): OptimizedWorldState => {
  ...
  const recentEvents = state.changeEvents.filter((event) => currentTime - event.timestamp < EVENT_MAX_AGE)
...
export const WorldOptimizedLive = Layer.effect(
  ...
  Effect.gen(function* (_) {
    ...
    return {
      ...
      addArchetype: (archetype: Archetype) => Ref.modify(state, (s) => addArchetypeOptimized(s, archetype)),

      removeEntity: (entityId: EntityId) => Ref.update(state, (s) => removeEntityOptimized(s, entityId)),
      ...
      updateComponent: <T extends ComponentName>(entityId: EntityId, componentName: T, data: Partial<ComponentOfName<T>>) =>
        Ref.get(state).pipe(
          Effect.flatMap((s) => {
            const componentStorage = s.components[componentName]
            if (!componentStorage) {
      ...
      querySoA: <T extends ReadonlyArray<ComponentName>>(query: LegacyQuery<T> | OptimizedQuery<T>) =>
        Ref.get(state).pipe(
          Effect.map((s) => {
            ...
            const components = Object.fromEntries(query.components.map((name, i) => [name, results.map(([, ...components]) => components[i])]))
      ...
      getChunk: (chunkX: number, chunkZ: number) => Ref.get(state).pipe(Effect.map((s) => HashMap.get(s.chunks, getChunkKey(chunkX, chunkZ)))),
      ...
      // Enhanced methods
      getEntitiesInChunk: (chunkX: number, chunkZ: number) => Ref.get(state).pipe(Effect.map((s) => Array.from(getEntitiesInChunk(s, chunkX, chunkZ)))),

      getWorldMetrics: () => Ref.get(state).pipe(Effect.map((s) => s.metrics)),

      optimizeWorld: () => Ref.update(state, optimizeWorldState),
...
export type { OptimizedWorldState, OptimizedComponentStorage, OptimizedArchetypeStorage, WorldChangeEvent, EntityBatch, CachedQuery }
