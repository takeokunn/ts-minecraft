  ...
  if (typeof WebAssembly !== 'undefined') {
    ...
    try {
      new WebAssembly.Module(
        new Uint8Array([
          0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00, 0x01, 0x05, 0x01, 0x60, 0x01, 0x7b, 0x01, 0x7b, 0x03, 0x02, 0x01, 0x00, 0x0a, 0x0a, 0x01, 0x08, 0x00, 0x20, 0x00, 0xfd,
          0x0f, 0x0b,
        ]),
      )
    ...
    try {
      new WebAssembly.Module(
        new Uint8Array([0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00, 0x05, 0x03, 0x01, 0x00, 0x01, 0x0a, 0x09, 0x01, 0x07, 0x00, 0x41, 0x00, 0x41, 0x00, 0xfc, 0x0a, 0x00]),
      )
    ...
    try {
      new WebAssembly.Module(
        new Uint8Array([0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00, 0x01, 0x04, 0x01, 0x60, 0x00, 0x00, 0x03, 0x02, 0x01, 0x00, 0x0a, 0x04, 0x01, 0x02, 0x00, 0x0b]),
      )
...
const instantiateWASMModule = (wasmModule: WASMModule, imports?: WebAssembly.Imports): Effect.Effect<WASMModule, never, never> =>
  Effect.gen(function* () {
    ...
    const instance = yield* _(Effect.promise(() => WebAssembly.instantiate(wasmModule.module, finalImports)))
 ...
 */
const createWASMFunction = (name: string, func: Function, params: WASMFunction['params'], returns: WASMFunction['returns']): WASMFunction => ({
  name,
  params,
  returns,
  func: (...args: any[]) => {
...
const MATH_WASM_BYTES = new Uint8Array([
  ...
  0x7f,
  0x01,
  0x7f, // Function signature: (i32, i32) -> i32
  0x03,
  0x02,
  0x01,
  ...
  0x01,
  0x6a,
  0x0b, // Function body: local.get 0, local.get 1, i32.add
])
...
export const WASMIntegrationLive = Layer.effect(
  ...
  Effect.gen(function* (_) {
    ...
    yield* _(loadBuiltinModules().pipe(Effect.catchAll((error) => Effect.sync(() => console.warn('Failed to load built-in WASM modules:', error)))))
    ...
    return {
        ...
        Effect.gen(function* () {
          ...
          for (const [exportName, exportValue] of Object.entries(instantiatedModule.exports)) {
            if (typeof exportValue === 'function') {
              ...
              functions.set(`${name}.${exportName}`, profilingEnabled ? profileWASMFunction(wasmFunc, (yield* _(Ref.get(stateRef))).performanceProfiles) : wasmFunc)
            }
          }
      ...
      createMemoryBuffer: (name: string, size: number, shared: boolean = false) =>
        Effect.gen(function* () {
          const buffer = shared && CONFIG.THREADING_ENABLED ? new SharedArrayBuffer(size) : new ArrayBuffer(size)
...
export type { WASMIntegrationState, WASMModule, WASMFunction, WASMMemoryBuffer, WASMPerformanceProfile }
