...
export interface WorkerManagerService {
  ...
  ) => Effect.Effect<LightingCalculationResponse, never, never>

  /**
   * Send computation request
   */
  readonly executeComputation: (
  ...
  ) => Effect.Effect<ComputationResponse, never, never>
...
const make = (config: WorkerManagerConfig = defaultConfig) =>
  Effect.gen(function* () {
    ...
    const requestQueues = yield* Ref.make<
      Map<
        WorkerType,
        Queue.Queue<{
          request: any
     ...
     */
    const createWorkerInstance = (type: WorkerType, workerId: string): Effect.Effect<WorkerInstance, never, never> =>
      Effect.gen(function* () {
        ...
        const inputSchemas: Record<WorkerType, any> = {
          ...
          lighting: LightingCalculationRequest,
        ...
        const outputSchemas: Record<WorkerType, any> = {
          ...
          lighting: LightingCalculationResponse,
          computation: ComputationResponse,
        }
    ...
    const getBestWorker = (workerType: WorkerType): Effect.Effect<WorkerInstance | null, never, never> =>
      Effect.gen(function* () {
        ...
        const availableWorkers = workers.filter((w) => w.status === 'idle' || (w.status === 'busy' && w.metrics.activeRequests < config[workerType].maxConcurrency))
        ...
        switch (config.globalSettings.loadBalancing) {
          ...
          case 'least-busy':
            return availableWorkers.reduce((best, current) => (current.metrics.activeRequests < best.metrics.activeRequests ? current : best))

          case 'priority':
            return availableWorkers.reduce((best, current) => (current.priority < best.priority ? current : best))
      ...
      Effect.gen(function* () {
        ...
        while (attempts <= maxRetries) {
          try {
            ...
            const response = yield* worker.client
              .sendRequest(request, {
              ...
              .pipe(
                ...
                Effect.tap(() => {
                  ...
                  worker.metrics.averageResponseTime = (worker.metrics.averageResponseTime + responseTime) / 2
    ...
    return {
      generateTerrain: (request, options) => executeRequest<TerrainGenerationRequest, TerrainGenerationResponse>('terrain', request, options),

      simulatePhysics: (request, options) => executeRequest<PhysicsSimulationRequest, PhysicsSimulationResponse>('physics', request, options),

      generateMesh: (request, options) => executeRequest<MeshGenerationRequest, MeshGenerationResponse>('mesh', request, options),

      calculateLighting: (request, options) => executeRequest<LightingCalculationRequest, LightingCalculationResponse>('lighting', request, options),

      executeComputation: (request, options) => executeRequest<ComputationRequest, ComputationResponse>('computation', request, options),
 ...
 */
export const WorkerManagerServiceLive = Layer.effect(WorkerManagerService, make())

/**
 * Create WorkerManagerService with custom config
 */
export const WorkerManagerServiceLiveWith = (config: Partial<WorkerManagerConfig>) => Layer.effect(WorkerManagerService, make({ ...defaultConfig, ...config }))
