...
export const TextureReference = S.Struct({
  ...
  wrapS: S.optional(S.Union(
    S.Literal('repeat'),
    S.Literal('clamp'),
    S.Literal('mirror')
  )),
  wrapT: S.optional(S.Union(
    S.Literal('repeat'),
    S.Literal('clamp'),
    S.Literal('mirror')
  ))
...
export const BoundingVolume = S.Struct({
  ...
  // Bounding sphere
  sphere: S.optional(S.Struct({
    center: Position3D,
    radius: S.Number.pipe(S.positive)
  })),
  
  // Oriented bounding box
  obb: S.optional(S.Struct({
    center: Position3D,
...
export const MeshGenerationRequest = S.Struct({
  ...
  // Material settings
  materialMapping: S.optional(S.Record({
 ...
 */
export const createTransferableVertexData = (
  positions: number[],
  normals?: number[],
  uvs?: number[],
  colors?: number[]
 ...
 */
export const validateMeshRequest = (request: unknown) =>
  S.decodeUnknown(MeshGenerationRequest)(request)

/**
 * Validate mesh generation response
 */
export const validateMeshResponse = (response: unknown) =>
  S.decodeUnknown(MeshGenerationResponse)(response)
