 ...
 */
export const BodyType = S.Union(S.Literal('static'), S.Literal('kinematic'), S.Literal('dynamic'), S.Literal('ghost'), S.Literal('trigger')).pipe(S.identifier('BodyType'))
 ...
 * Force types
 */
export const ForceType = S.Union(S.Literal('impulse'), S.Literal('force'), S.Literal('velocity_change'), S.Literal('acceleration')).pipe(S.identifier('ForceType'))
...
export const PhysicsField = S.Struct({
  id: S.String,
  type: S.Union(S.Literal('gravity'), S.Literal('wind'), S.Literal('magnetic'), S.Literal('buoyancy'), S.Literal('drag')),
  ...
  falloff: S.optional(S.Union(S.Literal('linear'), S.Literal('quadratic'), S.Literal('exponential'), S.Literal('constant'))),
 ...
 */
export const createPhysicsMaterial = (friction: number = 0.5, restitution: number = 0.3, density: number = 1.0): PhysicsMaterial => ({
  friction,
  restitution,
  density,
})
 ...
 */
export const validatePhysicsRequest = (request: unknown) => S.decodeUnknown(PhysicsSimulationRequest)(request)

/**
 * Validate physics simulation response
 */
export const validatePhysicsResponse = (response: unknown) => S.decodeUnknown(PhysicsSimulationResponse)(response)
