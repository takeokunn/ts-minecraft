...
describe('Entity ECS Architecture', () => {
  ...
  it.effect('should allocate unique entity IDs', () => Effect.gen(function* () {
    ...
    it.effect('should recycle deallocated IDs', () => Effect.gen(function* () {
    const pool = yield* EntityPool
    const id1 = yield* pool.allocate()
    ...
    it.effect('should track allocated status correctly', () => Effect.gen(function* () {
    const pool = yield* EntityPool
    ...
    it.effect('should fail when deallocating unallocated entity', () => Effect.gen(function* () {
    const pool = yield* EntityPool
    ...
    it.effect('should reset pool state correctly', () => Effect.gen(function* () {
    const pool = yield* EntityPool
    // いくつかのエンティティを割り当て
    const ids = yield* Effect.all([pool.allocate(), pool.allocate(
    }),
    pool.allocate()])
    // リセット
    yield* pool.reset()
    ...
    it.effect('should provide accurate statistics', () => Effect.gen(function* () {
    const pool = yield* EntityPool
    ...
    it.effect('should insert and retrieve components', () => Effect.gen(function* () {
    ...
    it.effect('should remove components and maintain array compactness', () => Effect.gen(function* () {
    const storage = createComponentStorage<TestComponent>()
    ...
    it.effect('should batch retrieve all components efficiently', () => Effect.gen(function* () {
    const storage = createComponentStorage<TestComponent>()
    ...
    return true
    ) {$2}.pipe(Effect.provide(TestContext.TestContext))
    )
    it.effect('should iterate over components efficiently', () => Effect.gen(function* () {
    const storage = createComponentStorage<TestComponent>()
    // 100個のコンポーネントを追加
    for (
    yield* storage.insert(EntityId(i), { x: i, y: i * 2, z: i * 3 ) {$2}
    // イテレーション
    let count = 0
    let sumX = 0
    ...
    it.effect('should provide raw data access for maximum performance', () => Effect.gen(function* () {
    const storage = createComponentStorage<TestComponent>()
    const numEntities = 10
    for (
    yield* storage.insert(EntityId(i), { x: i, y: 0, z: 0 ) {$2}
    ...
    it.effect('should clear all data', () => Effect.gen(function* () {
    const storage = createComponentStorage<TestComponent>()
    // データを追加
  ...
  it.effect('should create and cache archetypes', () => Effect.gen(function* () {
    ...
    it.effect('should create different archetypes for different component sets', () => Effect.gen(function* () {
    const manager = createArchetypeManager()
    ...
    it.effect('should move entities between archetypes', () => Effect.gen(function* () {
    const manager = createArchetypeManager()
    const entityId = EntityId(1)
    const components1 = new Set(['Position'])
    const components2 = new Set(['Position', 'Velocity'])
    ...
    it.effect('should remove entities from archetypes', () => Effect.gen(function* () {
    const manager = createArchetypeManager()
    const entityId = EntityId(1)
    ...
    it.effect('should get all entities with specific archetype', () => Effect.gen(function* () {
    const manager = createArchetypeManager()
    const components = new Set(['Position', 'Velocity'])
    ...
    it.effect('should clear all archetype data', () => Effect.gen(function* () {
    const manager = createArchetypeManager()
    const components1 = new Set(['Position'])
    const components2 = new Set(['Velocity'])
    yield* manager.moveEntity(EntityId(1), components1)
    yield* manager.moveEntity(EntityId(2
    }),
    components2)
    yield* manager.clear()
    const entities1 = yield* manager.getEntitiesWithArchetype(components1)
    ...
    it.effect('should handle large number of components efficiently', () => Effect.gen(function* () {
    const storage = createComponentStorage<{ value: number }>()
    // パフォーマンステスト - 10000個のコンポーネント挿入
    const insertionTest = Effect.gen(function* () {
    for (
    yield* storage.insert(EntityId(i), { value: i ) {$2}
    ...
    it.effect('should maintain memory efficiency with Structure of Arrays', () => Effect.gen(function* () {
    ...
    for (
    yield* storage.insert(EntityId(i), { x: i, y: i, z: i, w: i ) {$2}
    const stats = yield* storage.getStats()
    return stats
    ...
    return yield* Effect.fail(EntityPoolError('invalid_entity_id', `Entity ${id} is not allocated`))
    ...
    it.effect('エンティティ解放後にプールが再利用可能になる', () => Effect.gen(function* () {
    const pool = yield* EntityPool
    ...
    it.effect('freeList.length === 0 条件の境界値テスト', () => Effect.gen(function* () {
    const pool = yield* EntityPool
