...
describe('EntityManager - Effect-TS Pattern', () => {
  ...
  describe('Entity Creation and Destruction', () => {
    it.effect(
      'should create an entity with metadata',
      () =>
        Effect.gen(function* () {
    ...
    it.effect(
      'should destroy an entity and release its ID',
      () =>
        Effect.gen(function* () {
          const manager = yield* EntityManager
    ...
    it.effect(
      'should handle entity pool recycling',
      () =>
        Effect.gen(function* () {
          const manager = yield* EntityManager
  ...
  describe('Component Management', () => {
    it.effect(
      'should add and retrieve components',
      () =>
        Effect.gen(function* () {
          const manager = yield* EntityManager
          const entityId = yield* manager.createEntity()
    ...
    it.effect(
      'should get all components of an entity',
      () =>
        Effect.gen(function* () {
          const manager = yield* EntityManager
          const entityId = yield* manager.createEntity()
  ...
  describe('Query Operations', () => {
    it.effect(
      'should query entities by single component',
      () =>
        Effect.gen(function* () {
          const manager = yield* EntityManager
    ...
    it.effect(
      'should query entities by multiple components (AND)',
      () =>
        Effect.gen(function* () {
          const manager = yield* EntityManager
          const entity1 = yield* manager.createEntity()
  ...
  describe('Error Handling with it.effect', () => {
    it.effect(
      'should handle entity not found errors with Either',
      () =>
        Effect.gen(function* () {
          const manager = yield* EntityManager
    ...
    it.effect(
      'should handle component operations on non-existent entities',
      () =>
        Effect.gen(function* () {
          const manager = yield* EntityManager
          const invalidId = 99999 as EntityId
    ...
    it.effect(
      'should fail when setting active state of non-existent entity',
      () =>
        Effect.gen(function* () {
          const manager = yield* EntityManager
  ...
  describe('Legacy Property Testing', () => {
    it.effect(
      'should maintain component consistency',
      () =>
        Effect.gen(function* () {
          const manager = yield* EntityManager
  ...
  describe('Batch Operations and Advanced Features', () => {
    it.effect(
      'should batch get components efficiently',
      () =>
        Effect.gen(function* () {
          const manager = yield* EntityManager
          const entities: EntityId[] = []
    ...
    it.effect(
      'should iterate components efficiently',
      () =>
        Effect.gen(function* () {
          const manager = yield* EntityManager
          let totalX = 0
    ...
    it.effect(
      'should clear all entities and reset state',
      () =>
        Effect.gen(function* () {
          const manager = yield* EntityManager
  ...
  describe('Entity State Management', () => {
    it.effect(
      'should toggle entity active state',
      () =>
        Effect.gen(function* () {
          const manager = yield* EntityManager
          const entityId = yield* manager.createEntity()
    ...
    it.effect(
      'should handle non-existent tag searches',
      () =>
        Effect.gen(function* () {
          const manager = yield* EntityManager
    ...
    it.effect(
      'should handle system update calls',
      () =>
        Effect.gen(function* () {
          const manager = yield* EntityManager
    ...
    it.effect(
      'should remove entity from tag index when destroying entity with tags',
      () =>
        Effect.gen(function* () {
          const manager = yield* EntityManager
    ...
    it.effect(
      'should fail when removing component that is not registered',
      () =>
        Effect.gen(function* () {
          const manager = yield* EntityManager
    ...
    it.effect(
      'should handle multiple tags deletion during entity destruction',
      () =>
        Effect.gen(function* () {
          const manager = yield* EntityManager
    ...
    it.effect(
      'should handle edge case with empty tag set in tag index',
      () =>
        Effect.gen(function* () {
          const manager = yield* EntityManager
    ...
    it.effect(
      'should handle component removal when storage is missing during entity destruction',
      () =>
        Effect.gen(function* () {
          const manager = yield* EntityManager
    ...
    it.effect(
      'should handle tag index with undefined tag set during entity destruction',
      () =>
        Effect.gen(function* () {
          const manager = yield* EntityManager
    ...
    it.effect(
      'should clean up all components and tags when destroying complex entity',
      () =>
        Effect.gen(function* () {
          const manager = yield* EntityManager
  ...
  describe('Phase 3: Schema-based Testing', () => {
    it.effect(
      'should validate component schemas successfully',
      () =>
        Effect.gen(function* () {
    ...
    it.effect(
      'should validate EntityManagerError as tagged error',
      () =>
        Effect.gen(function* () {
          const manager = yield* EntityManager
          const invalidId = 99999 as EntityId
    ...
    it.effect(
      'should handle schema round-trip for components',
      () =>
        Effect.gen(function* () {
          const manager = yield* EntityManager
          const entity = yield* manager.createEntity()
  ...
  describe('Phase 3: Advanced Property-based Testing', () => {
    it.effect(
      'should maintain component invariants across random operations',
      () =>
        Effect.gen(function* () {
          const manager = yield* EntityManager
    ...
    it.effect(
      'should handle edge cases in component values',
      () =>
        Effect.gen(function* () {
          const manager = yield* EntityManager
    ...
    it.effect(
      'should maintain tag consistency across random entity operations',
      () =>
        Effect.gen(function* () {
          const manager = yield* EntityManager
  ...
  describe('Phase 3: Layer-based DI Integration Testing', () => {
    ...
    it.effect(
      'should work with proper layer dependency injection',
      () =>
        Effect.gen(function* () {
  ...
  describe('Phase 3: System Integration Testing', () => {
    it.effect(
      'should perform complete system test with all Phase 3 features',
      () =>
        Effect.gen(function* () {
  ...
  describe('Phase 3: Performance Testing with New Utils', () => {
    it.effect(
      'should maintain <50ms performance requirement',
      () =>
        Effect.gen(function* () {
