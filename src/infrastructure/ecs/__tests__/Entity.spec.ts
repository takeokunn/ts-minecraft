    ...
    it.effect('should clear all data', () =>
      Effect.gen(function* () {
        ...
        for (const [index, entity] of entities.entries()) {
          yield* storage.insert(entity, { x: index, y: index + 1, z: index + 2 })
        }

        const before = yield* storage.getStats()
        expect(before.size).toBe(BrandedTypes.createEntityCount(entities.length))

        yield* storage.clear()

        const stats = yield* storage.getStats()
        expect(stats.size).toBe(BrandedTypes.createEntityCount(0))
        expect(stats.capacity).toBe(BrandedTypes.createEntityCapacity(0))
        ...
        for (const entity of entities) {
          const has = yield* storage.has(entity)
