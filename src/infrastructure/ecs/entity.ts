...
export const createComponentStorage = <T>() => {
  ...
  const getAll = (): Effect.Effect<ReadonlyArray<[EntityId, T]>, never> =>
    Effect.sync(() =>
      Array.from({ length: array.size }, (_, index) => index).reduce<[EntityId, T][]>((acc, index) => {
        ...
        return maybePair._tag === 'Some' ? acc.concat([[maybePair.value.entity, maybePair.value.component]]) : acc
      }, [])
    )
