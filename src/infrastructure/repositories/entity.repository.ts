...
export interface IEntityRepository {
  ...
  readonly hasComponent: <T extends ComponentName>(
    entityId: EntityId, 
    componentName: T
  ) => Effect.Effect<boolean, never, never>
  ...
  // Query operations
  readonly findEntitiesByComponents: (
...
export class EntityRepository extends Context.GenericTag('EntityRepository')<
  EntityRepository,
  IEntityRepository
>() {}
...
export class EntityRepositoryImpl implements IEntityRepository {
  ...
  readonly destroyEntity = (entityId: EntityId): Effect.Effect<boolean, never, never> =>
    Effect.gen(function* (_) {
      ...
      // Update archetype tracking
      const newArchetypeToEntities = HashMap.modify(
        state.archetypeToEntities,
        metadata.archetypeKey,
  ...
  readonly addComponent = <T extends ComponentName>(
    ...
    Effect.gen(function* (_) {
      ...
      const newArchetypeToEntities = Option.match(oldArchetypeEntities, {
        ...
        onSome: (oldSet) => {
          const withoutOld = HashMap.set(
            state.archetypeToEntities,
            metadata.archetypeKey,
            HashSet.remove(oldSet, entityId)
          )
  ...
  readonly updateComponent = <T extends ComponentName>(
    ...
    Effect.gen(function* (_) {
      ...
      const newEntityMetadata = Option.match(metadataOpt, {
        ...
        onSome: (metadata) => HashMap.set(state.entityMetadata, entityId, {
          ...metadata,
          updatedAt: now,
          generation: metadata.generation + 1
        })
  ...
  readonly findEntitiesByComponents = (
    ...
    Effect.gen(function* (_) {
      ...
      const matchingEntities = Array.from(state.entityMetadata).filter(([_, metadata]) => {
  ...
  readonly findEntitiesByArchetype = (archetypeKey: string): Effect.Effect<ReadonlyArray<EntityMetadata>, never, never> =>
    Effect.gen(function* (_) {
      ...
      const metadata = entityIds.map(id => HashMap.get(state.entityMetadata, id))
    ...
    Effect.gen(function* (_) {
      ...
      const componentCounts = Object.fromEntries(
        Array.from(componentNamesSet).map(name => [
          name,
          HashMap.size(state.componentStorage[name])
        ])
      ) as Record<ComponentName, number>

      const archetypeCounts = Object.fromEntries(
        Array.from(state.archetypeToEntities).map(([key, entitySet]) => [
          key,
          HashSet.size(entitySet)
        ])
      )
  ...
  readonly compactStorage = (): Effect.Effect<void, never, never> =>
    Effect.gen(function* (_) {
      ...
      yield* _(Ref.update(this.stateRef, (s) => ({
        ...s,
        archetypeToEntities: HashMap.filter(s.archetypeToEntities, (entitySet) => 
          HashSet.size(entitySet) > 0
        )
      })))
    }.bind(this))
}
