...
export interface IChunkRepository {
  ...
  readonly getChunksInArea: (
    minX: number, 
    minZ: number, 
    maxX: number, 
    maxZ: number
  ) => Effect.Effect<ReadonlyArray<Chunk>, never, never>
  ...
  readonly updateBlocks: (
    ...
    updates: ReadonlyArray<{ index: number; blockType: BlockType }>
...
export class ChunkRepository extends Context.GenericTag('ChunkRepository')<
  ChunkRepository,
  IChunkRepository
>() {}
 ...
 * Helper functions
 */
const chunkKey = (coordinate: ChunkCoordinate): string => 
  `${coordinate.x},${coordinate.z}`

const regionKey = (coordinate: ChunkCoordinate, regionSize: number = 8): string =>
  `${Math.floor(coordinate.x / regionSize)},${Math.floor(coordinate.z / regionSize)}`
...
const calculateDistance = (a: ChunkCoordinate, b: ChunkCoordinate): number =>
...
export class ChunkRepositoryImpl implements IChunkRepository {
  ...
  readonly getChunk = (coordinate: ChunkCoordinate): Effect.Effect<Option.Option<Chunk>, never, never> =>
    Effect.gen(function* (_) {
      ...
      if (Option.isSome(chunk)) {
        yield* _(this.updateChunkMetadata(coordinate, (metadata) => ({
          ...metadata,
  ...
  readonly setChunk = (chunk: Chunk): Effect.Effect<void, never, never> =>
    Effect.gen(function* (_) {
      ...
      const newSpatialIndex = HashMap.set(
        state.spatialIndex, 
        regionKey_, 
        new Set([...existingRegion, key])
      )
  ...
  readonly removeChunk = (coordinate: ChunkCoordinate): Effect.Effect<boolean, never, never> =>
    Effect.gen(function* (_) {
      ...
      const newSpatialIndex = newRegion.size > 0
        ? HashMap.set(state.spatialIndex, regionKey_, newRegion)
        : HashMap.remove(state.spatialIndex, regionKey_)
  ...
  readonly updateChunkMetadata = (
    coordinate: ChunkCoordinate, 
    updater: (metadata: ChunkMetadata) => ChunkMetadata
  ): Effect.Effect<boolean, never, never> =>
    Effect.gen(function* (_) {
      const state = yield* _(Ref.get(this.stateRef))
      const key = chunkKey(coordinate)
    ...
    Effect.gen(function* (_) {
      ...
      if (chunkCoord) {
        changes = changes.filter(change => 
          change.chunkCoordinate.x === chunkCoord.x && change.chunkCoordinate.z === chunkCoord.z
        )
      }
  ...
  readonly validateChunkData = (coordinate: ChunkCoordinate): Effect.Effect<boolean, never, never> =>
    Effect.gen(function* (_) {
      ...
      const validBlocks = chunk.blocks.every(block => 
