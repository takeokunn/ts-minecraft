...
describe('greedyMeshChunk — property-based', () => {
  describe('indices are always within valid vertex range', () => {
    it('every index references an existing vertex for single-layer patterns', () => {
      fc.assert(
        fc.property(
          fc.uint8Array({ minLength: CHUNK_SIZE * CHUNK_SIZE, maxLength: CHUNK_SIZE * CHUNK_SIZE }),
          (layer) => {
    ...
    it('every index references an existing vertex for two-layer patterns', () => {
    ...
    it('index count is always a multiple of 3 (complete triangles)', () => {
      fc.assert(
        fc.property(
          fc.uint8Array({ minLength: CHUNK_SIZE * CHUNK_SIZE, maxLength: CHUNK_SIZE * CHUNK_SIZE }),
          (layer) => {
    ...
    it('vertex count is always a multiple of 4 (complete quads)', () => {
      fc.assert(
        fc.property(
          fc.uint8Array({ minLength: CHUNK_SIZE * CHUNK_SIZE, maxLength: CHUNK_SIZE * CHUNK_SIZE }),
          (layer) => {
  ...
  describe('array length consistency', () => {
    it('positions, normals, and colors always have equal lengths', () => {
      fc.assert(
        fc.property(
          fc.uint8Array({ minLength: CHUNK_SIZE * CHUNK_SIZE, maxLength: CHUNK_SIZE * CHUNK_SIZE }),
          (layer) => {
    ...
    it('uvs length is always 2/3 of positions length (2 UV coords per vertex)', () => {
      fc.assert(
        fc.property(
          fc.uint8Array({ minLength: CHUNK_SIZE * CHUNK_SIZE, maxLength: CHUNK_SIZE * CHUNK_SIZE }),
          (layer) => {
    ...
    it('index count is always (quadCount * 6) and vertex count is (quadCount * 4)', () => {
      fc.assert(
        fc.property(
          fc.uint8Array({ minLength: CHUNK_SIZE * CHUNK_SIZE, maxLength: CHUNK_SIZE * CHUNK_SIZE }),
          (layer) => {
  ...
  describe('all-air chunk', () => {
    ...
    it('empty chunk always produces zero geometry regardless of chunk coordinate offset', () => {
      fc.assert(
        fc.property(
  ...
  describe('output typed arrays', () => {
    it('always returns Float32Array for positions/normals/colors/uvs and Uint32Array for indices', () => {
      fc.assert(
        fc.property(
          fc.boolean(),
          (isEmpty) => {
  ...
  describe('world offset applied to vertex positions', () => {
    it('all X-coords are >= wx for any positive chunk coordinate', () => {
      fc.assert(
        fc.property(
