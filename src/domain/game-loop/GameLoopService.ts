...
export interface GameLoopService {
  ...
  ) => Effect.Effect<void, GameLoopInitError>
  ...
  // フレーム更新コールバックの登録
  readonly onFrame: (
    callback: (frameInfo: FrameInfo) => Effect.Effect<void>
  ) => Effect.Effect<() => void>
  ...
  readonly tick: (
    deltaTime?: number
  ) => Effect.Effect<FrameInfo, GameLoopRuntimeError>
