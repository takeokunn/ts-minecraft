...
describe('view_distance/types', () => {
  ...
  prop(
    'toRenderPriority clamps value within [0,1]',
  ...
  it.effect('decode functions accept valid inputs', () =>
    Effect.gen(function* () {
      const camera = yield* decodeCameraState(createCameraState({
        position: { x: 0, y: 1, z: 2 },
      ...
      const managed = yield* decodeManagedObject(createManagedObject({
        id: 'a',
      ...
      const cullable = yield* decodeCullableObject(createCullableObject({
        id: 'c',
        position: { x: 1, y: 0, z: 1 },
        boundingRadius: 1,
        priority: 0.3,
