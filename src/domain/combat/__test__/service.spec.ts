...
describe('combat/service', () => {
  ...
  it.effect('resolveAttack fails when cooldown remains', () =>
    Effect.gen(function* () {
      const { session, attacker, target } = yield* prepareSession()
      const attack = AttackKindFactory.magic(yield* makeDamage(100), yield* makeCooldown(4000), yield* makeCooldown(2000))
  ...
  it.prop(
    'resolved attacks never increase target health',
    [fc.integer({ min: 20, max: 240 })],
    ([base]) =>
      Effect.gen(function* () {
        const { session, attacker, target } = yield* prepareSession()
