...
import type { ArmorSlots, Inventory, InventoryState, ItemId, ItemStack, PlayerId } from './inventory-types'
...
export type InventoryServiceError =
  ...
  | {
      readonly _tag: 'InsufficientQuantity'
...
export interface InventoryService {
  ...
  readonly loadInventoryState: (state: InventoryState) => Effect.Effect<void, InventoryServiceError>
  readonly addItem: (
    playerId: PlayerId,
    item: ItemStack
  ...
  readonly setSelectedSlot: (playerId: PlayerId, hotbarIndex: number) => Effect.Effect<void, InventoryServiceError>
  readonly getSelectedItem: (playerId: PlayerId) => Effect.Effect<ItemStack | null, InventoryServiceError>
  ...
  readonly setOffhandItem: (playerId: PlayerId, item: ItemStack | null) => Effect.Effect<void, InventoryServiceError>
  readonly getOffhandItem: (playerId: PlayerId) => Effect.Effect<ItemStack | null, InventoryServiceError>
  readonly getEmptySlotCount: (playerId: PlayerId) => Effect.Effect<number, InventoryServiceError>
  readonly getUsedSlotCount: (playerId: PlayerId) => Effect.Effect<number, InventoryServiceError>
  readonly findItemSlots: (
    playerId: PlayerId,
    itemId: ItemId | string
  ) => Effect.Effect<ReadonlyArray<number>, InventoryServiceError>
  readonly countItem: (playerId: PlayerId, itemId: ItemId | string) => Effect.Effect<number, InventoryServiceError>
  ...
  readonly dropAllItems: (playerId: PlayerId) => Effect.Effect<ReadonlyArray<ItemStack>, InventoryServiceError>
  readonly clearInventory: (playerId: PlayerId) => Effect.Effect<void, InventoryServiceError>
}

export const InventoryService = Context.GenericTag<InventoryService>('@minecraft/domain/inventory/InventoryService')
