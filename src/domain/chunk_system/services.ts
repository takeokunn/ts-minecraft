...
export const ChunkSystemService = Context.Tag<ChunkSystemService>(
...
const createService = (
  ...
  Effect.gen(function* () {
    ...
    const initialState = yield* repository.load.pipe(
      Effect.orElse(() => makeInitialState(config))
    )
    const stateRef = yield* Ref.make(initialState)
    const persist = (result: TransitionResult) =>
      repository.save(result.state, result.events).pipe(
        Effect.zipRight(Ref.set(stateRef, result.state))
      )
