...
describe('chunk_system/model', () => {
  ...
  it.effect('capacity limit triggers ResourceBudgetExceeded', () =>
    Effect.gen(function* () {
      ...
      const exit = yield* Effect.either(
        applyCommand(consumed.state, ChunkCommand.Schedule({ request: secondRequest }))
      )
  ...
  it.effect('switch strategy only emits event when strategy changes', () =>
    Effect.gen(function* () {
      ...
      const changed = yield* applyCommand(
        state,
        ChunkCommand.SwitchStrategy({ strategy: 'alternate', decidedAt: 30 })
      )
