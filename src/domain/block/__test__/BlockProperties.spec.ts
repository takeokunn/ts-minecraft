...
describe('BlockProperties', () => {
  ...
  it.effect('デフォルトプロパティを提供する', () => Effect.gen(function* () {
    ...
    it.effect('withOpacityで不透明度を設定できる（0-15の範囲）', () => Effect.gen(function* () {
    ...
    it.effect('withFlammableで可燃性を設定できる', () => Effect.gen(function* () {
    ...
    it.effect('withTextureで任意のテクスチャを設定できる', () => Effect.gen(function* () {
  ...
  Effect.gen(function* () {
        ...
        it.effect('withTexturePerFaceで面別テクスチャを設定できる', () => Effect.gen(function* () {
    ...
    east: 'missing',
    west: 'missing',
    it.effect('withTexturePerFaceで既存テクスチャをデフォルトとして使用する', () => Effect.gen(function* () {
    const props = pipe(
    defaultBlockProperties,
    withTexture('oak_log'
    }),
  ...
  Effect.gen(function* () {
        const props = pipe(defaultBlockProperties, withDrop('wheat', 1, 1, 1.0
    }),
    withDrop('wheat_seeds', 0, 3, 1.0))
        ...
        it.effect('withDropSelfで自身をドロップする設定ができる', () => Effect.gen(function* () {
  ...
  it.effect('withSoundで個別のサウンドを設定できる', () => Effect.gen(function* () {
    ...
    it.effect('withSoundGroupでサウンドグループを設定できる', () => Effect.gen(function* () {
  ...
  it.effect('withTagでタグを追加できる', () => Effect.gen(function* () {
    const props = withTag('mineable')(defaultBlockProperties)
    expect(props.tags).toEqual(['mineable'])
    )
    it.effect('withTagで重複タグは追加されない', () => Effect.gen(function* () {
    const props = pipe(
    defaultBlockProperties,
    withTag('mineable'),
    withTag('stone'
    ...
    it.effect('stonePropertiesが石のプリセットを提供する', () => Effect.gen(function* () {
  ...
  Effect.gen(function* () {
    ...
    it.effect('woodPropertiesが木材のプリセットを提供する', () => Effect.gen(function* () {
    ...
    it.effect('glassPropertiesがガラスのプリセットを提供する', () => Effect.gen(function* () {
    ...
    it.effect('liquidPropertiesが液体のプリセットを提供する', () => Effect.gen(function* () {
    ...
    it.effect('unbreakablePropertiesが破壊不可のプリセットを提供する', () => Effect.gen(function* () {
  ...
  it.effect('pipeを使って複数の修飾子を合成できる', () => Effect.gen(function* () {
    const props = pipe(
    defaultBlockProperties,
    withHardness(2.0),
    withResistance(6.0),
