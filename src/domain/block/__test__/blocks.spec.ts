...
describe('blocks', () => {
  ...
  it.effect('53種類のブロックが定義されている', () => Effect.gen(function* () {
    expect(allBlocks.length).toBe(53)
    )
    it.effect('全てのブロックが必須フィールドを持つ', () => Effect.gen(function* () {
    pipe(
    allBlocks,
    ...
    it.effect('全てのブロックIDがユニーク', () => Effect.gen(function* () {
    const ids = pipe(
    allBlocks,
    ...
    it.effect('自然ブロックが正しくカテゴリー分けされている', () => Effect.gen(function* () {
    pipe(
    naturalBlocks,
    EffectArray.forEach((block) => {
    expect(block.category).toBe('natural')
    )
    it.effect('stoneBlockが正しく定義されている', () => Effect.gen(function* () {
    ...
    it.effect('grassBlockが面別テクスチャを持つ', () => Effect.gen(function* () {
    pipe(
    grassBlock.texture,
    Match.value,
    Match.when(
  ...
  Effect.gen(function* () {
        ...
        it.effect('木材は斧で採掘', () => Effect.gen(function* () {
    ...
    it.effect('鉱石ブロックが正しいツールレベルを持つ', () => Effect.gen(function* () {
    ...
    it.effect('鉱石ブロックがツルハシで採掘', () => Effect.gen(function* () {
    pipe(
    oreBlocks,
    EffectArray.forEach((block) => {
    expect(block.tool).toBe('pickaxe')
    )
    it.effect('鉱石ブロックがドロップアイテムを持つ', () => Effect.gen(function* () {
    pipe(
    oreBlocks,
    EffectArray.forEach((block) => {
    ...
    it.effect('技術ブロックがmiscellaneousカテゴリー', () => Effect.gen(function* () {
    pipe(
    technicalBlocks,
    EffectArray.forEach((block) => {
    expect(block.category).toBe('miscellaneous')
    )
    it.effect('かまどが面別テクスチャを持つ', () => Effect.gen(function* () {
    pipe(
  ...
  it.effect('全てのカテゴリーにブロックが存在する', () => Effect.gen(function* () {
    ...
    it.effect('カテゴリーが有効な値のみ', () => Effect.gen(function* () {
    ...
    it.effect('全てのブロックの光度が0-15の範囲', () => Effect.gen(function* () {
    pipe(
    allBlocks,
    EffectArray.forEach((block) => {
    ...
    it.effect('全てのブロックの不透明度が0-15の範囲', () => Effect.gen(function* () {
    pipe(
    allBlocks,
    EffectArray.forEach((block) => {
    ...
    it.effect('スタックサイズが1または64', () => Effect.gen(function* () {
    pipe(
    allBlocks,
    EffectArray.forEach((block) => {
    ...
    it.effect('全てのブロックが有効なツールタイプを持つ', () => Effect.gen(function* () {
    ...
    it.effect('ツールレベルが0-4の範囲', () => Effect.gen(function* () {
    pipe(
    allBlocks,
    EffectArray.forEach((block) => {
    ...
    it.effect('全てのブロックがサウンド設定を持つ', () => Effect.gen(function* () {
    pipe(
    allBlocks,
    EffectArray.forEach((block) => {
    ...
    it.effect('全てのブロックがテクスチャを持つ', () => Effect.gen(function* () {
    pipe(
    allBlocks,
    EffectArray.forEach((block) => {
    pipe(
