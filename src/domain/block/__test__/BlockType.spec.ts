...
describe('BlockType', () => {
  ...
  effectIt.scoped('BlockIdをBranded型として作成できる（Property-based）', () => Effect.gen(function* () {
  ...
  expect(id.length).toBe(value.length)
    ...
    effectIt.scoped('TextureIdをBranded型として作成できる（Property-based）', () => Effect.gen(function* () {
  ...
  effectIt.scoped('有効なツールタイプを検証できる（Property-based）', () => Effect.gen(function* () {
  ...
  expect(value).toBe(tool)
    ...
    effectIt.scoped('生成されたツールタイプは常に有効（Schemaベース）', () => Effect.gen(function* () {
        // 有効なツールタイプを使用したテスト
        const validTools = ['none', 'pickaxe', 'axe', 'shovel', 'hoe', 'shears', 'sword'] as const
    ...
    effectIt.scoped('無効なツールタイプを拒否する（Property-based）', () => Effect.gen(function* () {
  ...
  effectIt.scoped('6面のテクスチャを定義できる（Property-based）', () => Effect.gen(function* () {
  const testCases = [
  {
  top: 'grass_top',
  bottom: 'dirt',
  ...
  expect(value.west).toBe(west)
    ...
    effectIt.scoped('不完全なテクスチャ面定義を拒否する（Property-based）', () => Effect.gen(function* () {
  ...
  effectIt.scoped('単純なテクスチャ文字列を受け入れる（Property-based）', () => Effect.gen(function* () {
  ...
  expect(typeof value).toBe('string')
    ...
    effectIt.scoped('面別テクスチャオブジェクトを受け入れる（Property-based）', () => Effect.gen(function* () {
        const testCases = [
          {
            top: 'grass_top',
            bottom: 'dirt',
  ...
  effectIt.scoped('アイテムドロップを定義できる（Property-based）', () => Effect.gen(function* () {
  const testCases = [
  ...
  expect(value.chance).toBeGreaterThanOrEqual(0)
    ...
    it.effect('確率が範囲外でも受け入れる（バリデーションは別レイヤー）', () => Effect.gen(function* () {
    const itemDrop = {
    itemId: 'diamond',
    minCount: 1,
    maxCount: 3,
  ...
  it.effect('ブロックサウンドを定義できる', () => Effect.gen(function* () {
    const sound = {
  ...
  it.effect('ブロック物理特性を定義できる', () => Effect.gen(function* () {
    const physics = {
    hardness: 1.5,
    resistance: 6.0,
    luminance: 0,
  ...
  it.effect('有効なカテゴリーを検証できる', () => Effect.gen(function* () {
  ...
  it.effect('完全なブロック定義を検証できる', () => Effect.gen(function* () {
  ...
  it.effect('createDefaultPhysicsがデフォルト物理特性を返す', () => Effect.gen(function* () {
