  ...
  ;(describe('getBlock', () => {
          ...
          Either.match({
            onLeft: (error) =>
              Effect.sync(() => {
                ...
                expect((error as BlockNotFoundError).blockId).toBe(BrandedTypes.createBlockId('non_existent_block'))
              }),
            onRight: () => Effect.fail(new Error('Should have failed')),
          })
        )
      }).pipe(Effect.provide(BlockRegistryLive))
    )
    ...
    describe('Edge cases', () => {
      ...
      it.effect('複数のタグを持つブロックが各タグで検索可能', () =>
        Effect.gen(function* () {
          const registry = yield* BlockRegistry
          const multiTagBlock = createTestBlock({
            id: 'multi_tag',
            category: 'natural',
            tags: ['tag1', 'tag2', 'tag3'],
          })
