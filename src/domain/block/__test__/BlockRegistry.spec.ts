...
describe('BlockRegistry', () => {
  describe('getBlock', () => {
  it.effect('登録されているブロックを取得できる', () => Effect.gen(function* () {
    ...
    it.effect('存在しないブロックIDでエラーを返す', () => Effect.gen(function* () {
    const registry = yield* BlockRegistry
    ...
    it.effect('登録されているブロックをSomeで返す', () => Effect.gen(function* () {
    const registry = yield* BlockRegistry
    ...
    it.effect('存在しないブロックでNoneを返す', () => Effect.gen(function* () {
    const registry = yield* BlockRegistry
    ...
    it.effect('全てのブロックを取得できる', () => Effect.gen(function* () {
    const registry = yield* BlockRegistry
    ...
    it.effect('カテゴリーでブロックをフィルタリングできる', () => Effect.gen(function* () {
    const registry = yield* BlockRegistry
    ...
    it.effect('該当するブロックがない場合は空配列を返す', () => Effect.gen(function* () {
    const registry = yield* BlockRegistry
    ...
    it.effect('初期状態では全ブロックがタグなし', () => Effect.gen(function* () {
    const registry = yield* BlockRegistry
    const blocks = yield* registry.getBlocksByTag('mineable')
    expect(blocks).toEqual([])
}).pipe(Effect.provide(BlockRegistryLive))
    )
    it.effect('新しいブロックを登録した後、タグで検索できる', () => Effect.gen(function* () {
    const registry = yield* BlockRegistry
    ...
    it.effect('IDで検索できる', () => Effect.gen(function* () {
    const registry = yield* BlockRegistry
    ...
    it.effect('名前で検索できる', () => Effect.gen(function* () {
    const registry = yield* BlockRegistry
    ...
    it.effect('新しいブロックを登録できる', () => Effect.gen(function* () {
    const registry = yield* BlockRegistry
    ...
    it.effect('同じIDのブロックを登録しようとするとエラーを返す', () => Effect.gen(function* () {
    const registry = yield* BlockRegistry
    ...
    it.effect('既存のブロックと同じIDは拒否する', () => Effect.gen(function* () {
    const registry = yield* BlockRegistry
    ...
    it.effect('登録後はカテゴリーインデックスに反映される', () => Effect.gen(function* () {
    const registry = yield* BlockRegistry
    ...
    it.effect('登録後はタグインデックスに反映される', () => Effect.gen(function* () {
    const registry = yield* BlockRegistry
  ...
  it.effect('初期状態で53種類のブロックが登録されている', () => Effect.gen(function* () {
    const registry = yield* BlockRegistry
    const blocks = yield* registry.getAllBlocks()
    expect(blocks.length).toBe(53)
}).pipe(Effect.provide(BlockRegistryLive))
    )
    it.effect('主要なブロックが全て登録されている', () => Effect.gen(function* () {
    const registry = yield* BlockRegistry
    ...
    it.effect('カテゴリー分布が適切', () => Effect.gen(function* () {
    const registry = yield* BlockRegistry
    ...
    it.effect('外部API関数テスト - registerBlock（行202-206）とisBlockRegistered（行209-213）', () => Effect.gen(function* () {
    /*
    ...
    */
    ...
    it.effect('registerBlock関数でブロックを登録できる', () => Effect.gen(function* () {
    const registry = yield* BlockRegistry
    ...
    it.effect('isBlockRegistered関数で存在チェックができる', () => Effect.gen(function* () {
    const registry = yield* BlockRegistry
    // 存在しないブロック
    ...
    it.effect('searchBlocks関数で検索ができる', () => Effect.gen(function* () {
    const registry = yield* BlockRegistry
    ...
    it.effect('getBlocksByCategory関数でカテゴリー検索ができる', () => Effect.gen(function* () {
    const registry = yield* BlockRegistry
    const naturalBlocks = yield* registry.getBlocksByCategory('natural')
    expect(naturalBlocks.length).toBeGreaterThan(0)
    expect(naturalBlocks.every((b) => b.category === 'natural')).toBe(true)
    ...
    it.effect('getBlocksByTag関数でタグ検索ができる', () => Effect.gen(function* () {
    const registry = yield* BlockRegistry
    // タグ付きブロックを登録
    ...
    it.effect('module-level registerBlock function works', () => Effect.gen(function* () {
    ...
    it.effect('module-level isBlockRegistered function works', () => Effect.gen(function* () {
    const result = yield* isBlockRegistered(BrandedTypes.createBlockId('stone'))
    expect(result).toBe(true)
    }).pipe(Effect.provide(BlockRegistryLive))
    )
    it.effect('module-level searchBlocks function works', () => Effect.gen(function* () {
    const results = yield* searchBlocks('stone')
    expect(results.length).toBeGreaterThan(0)
    }).pipe(Effect.provide(BlockRegistryLive))
    )
    it.effect('module-level getBlocksByCategory function works', () => Effect.gen(function* () {
    const results = yield* getBlocksByCategory('natural')
    expect(results.length).toBeGreaterThan(0)
    }).pipe(Effect.provide(BlockRegistryLive))
    )
    it.effect('module-level getBlocksByTag function works', () => Effect.gen(function* () {
    ...
    describe('Edge cases', () => {
  it.effect('空文字列で検索すると全ブロックが返る', () => Effect.gen(function* () {
    const registry = yield* BlockRegistry
    ...
    it.effect('特殊文字を含む検索クエリを処理できる', () => Effect.gen(function* () {
    const registry = yield* BlockRegistry
    const blocks = yield* registry.searchBlocks('!@#$%')
    expect(blocks).toEqual([])
    }).pipe(Effect.provide(BlockRegistryLive))
    )
    it.effect('複数のタグを持つブロックが各タグで検索可能', () => Effect.gen(function* () {
    const registry = yield* BlockRegistry
  ...
  it.effect('完全に新しいカテゴリーでブロックを登録できる', () => Effect.gen(function* () {
    const registry = yield* BlockRegistry
    ...
    it.effect('複数の新しいカテゴリーでブロックを登録できる', () => Effect.gen(function* () {
    const registry = yield* BlockRegistry
    // 複数の新カテゴリーでブロックを作成
    ...
    it.effect('エクスポートされた getBlock 関数が正しく動作する（行168-171）', () => Effect.gen(function* () {
    ...
    it.effect('エクスポートされた getAllBlocks 関数が正しく動作する（行175-178）', () => Effect.gen(function* () {
    ...
    it.effect('エクスポートされた getBlocksByCategory 関数の完全なテスト（行181-185）', () => Effect.gen(function* () {
