...
describe('block_factory', () => {
  const withFactory = <A>(effect: Effect.Effect<A>) => effect.pipe(Effect.provideLayer(BlockFactoryLive))
  ...
  it.effect('kind指定でブロックを生成する', () =>
    withFactory(
      Effect.gen(function* () {
        const factory = yield* BlockFactory
        const block = yield* factory.fromKind({ kind: 'liquid', id: 'water', name: 'Water', viscosity: 1, flowRange: 4 })
