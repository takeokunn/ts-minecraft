...
describe('entities/types/core', () => {
  ...
  it('BlockPositionSchema enforces integral coordinates', () => {
    expect(Either.isRight(Schema.decodeUnknownEither(BlockPositionSchema)({ x: 1, y: 64, z: -5 }))).toBe(true)
    expect(Either.isLeft(Schema.decodeUnknownEither(BlockPositionSchema)({ x: 0.5, y: 64, z: -5 }))).toBe(true)
  })
