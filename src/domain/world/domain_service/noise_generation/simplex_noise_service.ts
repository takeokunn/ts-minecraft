...
import type { NoiseField, NoiseSample } from './perlin_noise_service.js'
...
export const SimplexNoiseConfigSchema = Schema.Struct({
  ...
  // スキュー・アンスキュー係数
  skewFactor: Schema.Number.pipe(Schema.finite(), Schema.between(-1, 1)).pipe(Schema.optional),
  unskewFactor: Schema.Number.pipe(Schema.finite(), Schema.between(-1, 1)).pipe(Schema.optional),
 ...
 */
export const SimplexNoiseService = Context.GenericTag<SimplexNoiseService>('@minecraft/domain/world/SimplexNoise')
 ...
 */
export const SimplexNoiseServiceLive = Layer.effect(
  SimplexNoiseService,
  Effect.succeed({
    ...
    sample4D: (coordinate, config) =>
      Effect.gen(function* () {
        ...
        const sample1 = yield* SimplexNoiseService.sample3D(coord1, config)
        const sample2 = yield* SimplexNoiseService.sample3D(coord2, config)
 ...
 * 2Dドット積
 */
const dot2D = (g: readonly number[], x: number, y: number): number => g[0] * x + g[1] * y

/**
 * 3Dドット積
 */
const dot3D = (g: readonly number[], x: number, y: number, z: number): number => g[0] * x + g[1] * y + g[2] * z
