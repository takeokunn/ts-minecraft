...
describe('WorldGenerator Interface', () => {
  ...
  it.effect(
  'デフォルトオプションでジェネレータを作成できる', () => Effect.gen(function* () {
  ...
  it.effect(
    '有効な座標でチャンクを生成できる', () => Effect.gen(function* () {
    const generator = yield* createWorldGenerator()
    ...
    it.effect(
      '構造物生成が有効な場合、構造物を生成できる', () => Effect.gen(function* () {
    ...
    it.effect(
    '構造物生成が無効な場合、エラーを返す', () => Effect.gen(function* () {
    ...
    it.effect(
    'すべての構造物タイプを生成できる', () => Effect.gen(function* () {
    const generator = yield* createWorldGenerator({ generateStructures: true
    })
  ...
  describe('ワールド情報取得', () => {
  it.effect(
  'スポーン地点を取得できる', () => Effect.gen(function* () {
    const generator = yield* createWorldGenerator()
    ...
    it.effect(
    'バイオーム情報を取得できる', () => Effect.gen(function* () {
    const generator = yield* createWorldGenerator()
    const position: Vector3 = { x: 0, y: 64, z: 0 }
    ...
    it.effect(
    '地形高さを取得できる', () => Effect.gen(function* () {
    const generator = yield* createWorldGenerator()
    ...
    it.effect(
      '構造物生成可能性をチェックできる', () => Effect.gen(function* () {
    const generator = yield* createWorldGenerator({ generateStructures: true
    })
    const position: Vector3 = { x: 0, y: 64, z: 0 }
    ...
    it.effect(
    '構造物生成が無効な場合、生成不可を返す', () => Effect.gen(function* () {
    const generator = yield* createWorldGenerator({ generateStructures: false
    })
    const position: Vector3 = { x: 0, y: 64, z: 0 }
    const canGenerate = yield* generator.canGenerateStructure('village', position)
    ...
    it.effect(
    '最近接構造物を検索できる', () => Effect.gen(function* () {
    const generator = yield* createWorldGenerator({ generateStructures: true
    })
    // まず構造物を生成
    ...
    it.effect(
    '範囲外の構造物は見つからない', () => Effect.gen(function* () {
    const generator = yield* createWorldGenerator({ generateStructures: true
    })
    // 遠い場所に構造物を生成
    ...
    it.effect('シード値を取得できる', () =>
{
  ...
  () => Effect.gen(function* () {
    ...
    it.effect(
      '無効な構造物タイプでの生成', () => Effect.gen(function* () {
    const generator = yield* createWorldGenerator()
    const position: Vector3 = { x: 0, y: 64, z: 0 }
