...
describe('NoiseGenerator', () => {
  ...
  it.effect('validates valid noise configuration', () => Effect.gen(function* () {
    const validConfig = {
    seed: 12345,
    octaves: 4,
    persistence: 0.5,
    lacunarity: 2.0,
}) Schema.decodeUnknownSync(NoiseConfigSchema)(validConfig)).not.toThrow()
    })

    it.effect('validates boundary values', () => Effect.gen(function* () {
  ...
  it.effect('creates NoiseGenerator with custom config', () => Effect.gen(function* () {
    const ng = yield* NoiseGeneratorTag
    ...
    it.effect('creates NoiseGenerator with default config', () => Effect.gen(function* () {
    const ng = yield* NoiseGeneratorTag
    ...
    it.effect('generates consistent 2D noise values for same input', () => Effect.gen(function* () {
    const ng = yield* NoiseGeneratorTag
    const testCoords = [
    { x: 0, y: 0 },
    { x: 1.5, y: 2.7 },
    ...
    it.effect('generates different values for different coordinates', () => Effect.gen(function* () {
    const ng = yield* NoiseGeneratorTag
    ...
    it.effect('generates consistent 3D noise values for same input', () => Effect.gen(function* () {
    const ng = yield* NoiseGeneratorTag
    ...
    it.effect('generates octave noise with different octave counts', () => Effect.gen(function* () {
    const ng = yield* NoiseGeneratorTag
    ...
    it.effect('generates identical results with same seed', () => Effect.gen(function* () {
    const ng = yield* NoiseGeneratorTag
    ...
    it.effect('handles zero octaves gracefully', () => Effect.gen(function* () {
    const ng = yield* NoiseGeneratorTag
