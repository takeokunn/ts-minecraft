...
describe('createWorldGenerator', () => {
  ...
  itEffect('creates world generator with default options', () => Effect.gen(function* () {
  ...
  expect(typeof generator.findNearestStructure).toBe('function')
    ...
    itEffect('creates world generator with custom options', () => Effect.gen(function* () {
    ...
    itEffect('provides consistent seed across calls', () => Effect.gen(function* () {
    ...
    itEffect('generates different seeds when not specified', () => Effect.gen(function* () {
  ...
  itEffect('provides default spawn point', () => Effect.gen(function* () {
  ...
  expect(spawnPoint).toEqual({ x: 0, y: 64, z: 0
    ...
    itEffect('returns consistent spawn point', () => Effect.gen(function* () {
        const generator = yield* createWorldGenerator(defaultOptions)
  ...
  itEffect('generates complete chunk data', () => Effect.gen(function* () {
  ...
  expect(Array.isArray(result.structures)).toBe(true)
    ...
    itEffect('generates different chunks for different positions', () => Effect.gen(function* () {
    ...
    itEffect('generates consistent chunks for same position', () => Effect.gen(function* () {
    ...
    itEffect('generates valid block placements', () => Effect.gen(function* () {
        const chunkPosition: ChunkPosition = { x: 0, z: 0 }
        const generator = yield* createWorldGenerator(defaultOptions)
        const result = yield* generator.generateChunk(chunkPosition)
  ...
  itEffect('provides terrain height for any coordinates', () => Effect.gen(function* () {
  const generator = yield* createWorldGenerator(defaultOptions)
  ...
  expect(height).toBeLessThanOrEqual(319)
    ...
    itEffect('provides consistent height values', () => Effect.gen(function* () {
        const x = 123
        const z = 456
        const generator = yield* createWorldGenerator(defaultOptions)
    ...
    itEffect('varies height across different coordinates', () => Effect.gen(function* () {
        const coordinates = [
          { x: 0, z: 0 },
          { x: 1000, z: 0 },
          { x: 0, z: 1000 },
  ...
  itEffect('provides biome information for any position', () => Effect.gen(function* () {
  ...
  expect(Number.isFinite(biome.elevation)).toBe(true)
    ...
    itEffect('provides consistent biome data for same position', () => Effect.gen(function* () {
  ...
  itEffect('generates structures when enabled', () => Effect.gen(function* () {
  ...
  expect(structures.length).toBeGreaterThanOrEqual(0)
    ...
    itEffect('prevents structure generation when disabled', () => Effect.gen(function* () {
    ...
    itEffect('respects individual feature flags', () => Effect.gen(function* () {
    ...
    itEffect('finds nearest structures', () => Effect.gen(function* () {
    ...
    itEffect('returns null when no structures found in range', () => Effect.gen(function* () {
        const centerPosition: Vector3 = { x: 0, y: 64, z: 0 }
  ...
  itEffect('generates valid chunks for arbitrary positions', () => Effect.gen(function* () {
  const position = { x: 5, z: 10 }
  const generator = yield* createWorldGenerator({ seed: 12345
}) // 固定シード
        const result = yield* generator.generateChunk(position)
    ...
    itEffect('provides consistent terrain heights for arbitrary coordinates', () => Effect.gen(function* () {
    ...
    itEffect('provides valid biomes for arbitrary positions', () => Effect.gen(function* () {
  ...
  describe('Edge Cases', () => {
  itEffect('handles extreme seed values', () => Effect.gen(function* () {
  const extremeSeeds = [0, 1, -1, Number.MAX_SAFE_INTEGER, Number.MIN_SAFE_INTEGER]
  for (
  const generator = yield* createWorldGenerator({ seed ) {$2}
  ...
  const chunk = yield* generator.generateChunk({ x: 0, z: 0
    ...
    itEffect('handles extreme sea level values', () => Effect.gen(function* () {
        const extremeSeaLevels = [-64, 0, 64, 319]

        for (
          const generator = yield* createWorldGenerator({ seaLevel ) {$2}
    ...
    itEffect('maintains state consistency across operations', () => Effect.gen(function* () {
        const generator = yield* createWorldGenerator(defaultOptions)
