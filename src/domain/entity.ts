...
export const getComponentUnsafe = <T extends Component>(
  ...
  if (!component) {
    throw new Error(
      `Component ${componentTag} not found on entity ${entity.id}`,
    );
  }
  return component;
};
...
export const query = <T extends Component['_tag'][]>(
  ...
  const result: QueryResult<{
    [K in keyof T]: Extract<Component, { _tag: T[K] }>;
  }>[] = [];
  for (const entity of entities.values()) {
    if (componentTags.every((tag) => hasComponent(entity, tag))) {
      const components = componentTags.map((tag) =>
        getComponentUnsafe(entity, tag),
      ) as { [K in keyof T]: Extract<Component, { _tag: T[K] }> };
