...
describe('MouseSensitivity', () => {
  ...
  it.effect('should get default configuration', () => Effect.gen(function* () {
    ...
    it.effect('should set configuration', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    ...
    it.effect('should apply sensitivity to mouse delta', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    ...
    it.effect('should set preset configuration', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    // Should not throw an error
    yield* mouseSensitivity.setPreset('high')
    }).pipe(Effect.provide(TestLayer))
    )
    it.effect('should set individual sensitivity values', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    // Should not throw an error
    ...
    it.effect('should set global multiplier', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    // Should not throw an error
    yield* mouseSensitivity.setGlobalMultiplier(1.8)
    }).pipe(Effect.provide(TestLayer))
    )
    it.effect('should invert axes', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    // Should not throw an error
    yield* mouseSensitivity.invertAxis(true, false)
    }).pipe(Effect.provide(TestLayer))
    )
    it.effect('should set sensitivity curve', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    // Should not throw an error
    yield* mouseSensitivity.setCurve('accelerated')
    }).pipe(Effect.provide(TestLayer))
    )
    it.effect('should reset to default configuration', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    // Should not throw an error
    ...
    const TestLayer = MockMouseSensitivity
    it.effect('should preserve original delta values', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    ...
    it.effect('should handle zero delta', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    ...
    it.effect('should handle config get error', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    ...
    it.effect('should handle sensitivity application error', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    const delta: MouseDelta = {
    deltaX: 10,
    deltaY: -5,
  ...
  it.effect('should ensure MouseSensitivityConfig interface is correctly typed', () => Effect.gen(function* () {
    ...
    it.effect('should ensure AdjustedMouseDelta interface is correctly typed', () => Effect.gen(function* () {
  ...
  it.effect('should create MouseSensitivityError with proper structure', () => Effect.gen(function* () {
    const error = MouseSensitivityError({
    message: 'Test error',
    config: { preset: 'invalid' },
})
).toBe('MouseSensitivityError')
      expect(error.message).toBe('Test error')
      expect(error.config).toEqual({ preset: 'invalid' })
    })

    it.effect('should create MouseSensitivityError without optional fields', () => Effect.gen(function* () {
    const error = MouseSensitivityError({
    message: 'Test error',
  })
).toBe('MouseSensitivityError')
  ...
  const TestLayer = MouseSensitivityLive
    ...
    it.effect('should get initial default configuration', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    const config = yield* mouseSensitivity.getConfig()
    expect(config).toEqual(defaultSensitivityConfig)
}).pipe(Effect.provide(TestLayer))
    )
    it.effect('should set and retrieve custom configuration', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    ...
    it.effect('should validate configuration and fail with invalid data', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    const invalidConfig = {
    ...defaultSensitivityConfig,
  ...
  it.effect(`should apply ${presetName} preset correctly`, () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    ...
    it.effect('should update individual sensitivity values', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    yield* mouseSensitivity.setSensitivity(
    ...
    it.effect('should enforce minimum sensitivity values', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    ...
    it.effect('should update global multiplier', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    ...
    it.effect('should enforce minimum global multiplier', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    ...
    it.effect('should apply linear curve correctly', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    ...
    it.effect('should apply accelerated curve correctly', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    const delta = createTestDelta(10, -5)
    yield* mouseSensitivity.setCurve('accelerated')
    const result = yield* mouseSensitivity.applySensitivity(delta)
    ...
    it.effect('should apply decelerated curve correctly', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    const delta = createTestDelta(10, -5)
    yield* mouseSensitivity.setCurve('decelerated')
    const result = yield* mouseSensitivity.applySensitivity(delta)
    ...
    it.effect('should apply custom curve with points', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    const customConfig: MouseSensitivityConfig = {
    ...defaultSensitivityConfig,
    curve: 'custom',
    ...
    it.effect('should fallback to linear when custom curve has no points', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    const customConfig: MouseSensitivityConfig = {
    ...defaultSensitivityConfig,
    curve: 'custom',
    ...
    it.effect('should apply dead zone correctly', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    ...
    it.effect('should apply axis inversion correctly', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    const delta = createTestDelta(10, -5)
    ...
    it.effect('should apply smoothing correctly', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    ...
    it.effect('should handle zero smoothing correctly', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    ...
    it.effect('should handle zero delta correctly', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    ...
    it.effect('should handle very large delta values', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    ...
    it.effect('should handle very small delta values', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    ...
    it.effect('should handle high sensitivity values correctly', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    ...
    it.effect('should create test delta correctly', () => Effect.gen(function* () {
  ...
  Effect.gen(function* () {const before = Date.now()
    ...
    it.effect('should validate delta expectations correctly', () => Effect.gen(function* () {
  ...
  it.effect('最後のセグメントに到達したときの処理をテスト (lines 164-170)', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    ...
    it.effect('カーブポイントが null の場合のフォールバック処理をテスト', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    ...
    it.effect('segmentIndex境界条件での onNone/onSome パス検証', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    ...
    it.effect('正規化された入力値=1.0での最終セグメント処理', () => Effect.gen(function* () {
    const mouseSensitivity = yield* MouseSensitivity
    // 複数ポイントカーブでの境界値テスト
