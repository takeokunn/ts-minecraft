import { describe, expect, as vitestIt, beforeEach, afterEach, vi } from 'vitest'
...
describe('MouseInput', () => {
  ...
  it.effect('should handle mouse move events', () => Effect.gen(function* () {
    ...
    it.effect('should handle mouse button state', () => Effect.gen(function* () {
    const mouseInput = yield* MouseInput
    // 基本的なボタン状態チェック
    ...
    it.effect('should handle pointer lock change events', () => Effect.gen(function* () {
    const mouseInput = yield* MouseInput
    // ポインターロック状態をモック
    ...
    it.effect('should request pointer lock on document body', () => Effect.gen(function* () {
    const mouseInput = yield* MouseInput
    ...
    it.effect('should request pointer lock on specific element', () => Effect.gen(function* () {
    const mouseInput = yield* MouseInput
    ...
    it.effect('should handle non-existent element error', () => Effect.gen(function* () {
    const mouseInput = yield* MouseInput
    ...
    it.effect('should exit pointer lock', () => Effect.gen(function* () {
    const mouseInput = yield* MouseInput
    // exitPointerLockをモック
    ...
    it.effect('should return default state for unpressed button', () => Effect.gen(function* () {
    const mouseInput = yield* MouseInput
    ...
    it.effect('should check different button states', () => Effect.gen(function* () {
    const mouseInput = yield* MouseInput
    // 各ボタンの状態をチェック
    ...
    it.effect('should reset delta values', () => Effect.gen(function* () {
    const mouseInput = yield* MouseInput
    // デルタをリセット
    ...
    it.effect('should handle missing document gracefully', () => Effect.gen(function* () {
    // documentを一時的に削除
    ...
    it.effect('should fail pointer lock without document', () => Effect.gen(function* () {
    const originalDocument = global.document
    delete (global as any).document
    try {
    const mouseInput = yield* MouseInput
    ...
    it.effect('should get mouse position', () => Effect.gen(function* () {
    const mouseInput = yield* MouseInput
    const position = yield* mouseInput.getPosition()
    ...
    it.effect('should get mouse delta', () => Effect.gen(function* () {
    const mouseInput = yield* MouseInput
    const delta = yield* mouseInput.getDelta()
    expect(typeof delta.deltaX).toBe('number')
    expect(typeof delta.deltaY).toBe('number')
    ...
    it.effect('should get button state', () => Effect.gen(function* () {
    const mouseInput = yield* MouseInput
    const buttonState = yield* mouseInput.getButtonState(0)
    ...
    it.effect('should check if button is pressed', () => Effect.gen(function* () {
    const mouseInput = yield* MouseInput
    const isPressed = yield* mouseInput.isButtonPressed(0)
    expect(typeof isPressed).toBe('boolean')
    }).pipe(Effect.provide(TestLayer))
    )
    it.effect('should request pointer lock', () => Effect.gen(function* () {
    const mouseInput = yield* MouseInput
    // Should not throw an error
    yield* mouseInput.requestPointerLock()
    }).pipe(Effect.provide(TestLayer))
    )
    it.effect('should request pointer lock with element ID', () => Effect.gen(function* () {
    const mouseInput = yield* MouseInput
    // Should not throw an error
    yield* mouseInput.requestPointerLock('game-canvas')
    }).pipe(Effect.provide(TestLayer))
    ...
    it.effect('should get pointer lock state', () => Effect.gen(function* () {
    const mouseInput = yield* MouseInput
    const state = yield* mouseInput.getPointerLockState()
    ...
    it.effect('should reset delta', () => Effect.gen(function* () {
    const mouseInput = yield* MouseInput
    // Should not throw an error
    yield* mouseInput.resetDelta()
    }).pipe(Effect.provide(TestLayer))
    ...
    message: 'Failed to get delta',
        ...
        exitPointerLock: () => Effect.fail(
            MouseInputError({
              message: 'Failed to exit pointer lock',
            })
),

        getPointerLockState: () => Effect.fail(
            MouseInputError({
              message: 'Failed to get pointer lock state',
            })
),

        resetDelta: () => Effect.fail(
            MouseInputError({
      ...
      it.effect('should handle position fetch error', () => Effect.gen(function* () {
    const mouseInput = yield* MouseInput
    ...
    it.effect('should handle delta fetch error', () => Effect.gen(function* () {
    const mouseInput = yield* MouseInput
    ...
    it.effect('should handle pointer lock request error', () => Effect.gen(function* () {
    const mouseInput = yield* MouseInput
    const result = yield* Effect.either(mouseInput.requestPointerLock('non-existent'))
    expect(result._tag).toBe('Left')
    if (result._tag === 'Left') {
    ...
    it.effect('should ensure MousePosition interface is correctly typed', () => Effect.gen(function* () {
    ...
    timestamp: Date.now(),
      ...
      it.effect('should ensure PointerLockState interface is correctly typed', () => Effect.gen(function* () {
    ...
    it.effect('should allow optional fields in PointerLockState', () => Effect.gen(function* () {
  ...
  it.effect('should create MouseInputError with proper structure', () => Effect.gen(function* () {
    const error = MouseInputError({
    message: 'Test error',
    cause: 'Test cause',
})
).toBe('MouseInputError')
        expect(error.message).toBe('Test error')
        expect(error.cause).toBe('Test cause')
      })

      it.effect('should create MouseInputError without optional fields', () => Effect.gen(function* () {
    const error = MouseInputError({
    message: 'Test error',
  })
).toBe('MouseInputError')
  ...
  Effect.gen(function* () {
      ...
      it.effect(
        'movementX/Y未対応ブラウザでのフォールバック処理をテスト', () => Effect.gen(function* () {
    const mouseInput = yield* MouseInput
    // 初期位置設定
    ...
    it.effect('typeof チェックによるmovementプロパティ検証', () => Effect.gen(function* () {
