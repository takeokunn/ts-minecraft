...
describe('InputService', () => {
  ...
  Effect.gen(function* () {
      ...
      getMouseDelta: () => Effect.gen(function* () {
          return {
            deltaX: 10.5,
            deltaY: -5.2,
            timestamp: Date.now(),
  ...
  Effect.gen(function* () {
    ...
    it.effect('should handle invalid key press check', () => Effect.gen(function* () {
    const inputService = yield* InputService
    ...
    it.effect('should check mouse button press state', () => Effect.gen(function* () {
    const inputService = yield* InputService
    ...
    it.effect('should handle invalid mouse button check', () => Effect.gen(function* () {
    const inputService = yield* InputService
    ...
    it.effect('should register input handler successfully', () => Effect.gen(function* () {
    const inputService = yield* InputService
    ...
    it.effect('should handle invalid handler registration', () => Effect.gen(function* () {
    const inputService = yield* InputService
    ...
    it.effect('should ensure InputHandler interface is correctly typed', () => Effect.gen(function* () {
    const handler: InputHandler = {
    onKeyDown: (key: string) => {
  ...
  it.effect('should create InputSystemError with proper structure', () => Effect.gen(function* () {
    const error = InputSystemError({
    message: 'Test error',
    key: 'test-key',
})
).toBe('InputSystemError')
      expect(error.message).toBe('Test error')
      expect(error.key).toBe('test-key')
    })

    it.effect('should create InputHandlerRegistrationError with proper structure', () => Effect.gen(function* () {
