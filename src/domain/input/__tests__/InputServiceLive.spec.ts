...
const waitUntil = (predicate: Effect.Effect<boolean>): Effect.Effect<void> =>
  Effect.gen(function* () {
    ...
    return yield* pipe(
      ...
      Match.orElse(() => Effect.sleep(Duration.millis(1)).pipe(Effect.flatMap(() => waitUntil(predicate))))
    )
  })
...
layer(InputServiceLive)('InputServiceLive', (it) => {
  ...
  it.effect('invokes registered handlers', () =>
    Effect.gen(function* () {
      ...
      yield* service.bindAction((_event, _snapshot) => Ref.update(counter, (value) => value + 1).pipe(Effect.asVoid))
  ...
  it.effect('tracks mouse delta updates', () =>
    Effect.gen(function* () {
      ...
      yield* waitUntil(
        service.latestMouseDelta().pipe(Effect.map((latest) => latest.deltaX === delta.x && latest.deltaY === delta.y))
      )
