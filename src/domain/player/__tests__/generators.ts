...
const decode =
  <A>(schema: Schema.Schema<unknown, A>) =>
  (value: unknown): A =>
...
export const gameModeArb = fc.constantFrom(...gameModeBase).map(decode(PlayerGameModeSchema))

const lifecycleBase = ['loading', 'alive', 'dead', 'respawning', 'teleporting', 'disconnected'] as const

export const lifecycleArb = fc.constantFrom(...lifecycleBase).map(decode(PlayerLifecycleStateSchema))
...
export const aggregateArb = fc
  ...
  .chain(([id, name, position, motion, vitals, state, gameMode]) =>
    fc.integer({ min: 0, max: 2 ** 31 }).chain((createdAt) =>
      fc.integer({ min: createdAt, max: createdAt + 10_000 }).map((updatedAt) =>
...
export const snapshotArb = aggregateArb.chain((aggregate) =>
  fc
    .integer({ min: aggregate.updatedAt, max: aggregate.updatedAt + 5000 })
    .map((capturedAt) => Schema.decodeUnknownSync(PlayerSnapshotSchema)({ aggregate, capturedAt }))
)
