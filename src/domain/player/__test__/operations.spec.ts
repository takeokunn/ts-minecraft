...
describe('player operations', () => {
  ...
  it('transitionStateは許可された遷移のみ成功する', async () => {
    const transitions: ReadonlyArray<readonly ['loading' | 'alive' | 'dead' | 'respawning' | 'teleporting' | 'disconnected', 'loading' | 'alive' | 'dead' | 'respawning' | 'teleporting' | 'disconnected']> = [
      ['loading', 'alive'],
      ['alive', 'dead'],
      ['dead', 'respawning'],
      ['respawning', 'alive'],
    ...
    await fc.assert(
      fc.asyncProperty(aggregateArb, fc.constantFrom(...transitions), fc.integer({ min: 1, max: 4000 }), async (aggregate, [from, to], delta) => {
  ...
  it('applyCommandでCreatePlayerを既存集約に適用すると失敗する', async () => {
    await fc.assert(
      fc.asyncProperty(aggregateArb, creationInputArb, async (aggregate, input) => {
        const exit = await Effect.runPromiseExit(applyCommand(aggregate, {
          _tag: 'CreatePlayer',
          id: aggregate.id,
          name: input.name,
  ...
  it('updatePositionは位置とモーションを更新する', async () => {
    await fc.assert(
      fc.asyncProperty(aggregateArb, positionArb, motionArb, fc.integer({ min: 1, max: 5000 }), async (aggregate, position, motion, delta) => {
        const timestamp = aggregate.updatedAt + Math.abs(delta)
  ...
  it('computeJumpVelocityはゲームモードに応じた速度となる', async () => {
    await fc.assert(
      fc.asyncProperty(fc.constantFrom<'survival' | 'creative' | 'spectator' | 'adventure'>('survival', 'creative', 'spectator', 'adventure'), async (mode) => {
