...
const makeHungerService = Effect.gen(function* () {
  // 並行安全な状態管理
  const hungerStatesRef = yield* Ref.make(
    HashMap.empty<PlayerId, HungerState>()
  )
  ...
  const initializePlayer = (playerId: PlayerId) =>
    Effect.gen(function* () {
      ...
      yield* Ref.set(
        hungerStatesRef,
        HashMap.set(states, playerId, initialState)
      )
  ...
  const consumeFood = (playerId: PlayerId, foodItem: FoodItem) =>
    Effect.gen(function* () {
      ...
      const newHunger = Math.min(
        HUNGER_CONSTANTS.MAX_HUNGER,
        state.hunger + foodItem.nutrition
      ) as HungerLevel
      ...
      yield* Ref.set(
        hungerStatesRef,
        HashMap.set(states, playerId, newState)
      )
      ...
      yield* Ref.set(eventsRef, [...events, event])
  ...
  // 空腹度減少処理
  const decreaseHunger = (playerId: PlayerId, reason: HungerDecreaseReason): Effect.Effect<HungerLevel, PlayerNotFoundError, never> =>
    Effect.gen(function* () {
      // 各アクションに応じた疲労度計算
  ...
  const canSprint = (playerId: PlayerId) =>
    Effect.gen(function* () {
      ...
      return state.hunger >= HUNGER_CONSTANTS.MIN_SPRINT_HUNGER
    })
  ...
  // 食料効果適用（簡易実装）
  const applyFoodEffects = (playerId: PlayerId, effects: Array<any>) =>
    Effect.gen(function* () {
  ...
  const starvationDamageStream = pipe(
    Stream.repeat(Schedule.fixed(HUNGER_CONSTANTS.STARVATION_DAMAGE_INTERVAL)),
    Stream.mapEffect(() =>
      Effect.gen(function* () {
        const states = yield* Ref.get(hungerStatesRef)

        // 全プレイヤーの飢餓状態をチェック
        const starvingPlayers = HashMap.filter(
          states,
          (state) => state.isStarving
        )

        yield* Effect.forEach(
          HashMap.values(starvingPlayers),
          (state) =>
            Effect.gen(function* () {
  ...
  // バックグラウンドタスクの起動
  yield* Effect.forkDaemon(starvationDamageStream)
 ...
 * 依存サービスの注入と初期化
 */
export const HungerServiceLive = Layer.effect(
  HungerService,
  makeHungerService
)