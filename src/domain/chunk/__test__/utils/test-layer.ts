...
import { Effect, TestContext, Layer, Ref, Random, TestClock, Duration } from 'effect'
import type {
  ChunkState,
  ChunkOperation,
  ChunkError,
 ...
 */
export class MockChunkDataGenerator extends Effect.Service<MockChunkDataGenerator>()(
  'MockChunkDataGenerator',
  {
    effect: Effect.succeed({
      generateEmptyChunk: (): ChunkDataBytes =>
        new Uint8Array(TEST_CONSTANTS.CHUNK_VOLUME).fill(0) as ChunkDataBytes,
 ...
 */
export class MockTimeProvider extends Effect.Service<MockTimeProvider>()(
  'MockTimeProvider',
  {
    effect: Effect.gen(function* () {
      ...
      return {
        ...
        setTime: (timestamp: number): Effect.Effect<void, never> =>
          Ref.set(baseTime, timestamp),

        resetTime: (): Effect.Effect<void, never> =>
          Ref.set(baseTime, Date.now())
      }
    })
  }
) {}
 ...
 */
export class MockMetricsCollector extends Effect.Service<MockMetricsCollector>()(
  'MockMetricsCollector',
  {
    effect: Effect.gen(function* () {
      ...
      return {
        ...
        getMetrics: (): Effect.Effect<typeof metrics extends Ref.Ref<infer T> ? T : never, never> =>
          Ref.get(metrics),
...
export const EffectTestHelpers = {
  ...
  testConcurrency: <A, E, R>(
    ...
    Effect.all(effects, { concurrency: maxConcurrency }),
  ...
  testWithRetry: <A, E, R>(
    ...
    effect.pipe(Effect.retry({ times: maxRetries }))
} as const
 ...
 * 最小テストレイヤー - 軽量テスト用
 */
export const ChunkMinimalTestLayer = Layer.mergeAll(
  TestContext.TestContext
)
