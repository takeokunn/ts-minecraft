...
import { ChunkDataOptics } from '../../aggregate/chunk/optics'
import type { ChunkData } from '../../aggregate/chunk_data/types'
import type { HeightValue, ChunkMetadata } from '../../value_object/chunk-metadata'
...
describe('ParallelChunkTraversalOptics', () => {
  describe('並列ブロック更新', () => {
    it('並列ブロック更新が正常に実行される', () => {
      ...
      const result = Effect.runSync(ParallelChunkTraversalOptics.parallelBlockUpdate(chunk, updates))

      expect(ChunkDataOptics.blockAt(0).get(result)).toBe(7)
      expect(ChunkDataOptics.blockAt(32).get(result)).toBe(8)
      expect(ChunkDataOptics.blockAt(0).get(chunk)).toBe(0)
    })

    it('大量の並列更新が効率的に処理される', () => {
      const chunk = createTestChunkData()
      const updates = Array.from({ length: 1024 }, (_, index) => ({
        index,
        blockId: (index % 5) + 1,
      }))
  ...
  describe('並列高さマップ更新', () => {
    it('並列高さマップ更新が正常に実行される', () => {
      const chunk = createTestChunkData()
      const heightUpdates = [
        { index: 0, height: makeHeightValue(90) },
        { index: 10, height: makeHeightValue(95) },
      ]

      const result = Effect.runSync(
        ParallelChunkTraversalOptics.parallelHeightMapUpdate(chunk, heightUpdates)
      )

      expect(ChunkDataOptics.heightMapAt(0).get(result)).toBe(makeHeightValue(90))
      expect(ChunkDataOptics.heightMapAt(10).get(result)).toBe(makeHeightValue(95))
      expect(ChunkDataOptics.heightMapAt(0).get(chunk)).toBe(64)
    })
  })

  describe('条件付き並列変換', () => {
    it('条件付き並列ブロック変換が正常に動作する', () => {
      const chunk = createTestChunkData(createVariedBlocks())
      ...
      Array.from({ length: 20 }, (_, index) => index).forEach((index) => {
        const original = ChunkDataOptics.blockAt(index).get(chunk)
        const transformed = ChunkDataOptics.blockAt(index).get(result)
        if (original % 2 === 0) {
          expect(transformed).toBe(original + 10)
        } else {
