...
import { Effect, Layer, TestContext, Schema } from 'effect'
  ...
  effect: Effect.gen(function* () {
    return {
      ...
      unloadChunk: (position: ChunkPosition) =>
        Effect.succeed({ position, unloaded: true }),
...
describe('Chunk System - Simple Effect-TS Patterns', () => {
  ...
  describe('Error Handling Patterns', () => {
    it.effect('should handle errors gracefully with Either', () =>
      Effect.gen(function* () {
        const chunkService = yield* ChunkService
        const position = yield* ChunkPosition.create(5, 5)

        const result = yield* Effect.either(
          chunkService.isChunkLoaded(position).pipe(
            Effect.flatMap((isLoaded) =>
              isLoaded
                ? Effect.succeed('already loaded')
        ...
        expect(result._tag).toBe('Left')
        if (result._tag === 'Left') {
          expect(result.left.message).toContain('not loaded')
        }
      }).pipe(Effect.provide(TestLayer))
    )
  })
