...
import { executeChunkOperation } from '../../aggregate/chunk/pattern-matching'
...
describe('ChunkOperation ADT Tests', () => {
  ...
  describe('ChunkOperation Execution', () => {
    it('should execute Read operation', async () => {
      ...
      expect(message).toContain('チャンク読み込み')
    })

    it('should execute Write operation', async () => {
      const operation = ChunkOperations.write(mockPosition, mockData, mockMetadata)
      const message = await Effect.runPromise(executeChunkOperation(operation))
      expect(message).toContain('書き込み')
      expect(message).toContain(String(mockData.length))
    })
    ...
    it('should execute Validate operation without checksum', async () => {
      ...
      const message = await Effect.runPromise(executeChunkOperation(operation))
      expect(message).toContain('チェックサムなし')
    })

    it('should execute Validate operation with checksum', async () => {
      const operation = ChunkOperations.validate(mockPosition, 'checksum')
      const message = await Effect.runPromise(executeChunkOperation(operation))
  ...
  describe('OptimizationStrategy Pattern Matching', () => {
    it('should handle Memory optimization strategy', async () => {
      ...
      const message = await Effect.runPromise(executeChunkOperation(operation))
      expect(message).toContain('メモリ最適化')
    })

    it('should handle Compression optimization strategy', async () => {
      const operation = ChunkOperations.optimize(mockPosition, OptimizationStrategy.Compression())
      const message = await Effect.runPromise(executeChunkOperation(operation))
      expect(message).toContain('圧縮最適化')
    })
    ...
    it('should handle Speed optimization strategy', async () => {
      ...
      const message = await Effect.runPromise(executeChunkOperation(operation))
      expect(message).toContain('速度最適化')
    })
  })
  ...
  describe('SerializationFormat Pattern Matching', () => {
    it('should handle Binary serialization format', async () => {
      const operation = ChunkOperations.serialize(mockData, SerializationFormat.Binary(), mockMetadata)
      const message = await Effect.runPromise(executeChunkOperation(operation))
      expect(message).toContain('バイナリ')
    })

    it('should handle JSON serialization format', async () => {
      const operation = ChunkOperations.serialize(mockData, SerializationFormat.JSON(), mockMetadata)
      const message = await Effect.runPromise(executeChunkOperation(operation))
      expect(message).toContain('JSON')
    })

    it('should handle Compressed serialization format with algorithm', async () => {
      const operation = ChunkOperations.serialize(
        mockData,
        SerializationFormat.Compressed({ algorithm: 'lz4' }),
        mockMetadata
      )
      const message = await Effect.runPromise(executeChunkOperation(operation))
      expect(message).toContain('lz4')
    })
  })
  ...
  describe('Complex Operation Flow Scenarios', () => {
    it('should handle sequential operations', async () => {
      ...
      expect(results[1]).toContain('書き込み')
      expect(results[2]).toContain('削除')
    })

    it('should handle operation variants with different complexity', async () => {
      const complexOperation = ChunkOperations.serialize(
        mockData,
        SerializationFormat.Compressed({ algorithm: 'gzip' }),
        mockMetadata
      )

      const simpleOperation = ChunkOperations.read(mockPosition)

      const [complexMessage, simpleMessage] = await Promise.all([
        Effect.runPromise(executeChunkOperation(complexOperation)),
        Effect.runPromise(executeChunkOperation(simpleOperation)),
      ])

      expect(complexMessage).toContain('gzip')
      expect(simpleMessage).toContain('読み込み')
    })
  })
