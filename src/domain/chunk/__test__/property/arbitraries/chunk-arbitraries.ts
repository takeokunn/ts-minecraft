...
import { ChunkStates, ChunkOperations, ChunkErrors } from '../../../types/core'
import type { ChunkPosition, ChunkDistance, ChunkHash } from '../../../value_object/chunk_position/types'
 ...
 * 仕様に準拠したチャンクデータバイト配列
 */
export const chunkDataBytesArb: fc.Arbitrary<ChunkDataBytes> = fc.uint8Array({
 ...
 * 進行度の型安全生成
 */
export const loadProgressArb: fc.Arbitrary<LoadProgress> = fc.integer({
  min: 0,
  max: 100
 ...
 * 非負整数のリトライ回数
 */
export const retryCountArb: fc.Arbitrary<RetryCount> = fc.integer({
  min: 0,
  max: 10
 ...
 * 現実的なタイムスタンプ範囲
 */
export const chunkTimestampArb: fc.Arbitrary<ChunkTimestamp> = fc.integer({
 ...
 * 非負有限数の距離値
 */
export const chunkDistanceArb: fc.Arbitrary<ChunkDistance> = fc.double({
  min: 0,
  max: 1000000,
  noNaN: true
 ...
 * SHA-256ライクなハッシュ文字列
 */
export const chunkHashArb: fc.Arbitrary<ChunkHash> = fc.string({
...
export const chunkStateArb: fc.Arbitrary<ChunkState> = fc.oneof(
  ...
  // Loaded
  fc.tuple(chunkDataBytesArb, chunkMetadataArb).map(([data, metadata]) =>
    ChunkStates.loaded(data, metadata)
  ),

  // Failed
  fc.tuple(fc.string(), retryCountArb).map(([error, retryCount]) =>
    ChunkStates.failed(error, retryCount)
  ),

  // Dirty
  fc.tuple(chunkDataBytesArb, changeSetArb, chunkMetadataArb).map(([data, changes, metadata]) =>
    ChunkStates.dirty(data, changes, metadata)
  ),

  // Saving
  fc.tuple(chunkDataBytesArb, loadProgressArb, chunkMetadataArb).map(([data, progress, metadata]) =>
    ChunkStates.saving(data, progress, metadata)
  ),

  // Cached
  fc.tuple(chunkDataBytesArb, chunkMetadataArb).map(([data, metadata]) =>
    ChunkStates.cached(data, metadata)
  )
)
...
export const chunkOperationArb: fc.Arbitrary<ChunkOperation> = fc.oneof(
  ...
  // Write
  fc.tuple(chunkPositionArb, chunkDataBytesArb, chunkMetadataArb).map(([position, data, metadata]) =>
  ...
  // Validate
  fc.tuple(chunkPositionArb, fc.option(fc.string())).map(([position, checksum]) =>
    ChunkOperations.validate(position, checksum)
  ),

  // Optimize
  fc.tuple(chunkPositionArb, optimizationStrategyArb).map(([position, strategy]) =>
    ChunkOperations.optimize(position, strategy)
  ),

  // Serialize
  fc.tuple(chunkDataBytesArb, serializationFormatArb, chunkMetadataArb).map(([data, format, metadata]) =>
    ChunkOperations.serialize(data, format, metadata)
  )
)
...
export const chunkErrorArb: fc.Arbitrary<ChunkError> = fc.oneof(
  // ValidationError
  fc.tuple(fc.string(), fc.anything(), fc.string()).map(([field, value, constraint]) =>
  ...
  fc.tuple(
    ...
    ChunkErrors.bounds(coordinates, bounds)
  ),

  // SerializationError
  fc.tuple(fc.string(), fc.anything()).map(([format, originalError]) =>
    ChunkErrors.serialization(format, originalError)
  ),

  // CorruptionError
  fc.tuple(fc.string(), fc.string()).map(([checksum, expected]) =>
    ChunkErrors.corruption(checksum, expected)
  ),

  // TimeoutError
  fc.tuple(fc.string(), fc.integer({ min: 0 })).map(([operation, duration]) =>
    ChunkErrors.timeout(operation, duration)
  ),

  // NetworkError
  fc.tuple(fc.webUrl(), fc.integer({ min: 100, max: 599 })).map(([url, status]) =>
    ChunkErrors.network(url, status)
  )
)
