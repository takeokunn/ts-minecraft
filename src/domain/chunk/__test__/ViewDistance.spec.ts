...
describe('Performance Analysis Functions', () => {
  ...
  it.effect('履歴が不足している場合は全て安定を返す', () => Effect.gen(function* () {
  ...
  Effect.gen(function* () {const config = { ...defaultViewDistanceConfig, adjustmentThreshold: 0.1 }
        // 古いメトリクス: 低いフレームレート
    ...
    it.effect('メモリ使用量悪化を正しく検出する', () => Effect.gen(function* () {
  ...
  const config = defaultViewDistanceConfig
    it.effect('低フレームレートで描画距離を減らす', () => Effect.gen(function* () {
    const metrics = createMockMetrics({ frameRate: 30
}) // 目標60の50%
  ...
  Effect.gen(function* () {const metrics = createMockMetrics({ memoryUsageMB: 1800 }) // 上限2000の90%
        const result = calculateOptimalViewDistance(16, metrics, config)
        expect(result.suggestedDistance).toBeLessThan(16)
        expect(result.reason).toBe('memory_high')
        expect(result.confidence).toBeGreaterThan(0.8)  })
    it.effect('高ロード時間で描画距離を減らす', () => Effect.gen(function* () {
    const metrics = createMockMetrics({ averageChunkLoadTimeMs: 300
  })
).toBeLessThan(16)
    expect(result.reason).toBe('load_time_high')
    expect(result.confidence).toBeGreaterThan(0.6)  })
    it.effect('良好なパフォーマンスで描画距離を増やす', () => Effect.gen(function* () {
    const metrics = createMockMetrics({
    frameRate: 75, // 目標の125%
    ...
    it.effect('最小・最大描画距離の制限を守る', () => Effect.gen(function* () {
...
describe('Chunk Position Functions', () => {
  ...
  it.effect('描画距離0では中心チャンクのみ返す', () => Effect.gen(function* () {
  ...
  Effect.gen(function* () {const center = { x: 0, y: 64, z: 0 }
        ...
        expect(positions).toContainEqual({ x: 1, z: 1 }) // 角  })
    it.effect('描画距離2では正しい数のチャンクを返す', () => Effect.gen(function* () {
  ...
  it.effect('プレイヤーに近いチャンクほど高い優先度を持つ', () => Effect.gen(function* () {
  ...
  Effect.gen(function* () {const playerPos = { x: 0, y: 64, z: 0 }
        const viewDistance = 4
    ...
    it.effect('優先度が0-1の範囲内にある', () => Effect.gen(function* () {
    const playerPos = { x: 0, y: 64, z: 0 }
    const viewDistance = 4
...
describe('ViewDistance Service', () => {
  ...
  it.effect('デフォルト設定でサービスを作成できる', () => Effect.gen(function* () {
    ...
    it.effect('描画距離を設定できる', () => Effect.gen(function* () {
    const viewDistance = yield* ViewDistance
    ...
    it.effect('設定時に最小・最大値で制限される', () => Effect.gen(function* () {
    const viewDistance = yield* ViewDistance
    // 最小値未満
    ...
    it.effect('同じ値の設定では履歴が更新されない', () => Effect.gen(function* () {
    const viewDistance = yield* ViewDistance
    ...
    it.effect('パフォーマンスメトリクスを更新できる', () => Effect.gen(function* () {
    const viewDistance = yield* ViewDistance
    const metrics = createMockMetrics({ frameRate: 45
        ...
        it.effect('複数のメトリクスを履歴として保持する', () => Effect.gen(function* () {
    const viewDistance = yield* ViewDistance
        ...
        it.effect('履歴サイズの制限が適用される', () =>
{
    ...
    // 制限を超えるメトリクスを追加
    for (
    yield* viewDistance.updateMetrics(createMockMetrics({ frameRate: 60 + i ) {$2
  })
        const stats = yield* viewDistance.getPerformanceStats()
        // 最新の3つのみ保持されていることを確認
  ...
  ) {$2}.pipe(Effect.provide(ViewDistanceLive(config)))
  it.effect('メトリクス履歴が不足している場合はnullを返す', () => Effect.gen(function* () {
    const viewDistance = yield* ViewDistance
    // 少ないメトリクスのみ追加
    yield* viewDistance.updateMetrics(createMockMetrics())
    const adjustment = yield* viewDistance.performAdaptiveAdjustment()
    ...
    it.effect('低パフォーマンスで自動的に描画距離を減らす', () => Effect.gen(function* () {
    const viewDistance = yield* ViewDistance
    // 低パフォーマンスメトリクスを追加
    for (
    yield* viewDistance.updateMetrics(createMockMetrics({ frameRate: 30 ) {$2
  })
  ...
  it.effect('現在の描画距離に基づいて見えるチャンクを返す', () => Effect.gen(function* () {
    const viewDistance = yield* ViewDistance
  ...
  it.effect('優先度付きでチャンクを返し、高い順にソートされる', () => Effect.gen(function* () {
  const viewDistance = yield* ViewDistance
  ...
  ) {$2}.pipe(Effect.provide(ViewDistanceLive()))
  )
  describe('updateConfig', () => {
  it.effect('設定を部分的に更新できる', () => Effect.gen(function* () {
    const viewDistance = yield* ViewDistance
  ...
  it.effect('パフォーマンス統計を正しく取得できる', () => Effect.gen(function* () {
    const viewDistance = yield* ViewDistance
    // メトリクスを追加
  ...
  it.effect('完全なワークフロー: メトリクス更新→適応調整→チャンク取得', () => Effect.gen(function* () {
  const viewDistance = yield* ViewDistance
  // 初期状態
