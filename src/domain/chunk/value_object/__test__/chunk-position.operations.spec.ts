...
const effectProperty = <T>(
  ...
  fc.asyncProperty(arbitrary, (value) => Effect.runPromise(predicate(value)).then(() => true))
      ...
      effectProperty(
        ...
        ({ minX, minZ, width, height }) =>
          Effect.sync(() => {
            ...
            expect(ReadonlyArray.last(positions)).toStrictEqual(
              Option.some(createChunkPositionSync(maxX, maxZ))
            )
          })
      ),
      { numRuns: 100 }
