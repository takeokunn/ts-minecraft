...
export const OptimizedChunkOptics = {
  ...
  lazyBlockUpdate: (
    ...
    Effect.gen(function* () {
      ...
      const lazyUpdates = updates.map(({ index, blockId }) =>
        Effect.sync(() => ChunkDataOptics.blockAt(index).replace(blockId))
      )
   ...
   */
  batchBlockUpdate: (chunk: ChunkData, blockUpdates: ReadonlyMap<number, number>): ChunkData => {
    // MapをArrayに変換してtraverseで効率的に処理
    const updates = Array.from(blockUpdates.entries())

    // 更新を1回のパスで適用（copy-on-writeを最小化）
    return updates.reduce((acc, [index, blockId]) => ChunkDataOptics.blockAt(index).replace(blockId)(acc), chunk)
  },
                          ...
                          Effect.sync(() => {
                            ...
                            return pipe(
                              srcIndex < sourceChunk.blocks.length &&
                                ...
                                tgtZ < 16,
                              Match.value,
                              Match.when(true, () => updates.set(tgtIndex, sourceChunk.blocks[srcIndex])),
                              Match.when(false, () => {}),
                              Match.exhaustive
                            )
                          })
...
export const StreamingChunkOptics = {
  ...
  streamBlockProcessing: (
    ...
    Effect.gen(function* () {
      ...
      return pipe(chunk, ChunkDataOptics.blocks.replace(new Uint16Array(processedBlocks)))
    }),
