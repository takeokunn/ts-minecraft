...
import { ChunkValidationService, ChunkValidationServiceLive } from '../chunk-validator'
import { ChunkBoundsError, ChunkDataSchema } from '../../aggregate/chunk'
  ...
  describe('validateChecksum', () => {
    it('有効なチェックサムを検証できる', async () => {
      ...
      const expectedChecksum = await Effect.gen(function* () {
        const service = yield* ChunkValidationService
        const serialization = yield* import('../chunk-serializer').then(m => m.ChunkSerializationService)
        return yield* serialization.calculateChecksum(data)
      }).pipe(
        Effect.provide(Layer.mergeAll(TestLayer, import('../chunk-serializer').then(m => m.ChunkSerializationServiceLive))),
        Effect.runPromise
      )
