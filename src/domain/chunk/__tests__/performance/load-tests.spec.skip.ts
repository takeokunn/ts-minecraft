...
class LoadBalancer extends Effect.Service<LoadBalancer>()('LoadBalancer', {
  effect: Effect.gen(function* () {
    ...
    return {
      ...
      clear: () => Effect.all([Ref.set(activeConnections, 0), Ref.set(requestQueue, [])]),
    }
  }),
}) {}
  ...
  effect: Effect.gen(function* () {
    ...
    return {
      allocateChunks: (count: number) =>
        Effect.gen(function* () {
          const chunks = Array.from(
            { length: count },
            (_, i) => new Uint8Array(CHUNK_VOLUME).fill(i % 256) as ChunkDataBytes
          )
      ...
      deallocateChunks: (count: number) =>
        Effect.gen(function* () {
          yield* Ref.update(allocatedChunks, (chunks) => chunks.slice(Math.min(count, chunks.length)))
        }),

      getAllocatedCount: () => Effect.map(Ref.get(allocatedChunks), (chunks) => chunks.length),

      getMemoryUsage: () => Effect.sync(() => process.memoryUsage()),
    ...
    it.effect('メモリプレッシャー下でのパフォーマンス', () =>
      Effect.gen(function* () {
        ...
        const pressureFiber = yield* Fiber.fork(
          Effect.forever(memoryStress.simulateMemoryPressure(0.7).pipe(Effect.delay(Duration.millis(100))))
        )
    ...
    it.effect('並行性制限・背圧制御テスト', () =>
      Effect.gen(function* () {
        ...
        const submissionFiber = yield* Fiber.fork(
          Effect.all(
            operations.map((operation) => loadBalancer.submitOperation(operation)),
          ...
          Fiber.fork(
            Effect.gen(function* () {
              for (let i = 0; i < operationsPerFiber; i++) {
                ...
                yield* simulateChunkOperation(ChunkOperations.read({ x: fiberIndex, z: i }))
      ...
      Effect.gen(function* () {
        ...
        const processed = yield* Stream.range(0, streamSize).pipe(
          ...
          Stream.mapEffect((batch) =>
            Effect.gen(function* () {
              const start = Date.now()
              yield* Effect.all(batch.map(simulateChunkOperation), { concurrency: batchSize })
              const end = Date.now()
              yield* monitor.recordOperation(end - start, true)
              return batch.length
            })
