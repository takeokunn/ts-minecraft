...
import { CHUNK_VOLUME, ChunkErrors, type ChunkDataBytes } from '../../types/core'
    ...
    return {
      ...
      load: (position: ChunkPosition) =>
        Effect.gen(function* () {
          ...
          return data ? Effect.succeed(data) : Effect.fail(ChunkErrors.network(`chunk://${key}`, 404))
        }).pipe(Effect.flatten),
    ...
    validatePosition: (position: ChunkPosition) =>
      Effect.gen(function* () {
        if (!Number.isInteger(position.x) || !Number.isInteger(position.z)) {
          yield* Effect.fail(ChunkErrors.validation('position', position, 'coordinates must be integers'))
        }
        if (Math.abs(position.x) > 30000000 || Math.abs(position.z) > 30000000) {
          yield* Effect.fail(
            ChunkErrors.bounds({ x: position.x, y: 0, z: position.z }, { min: -30000000, max: 30000000 })
          )
        }
        return true
      }),
    ...
    validateData: (data: ChunkDataBytes) =>
      Effect.gen(function* () {
        if (data.length !== CHUNK_VOLUME) {
          yield* Effect.fail(ChunkErrors.validation('data', data.length, `must be ${CHUNK_VOLUME} bytes`))
        }
        if (!data.every((byte) => byte >= 0 && byte <= 255)) {
          yield* Effect.fail(ChunkErrors.validation('data', 'invalid', 'all bytes must be 0-255'))
        }
        return true
      }),
    ...
    return {
      ...
      getEvents: (position: ChunkPosition) =>
        Effect.gen(function* () {
          ...
          return allEvents.filter((event) => event.position.x === position.x && event.position.z === position.z)
        }),
  ...
  describe('Complete Chunk Lifecycle Integration', () => {
    ...
    it.effect('複数チャンク並行処理統合テスト', () =>
      Effect.gen(function* () {
        ...
        const chunkDataList = positions.map((_, index) => new Uint8Array(CHUNK_VOLUME).fill(index) as ChunkDataBytes)
        ...
        // 並行保存
        yield* Effect.all(
          positions.map((position, index) => repository.save(position, chunkDataList[index])),
          { concurrency: 'unbounded' }
        )
  ...
  describe('Error Handling Integration', () => {
    it.effect('統合エラーハンドリングテスト', () =>
      Effect.gen(function* () {
        ...
        const validationResult = yield* Effect.either(validator.validatePosition(invalidPosition))
        expect(validationResult._tag).toBe('Left')

        // 存在しないチャンクの読み込みエラー
        const nonExistentPosition: ChunkPosition = { x: 999, z: 999 }
        const loadResult = yield* Effect.either(repository.load(nonExistentPosition))
        expect(loadResult._tag).toBe('Left')

        // 無効なデータでのエラーハンドリング
        const invalidData = new Uint8Array(100) as ChunkDataBytes // 不正サイズ
        const dataValidationResult = yield* Effect.either(validator.validateData(invalidData))
    ...
    it.effect('リカバリー戦略統合テスト', () =>
      Effect.gen(function* () {
        ...
        // エラー検出とリカバリー
        const corruptionValidation = yield* Effect.either(validator.validateData(corruptedData))
