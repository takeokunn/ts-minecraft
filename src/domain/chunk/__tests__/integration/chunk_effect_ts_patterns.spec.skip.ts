...
import { Effect, Layer, Random, Schema, TestClock, TestContext } from 'effect'
...
class PlayerService extends Effect.Service<PlayerService>()('PlayerService', {
  effect: Effect.gen(function* () {
    return {
      getPosition: () => Effect.succeed({ x: 8.5, z: 12.3 }), // プレイヤー座標
    ...
    it.effect('should handle service errors gracefully', () =>
      Effect.gen(function* () {
        ...
        pipe(
          result,
          Match.value,
          Match.when({ _tag: 'Left' }, (error) => {
            // エラーケースのテスト
            expect(error.left).toBeDefined()
          }),
          Match.when({ _tag: 'Right' }, (success) => {
            // 成功ケースのテスト
  ...
  describe('Resource Management', () => {
    it.scoped('should manage chunk lifecycle with scoped resources', () =>
      Effect.gen(function* () {
        ...
        const resource = yield* Effect.acquireRelease(chunkService.loadChunk(position), (chunk) =>
          chunkService.unloadChunk(chunk.position)
        )
