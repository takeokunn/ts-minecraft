...
import { CHUNK_VOLUME, ChunkErrors, ChunkOperations, type ChunkDataBytes, type ChunkOperation } from '../../types/core'
  ...
  describe('Fiber Management & Control', () => {
    it.effect('Fiber並行実行・協調的中断テスト', () =>
      Effect.gen(function* () {
        ...
        // 残りFiberの完了待機
        const remainingResults = yield* Effect.all(fibers.slice(2).map(Fiber.join), { concurrency: 'unbounded' })
    ...
    it.effect('高度なSchedule・リトライ戦略テスト', () =>
      Effect.gen(function* () {
        ...
        const retryStrategies = [
          {
            name: 'exponential',
            schedule: Schedule.exponential(Duration.millis(10), 2.0).pipe(Schedule.compose(Schedule.recurs(3))),
          },
          {
            name: 'linear',
            schedule: Schedule.linear(Duration.millis(20)).pipe(Schedule.compose(Schedule.recurs(3))),
          },
          {
            name: 'fibonacci',
            schedule: Schedule.fibonacci(Duration.millis(5)).pipe(Schedule.compose(Schedule.recurs(4))),
          },
        ]
          ...
          retryStrategies.map(({ name, schedule }) =>
            Effect.gen(function* () {
              ...
              const operation = Effect.gen(function* () {
                attempts++
                yield* Ref.update(attemptCounts, (map) => new Map(map.set(name, attempts)))
    ...
    it.effect('条件付きSchedule・複雑なリトライロジック', () =>
      Effect.gen(function* () {
        ...
        const conditionalRetrySchedule = Schedule.whileInput((error: unknown) => {
          ...
          return false
        }).pipe(Schedule.compose(Schedule.exponential(Duration.millis(50))), Schedule.compose(Schedule.recurs(5)))
        ...
        const results = yield* Effect.all(
          operations.map((op) => op.pipe(Effect.retry(conditionalRetrySchedule), Effect.either)),
          { concurrency: 'unbounded' }
        )
        ...
        yield* Stream.fromIterable(mixedOperations).pipe(
          Stream.mapEffect(
            (operation) =>
              ...
              }).pipe(Effect.catchAll((error) => Effect.succeed(`error-handled-${error.message}`))),
            { concurrency: 20 }
          ),
          Stream.runCollect
        )
  ...
  describe('Deferred & Synchronization', () => {
    it.effect('Deferred・非同期調整・同期プリミティブ', () =>
      Effect.gen(function* () {
        ...
        const results = yield* Ref.make<Array<string>>([])
        const completionSignals = yield* Effect.all(Array.from({ length: 5 }, () => Deferred.make<string>()))
