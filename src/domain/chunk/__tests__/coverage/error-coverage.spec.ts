...
import { Cause, Effect, Either, Exit, TestContext } from 'effect'
import { describe, expect, it } from 'vitest'
import { CHUNK_VOLUME, ChunkErrors, type ChunkError } from '../../types/core'
  ...
  describe('Effect Error Handling Coverage', () => {
    ...
    it.effect('Effect.retry - リトライパス', () =>
      Effect.gen(function* () {
        ...
        const result = yield* flakyEffect.pipe(Effect.retry({ times: 5 }), Effect.either)
        ...
        for (const invalidData of invalidDataSizes) {
          const validationEffect = Effect.sync(() => {
            if (invalidData.length !== CHUNK_VOLUME) {
              throw ChunkErrors.validation('chunkData', invalidData.length, `must be ${CHUNK_VOLUME} bytes`)
            }
            return invalidData
          })
        ...
        for (const progress of invalidProgresses) {
          const validationEffect = Effect.sync(() => {
            if (progress < 0 || progress > 100 || !Number.isFinite(progress)) {
              throw ChunkErrors.validation('progress', progress, 'must be finite number 0-100')
            }
            return progress
          })
