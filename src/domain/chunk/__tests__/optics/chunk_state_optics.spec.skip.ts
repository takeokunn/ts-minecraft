...
import type { ChangeSet, ChunkDataBytes, ChunkState, ChunkTimestamp, LoadProgress, RetryCount } from '../../types/core'
...
describe('ParallelChunkStateOptics', () => {
  describe('並列状態処理', () => {
    it('複数状態の並列更新ができる', async () => {
      const states = [createLoadingState(), createLoadedState(), createFailedState()]

      const operation = (state: ChunkState) =>
        Effect.succeed(
          state._tag === 'Loading' ? ChunkStateOpticsHelpers.updateLoadingProgress(state, 100 as LoadProgress) : state
        )

      const result = await Effect.runPromise(ParallelChunkStateOptics.parallelUpdate(states, operation))
    ...
    it('状態統計の並列計算ができる', async () => {
      ...
      const statistics = await Effect.runPromise(ParallelChunkStateOptics.parallelStatistics(states))
...
describe('SafeChunkStateOptics', () => {
  describe('安全な状態操作', () => {
    it('安全なプロパティアクセスができる', () => {
      ...
      // 存在するプロパティへのアクセス
      const progress = SafeChunkStateOptics.safeGet(ChunkStateOptics.loadingProgress, fallbackValue)(loadingState)

      expect(progress).toBe(50)

      // 存在しないプロパティへのアクセス（loadedStateからloadingProgress）
      const loadedState = createLoadedState()
      const safeProgress = SafeChunkStateOptics.safeGet(ChunkStateOptics.loadingProgress, fallbackValue)(loadedState)
    ...
    it('安全なプロパティ更新ができる', () => {
      ...
      const result = SafeChunkStateOptics.safeSet(ChunkStateOptics.loadingProgress, 90 as LoadProgress)(loadingState)
...
describe('状態遷移テスト', () => {
  it('型安全な状態遷移が正常に動作する', () => {
    ...
    // 進行度を100に設定して再試行
    const completedState = ChunkStateOpticsHelpers.updateLoadingProgress(loadingState, 100 as LoadProgress)
