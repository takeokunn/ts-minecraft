...
describe('ChunkTraversalOptics', () => {
  ...
  describe('条件付きブロックTraversal', () => {
    it('特定の条件を満たすブロックのみを選択できる', () => {
      ...
      // 手動で条件確認
      const oddBlocks = Array.from(chunk.blocks).filter((blockId, index) => isOddBlock(blockId, index))
...
describe('ParallelChunkTraversalOptics', () => {
  ...
  describe('並列統計計算', () => {
    it('ブロック統計の並列計算が正常に動作する', async () => {
      ...
      const statistics = await Effect.runPromise(ParallelChunkTraversalOptics.parallelBlockStatistics(chunk))
...
describe('エラーハンドリング', () => {
  ...
  it('空の更新配列でも正常に処理される', async () => {
    ...
    const result = await Effect.runPromise(ParallelChunkTraversalOptics.parallelBlockUpdate(chunk, []))
