...
import { SerializationFormat } from '../../domain_service'
import { ChunkFactoryService } from '../chunk_factory'
...
const createSerializedChunkData = async () => {
  ...
  return await Effect.gen(function* () {
    const { ChunkSerializationService } = yield* import('../../domain_service')
...
describe('ChunkFactoryService', () => {
  describe('createValidatedChunk', () => {
    ...
    it('無効な位置でエラーを返す', async () => {
      ...
      }).pipe(Effect.provide(ChunkCompleteLayer), Effect.flip, Effect.runPromise)
