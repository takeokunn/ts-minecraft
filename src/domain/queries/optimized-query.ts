...
import { QueryEntity, QueryConfig, QueryMetrics, startQueryContext, finalizeQueryContext } from './types'
 ...
 */
const addEntityToIndex = (entity: QueryEntity, indexRef: Ref.Ref<ComponentIndexState>): Effect.Effect<void> =>
  Effect.gen(function* () {
 ...
 */
const removeEntityFromIndex = (entity: QueryEntity, indexRef: Ref.Ref<ComponentIndexState>): Effect.Effect<void> =>
  Effect.gen(function* () {
    const state = yield* Ref.get(indexRef)
 ...
 */
const getEntitiesWithComponent = (component: ComponentName, indexRef: Ref.Ref<ComponentIndexState>): Effect.Effect<ReadonlyArray<QueryEntity>> =>
  Effect.gen(function* () {
    const state = yield* Ref.get(indexRef)
 ...
 */
const getEntitiesWithAllComponents = (components: ReadonlyArray<ComponentName>, indexRef: Ref.Ref<ComponentIndexState>): Effect.Effect<ReadonlyArray<QueryEntity>> =>
  Effect.gen(function* () {
    ...
    const componentSizes = components
      .map((comp) => ({
        component: comp,
 ...
 */
const getIndexStats = (indexRef: Ref.Ref<ComponentIndexState>): Effect.Effect<ComponentIndexStats> =>
  Effect.gen(function* () {
    const state = yield* Ref.get(indexRef)
...
const createExecutionPlan = <T extends ReadonlyArray<ComponentName>>(
  ...
  Effect.gen(function* () {
    ...
    if (config.predicate) {
      plan.cacheKey = keyGenerator.forComponents(config.withComponents, config.withoutComponents, keyGenerator.hashPredicate(config.predicate))
    } else {
      plan.cacheKey = keyGenerator.forComponents(config.withComponents, config.withoutComponents)
    }
  ...
  Effect.gen(function* () {
    ...
    if (config.withoutComponents && config.withoutComponents.length > 0) {
      candidates = candidates.filter((entity) => {
        return !config.withoutComponents!.some((comp) => entity.components[comp] !== undefined)
      })
    }
      ...
      const batchResults = yield* Effect.all(
        batch.map((entity) =>
          Effect.gen(function* () {
            ...
            try {
              return yield* Effect.promise(() => Promise.resolve(config.predicate!(entityProxy as any)))
            } catch (error) {
 ...
 */
const addEntityToOptimization = (entity: QueryEntity): Effect.Effect<void> =>
  Effect.gen(function* () {
    const componentIndex = yield* ComponentIndexService
    const archetypeQuery = yield* ArchetypeQueryService
 ...
 */
const removeEntityFromOptimization = (entity: QueryEntity): Effect.Effect<void> =>
  Effect.gen(function* () {
    const componentIndex = yield* ComponentIndexService
    const archetypeQuery = yield* ArchetypeQueryService
 ...
 */
const updateEntityInOptimization = (entity: QueryEntity): Effect.Effect<void> =>
  Effect.gen(function* () {
    const componentIndex = yield* ComponentIndexService
    const archetypeQuery = yield* ArchetypeQueryService
 ...
 */
const invalidateOptimizationCache = (modifiedComponents: ComponentName[]): Effect.Effect<number> =>
  Effect.gen(function* () {
    const queryCache = yield* QueryCacheService
 ...
 */
const clearExecutionPlan = (queryName: string, stateRef: Ref.Ref<OptimizedQueryState>): Effect.Effect<void> =>
  Effect.gen(function* () {
    yield* Ref.update(stateRef, (state) => ({
      ...state,
...
export const OptimizedQueryServiceLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    return OptimizedQueryService.of({
      execute: <T extends ReadonlyArray<ComponentName>>(config: QueryConfig<T>, entities?: ReadonlyArray<QueryEntity>) => executeOptimizedQuery(config, entities, stateRef),
