...
export interface ISpatialGridPort {
  ...
  // Batch operations
  readonly insertMany: (
    entities: ReadonlyArray<{ entityId: EntityId; bounds: AABB }>
  ...
  // Range queries
  readonly queryRadius: (
    center: { x: number; y: number; z: number },
    radius: number
  ) => Effect.Effect<ReadonlySet<EntityId>, never, never>
  
  // Nearest neighbor queries
  readonly queryNearest: (
    position: { x: number; y: number; z: number },
    maxResults?: number
  ) => Effect.Effect<ReadonlyArray<EntityId>, never, never>
}

export class SpatialGridPort extends Context.GenericTag('SpatialGridPort')<
  SpatialGridPort,
  ISpatialGridPort
>() {}