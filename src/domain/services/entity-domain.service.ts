...
export interface EntityRepositoryPort {
  readonly saveEntity: (entity: Entity) => Effect.Effect<void, never, never>
  readonly loadEntity: (entityId: EntityId) => Effect.Effect<Option.Option<Entity>, never, never>
  readonly deleteEntity: (entityId: EntityId) => Effect.Effect<void, never, never>
}

export interface EntityQueryPort {
  readonly queryByComponents: (components: readonly ComponentName[]) => Effect.Effect<readonly EntityId[], never, never>
  readonly queryInRadius: (center: Position, radius: number) => Effect.Effect<readonly EntityId[], never, never>
}

// ===== ENTITY DOMAIN SERVICE INTERFACE =====

export interface EntityDomainServiceInterface {
  // Entity lifecycle
