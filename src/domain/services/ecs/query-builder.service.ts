...
class QueryBuilderImpl implements QueryBuilder {
  ...
  execute(): Effect.Effect<ReadonlyArray<Entity>> {
    return Effect.gen(function* () {
      ...
      const result = yield* this.queryService.execute(
        this.name,
        required,
        forbidden,
        this.buildPredicate()
  ...
  first(): Effect.Effect<Option.Option<Entity>> {
    return Effect.map(
      this.execute(),
      entities => ReadonlyArray.head(entities)
    )
  }
  ...
  parallel(batchSize: number = 100): Effect.Effect<ReadonlyArray<Entity>> {
    return Effect.gen(function* () {
      ...
      const batches = Chunk.fromIterable(entities).pipe(
        Chunk.chunksOf(batchSize)
      )
      
      const results = yield* Effect.forEach(
        batches,
        batch => Effect.succeed(Chunk.toReadonlyArray(batch)),
...
export const QueryBuilderServiceLive = Layer.effect(
  ...
  Effect.gen(function* () {
      ...
      Effect.gen(function* () {
        ...
        const execute = (entities?: ReadonlyArray<Entity>) =>
          Effect.gen(function* () {
            ...
            const result = yield* queryService.execute(
              'compiled_query',
              components.required,
    ...
    // Helper functions
    const extractComponents = (expr: QueryExpression): {
    ...
    const formatExpression = (expr: QueryExpression, indent: number): string => {
      ...
      switch (expr.type) {
        ...
        case 'node':
          const children = expr.value.children
            .map(child => formatExpression(child, indent + 1))
            .join('\n')
...
export const spatialQuery = (
  ...
  Effect.gen(function* () {
    const service = yield* QueryBuilderService
    return service.create('spatial_query')
...
export const tagQuery = (tags: string[]) =>
  Effect.gen(function* () {
    const service = yield* QueryBuilderService
    return service.create('tag_query')
...
export const hierarchyQuery = (parentId: EntityId) =>
  Effect.gen(function* () {
    const service = yield* QueryBuilderService
    return service.create('hierarchy_query')
