...
import { EntityId, ComponentName, ComponentData } from './archetype-query.service'
...
export class ComponentValidationError extends S.TaggedError<ComponentValidationError>()(
...
export interface ComponentService {
  ...
  readonly validate: (
    name: ComponentName,
    data: unknown
  ) => Effect.Effect<ComponentData, ComponentValidationError>
      ...
      Effect.gen(function* () {
        ...
        for (const tag of metadata.tags) {
          yield* Ref.update(tagIndex, index =>
            HashMap.modify(index, tag, set =>
              Option.getOrElse(set, () => HashSet.empty<ComponentName>()).pipe(
                HashSet.add(metadata.name)
              )
            )
          )
        }
          ...
          onSome: meta =>
            Effect.gen(function* () {
              ...
              for (const tag of meta.tags) {
                yield* Ref.update(tagIndex, index =>
                  HashMap.modify(index, tag, set =>
                    Option.map(set, s => HashSet.remove(s, name))
                  )
                )
              }
            })
...
export interface ComponentPoolService {
  readonly createPool: <T extends ComponentData>(
    componentName: ComponentName,
    factory: () => T,
    reset: (item: T) => void,
    maxSize: number
  ...
  readonly acquire: <T extends ComponentData>(
    componentName: ComponentName
  ) => Effect.Effect<T, ComponentError>
  
  readonly release: <T extends ComponentData>(
    componentName: ComponentName,
    item: T
  ) => Effect.Effect<void>
...
export const ComponentPoolServiceLive = Layer.effect(
  ...
  Effect.gen(function* () {
    const pools = yield* Ref.make(HashMap.empty<ComponentName, {
    ...
    const createPool = <T extends ComponentData>(
      componentName: ComponentName,
      factory: () => T,
      reset: (item: T) => void,
      maxSize: number
...
export const validateComponents = (
  components: ReadonlyArray<{ name: ComponentName; data: unknown }>
) =>
  Effect.gen(function* () {
    const service = yield* ComponentService
    const results = yield* Effect.forEach(
      components,
      ({ name, data }) => service.validate(name, data),
      { concurrency: 'unbounded' }
    )
    return results
  })
...
export const ComponentSystemLive = Layer.merge(
