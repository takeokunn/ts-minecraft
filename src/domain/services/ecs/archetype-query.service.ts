...
export interface ArchetypeService {
  ...
  readonly findMatchingArchetypes: (
    signature: ArchetypeSignature
  ) => Effect.Effect<ReadonlyArray<Archetype>>
...
export const ArchetypeServiceLive = Layer.effect(
  ...
  Effect.gen(function* () {
          ...
          const index = yield* Option.match(existingIndex, {
            onNone: () =>
              Effect.gen(function* () {
                ...
                yield* Ref.update(componentIndices, map =>
                  HashMap.set(map, component, idx)
                )
                return idx
              }),
    ...
    const getOrCreateArchetype = (
      signature: ArchetypeSignature
    ): Effect.Effect<Archetype, ArchetypeError> =>
      Effect.gen(function* () {
          ...
          onNone: () =>
            Effect.gen(function* () {
              ...
              yield* Ref.update(archetypes, map =>
                HashMap.set(map, signature.hash, newArchetype)
              )
    ...
    const addEntity = (entity: Entity): Effect.Effect<void, ArchetypeError> =>
      Effect.gen(function* () {
        ...
        yield* Ref.update(entityToArchetype, map =>
          HashMap.set(map, entity.id, signature.hash)
        )
      })
...
export const batchQuery = <T extends ReadonlyArray<ComponentName>>(
  queries: ReadonlyArray<{ name: string; components: T }>
) =>
  Effect.gen(function* () {
    const results = yield* parallelQueries(
      queries.map(q => createQuery(q.name, q.components).execute())
    )
