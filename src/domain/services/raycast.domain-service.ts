...
/**
 * Raycast Domain Service Interface - Pure domain raycast calculations
 */
export interface IRaycastDomainService {
...
/**
 * Raycast Domain Service Context Tag
 */
export const RaycastDomainService = Context.GenericTag<IRaycastDomainService>('RaycastDomainService')

/**
 * Raycast Domain Service Live Implementation
 */
export const RaycastDomainServiceLive = Layer.effect(
  RaycastDomainService,
  Effect.gen(function* () {
    
    const validateRay = (ray: Ray): Effect.Effect<boolean> =>
      Effect.succeed(
        ray.maxDistance > 0 && 
    ...
    const normalizeRayDirection = (direction: Vector3): Effect.Effect<Vector3> => {
    ...
    const calculateRayAABBIntersection = (ray: Ray, aabb: AABB): Effect.Effect<Option.Option<number>> =>
      Effect.gen(function* () {
        const normalizedDir = yield* normalizeRayDirection(ray.direction)
    ...
    const calculateRayPlaneIntersection = (ray: Ray, planePoint: Vector3, planeNormal: Vector3): Effect.Effect<Option.Option<number>> =>
      Effect.gen(function* () {
        const normalizedDir = yield* normalizeRayDirection(ray.direction)
    ...
    const isPointInRayRange = (ray: Ray, distance: number): Effect.Effect<boolean> =>
      Effect.succeed(distance >= 0 && distance <= ray.maxDistance)

    return {
      validateRay,
      normalizeRayDirection,
      calculateRayAABBIntersection,
      calculateRayPlaneIntersection,
      isPointInRayRange,
    }
  }),
)
