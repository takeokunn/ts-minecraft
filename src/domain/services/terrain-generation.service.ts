...
/**
 * Multi-octave fractal noise implementation
 * Pure functional module for generating natural-looking noise
 */
export const DomainNoise = {
  /**
   * Generate fractal brownian motion noise
   */
  fbm: (x: number, y: number, z: number, settings: AdvancedNoiseSettings, seed: number): number => {
    let value = 0
    let amplitude = 1
    ...
    for (let i = 0; i < settings.octaves; i++) {
      const noiseValue = DomainNoise.simplex3D(x * frequency, y * frequency, z * frequency, seed + i)
   ...
   */
  ridgedMultifractal: (x: number, y: number, z: number, settings: AdvancedNoiseSettings, seed: number): number => {
    let value = 0
    let amplitude = 1
    let frequency = settings.scale
    ...
    for (let i = 0; i < settings.octaves; i++) {
      let noiseValue = DomainNoise.simplex3D(x * frequency, y * frequency, z * frequency, seed + i)
   ...
   */
  billow: (x: number, y: number, z: number, settings: AdvancedNoiseSettings, seed: number): number => {
    let value = 0
    let amplitude = 1
    let frequency = settings.scale
   ...
   */
  domainWarp: (x: number, y: number, z: number, settings: AdvancedNoiseSettings, seed: number): [number, number, number] => {
    const warpStrength = settings.domainWarpStrength ?? 50.0
    const warpX = DomainNoise.simplex3D(x * 0.02, y * 0.02, z * 0.02, seed + 1000) * warpStrength
    const warpY = DomainNoise.simplex3D(x * 0.02, y * 0.02, z * 0.02, seed + 2000) * warpStrength
    const warpZ = DomainNoise.simplex3D(x * 0.02, y * 0.02, z * 0.02, seed + 3000) * warpStrength
  ...
  generateNoise: (x: number, y: number, z: number, settings: AdvancedNoiseSettings, seed: number): number => {
    ...
    switch (settings.fractalType ?? 'fbm') {
      case 'ridged':
        return DomainNoise.ridgedMultifractal(warpedX, warpedY, warpedZ, settings, seed)
      case 'billow':
        return DomainNoise.billow(warpedX, warpedY, warpedZ, settings, seed)
      case 'fbm':
      default:
        return DomainNoise.fbm(warpedX, warpedY, warpedZ, settings, seed)
    }
  },
 ...
 * Pure functional module for terrain height generation
 */
export const TerrainGenerationAlgorithms = {
  /**
   ...
   */
  applyBiomeHeightModifications: (height: number, biome: EnhancedBiomeConfig): number => {
 ...
 * Pure functional module for biome-aware block generation
 */
export const BiomeAwareBlockPlacement = {
  /**
   * Generate blocks with advanced biome-specific placement
   */
  generateAdvancedBlocks: (
    chunkX: number,
    chunkZ: number,
    ...
    for (let y = 0; y < maxHeight; y++) {
      for (let z = 0; z < chunkSize; z++) {
        for (let x = 0; x < chunkSize; x++) {
          ...
          if (BiomeAwareBlockPlacement.shouldGenerateStructure(worldX, worldY, worldZ, structures, seed)) {
            const structureBlock = BiomeAwareBlockPlacement.generateStructureBlock(worldX, worldY, worldZ, structures, seed)
            if (structureBlock) {
          ...
          if (blockType !== 'air') {
            ...
            blocks.push({
              position,
              blockType,
              lightLevel: BiomeAwareBlockPlacement.calculateAdvancedLightLevel(worldY, terrainHeight, blockType),
            })
          }
        }
      }
  ...
  determineAdvancedBlockType: (x: number, y: number, z: number, terrainHeight: number, biome: EnhancedBiomeConfig, features: FeatureConfig, seed: number): BlockType => {
    ...
    if (y <= terrainHeight - rules.subsurfaceDepth) {
      ...
      if (features.generateOres) {
        blockType = BiomeAwareBlockPlacement.generateOres(x, y, z, biome, features, seed) ?? blockType
      }
    ...
    if (y === terrainHeight) {
      ...
      if (BiomeAwareBlockPlacement.shouldGenerateVegetation(x, z, biome, seed)) {
    ...
    if (y > terrainHeight && y <= terrainHeight + 10) {
      const vegetationBlock = BiomeAwareBlockPlacement.generateAboveSurfaceVegetation(x, y, z, terrainHeight, biome, seed)
      if (vegetationBlock) {
   ...
   */
  selectVegetationBlock: (biome: EnhancedBiomeConfig, seed: number): BlockType => {
   ...
   */
  calculateAdvancedLightLevel: (y: number, terrainHeight: number, blockType: BlockType): number => {
 ...
 * Pure functional service factory for terrain generation with Effect-TS
 */
export const createEnhancedTerrainGenerationService = (): ITerrainGenerator => {
  // Helper functions for the service
  const createAdvancedNoiseSettings = (noise: NoiseSettings): AdvancedNoiseSettings => ({
    ...noise,
    fractalType: 'fbm',
  ...
  const createBiomeGenerationRules = (biomeType: string): BiomeGenerationRules => {
    switch (biomeType) {
      case 'mountain':
        return {
          surfaceDepth: 1,
  ...
  const createBlockVariants = (biomeType: string): { readonly [key: string]: BlockType[] } => {
    switch (biomeType) {
      case 'mountain':
        return {
  ...
  const createEnhancedBiome = (biome: BiomeConfig): EnhancedBiomeConfig => {
    ...
    return {
      ...biome,
      generationRules,
      blockVariants: createBlockVariants(biome.type),
    }
  }

  const createDefaultStructures = (): StructureConfig => ({
    caves: {
      enabled: true,
      density: 0.15,
      minSize: 3,
  ...
  const selectBiomeByClimate = (biomeNoise: number, temperature: number, humidity: number): BiomeConfig => {
    // Cold biomes
    if (temperature < 0.3) {
      if (humidity > 0.6) {
  ...
  const generateBiomeMap = (biome: EnhancedBiomeConfig): readonly string[] => {
  ...
  return {
    generateTerrain: (request: EnhancedTerrainGenerationRequest): Effect.Effect<TerrainGenerationResult, never, never> =>
      Effect.gen(function* () {
        ...
        const advancedNoise = request.advancedNoise ?? createAdvancedNoiseSettings(noise)
        const enhancedBiome = request.enhancedBiome ?? createEnhancedBiome(biome)
