    ...
    it.effect('座標変換の可逆性（恒等変換）', () =>
      Effect.gen(function* () {
        yield* Effect.promise(() =>
          fc.assert(
            effectProperty(
              fc.tuple(position3DArbitrary, cameraRotationArbitrary),
              ([position, rotation]) =>
                Effect.gen(function* () {
                  // 変換 → 逆変換で元の値に戻る
    ...
    it.effect('回転行列の直交性（行列式 = ±1）', () =>
      Effect.gen(function* () {
        yield* Effect.promise(() =>
          fc.assert(
          effectProperty(
            fc.tuple(pitchArbitrary, yawArbitrary),
            ([pitch, yaw]) =>
              Effect.gen(function* () {
    ...
    it.effect('距離計算の交換法則', () =>
      Effect.gen(function* () {
        yield* Effect.promise(() =>
          fc.assert(
          effectProperty(
            fc.tuple(position3DArbitrary, position3DArbitrary),
            ([pos1, pos2]) =>
              Effect.gen(function* () {
    ...
    it.effect('内積の交換法則', () =>
      Effect.gen(function* () {
        yield* Effect.promise(() =>
          fc.assert(
          effectProperty(
            fc.tuple(direction3DArbitrary, direction3DArbitrary),
            ([dir1, dir2]) =>
              Effect.gen(function* () {
  ...
  describe('エッジケース・境界値の数学的性質', () => {
    ...
    it.effect('大量データでの数値安定性', () =>
      Effect.gen(function* () {
        yield* Effect.promise(() =>
          fc.assert(
          effectProperty(largeBatchPositionArbitrary.map(positions => positions.slice(0, 100)), (positions) =>
            Effect.gen(function* () {
              // 大量の座標に対する距離計算の安定性
              let allValid = true
