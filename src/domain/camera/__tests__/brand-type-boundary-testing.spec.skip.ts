...
import { Brand, Effect, Either, ParseResult, Schema } from 'effect'
 ...
 */
const FOVSchema = Schema.Number.pipe(Schema.between(30, 120), Schema.brand('FOV'))

const SensitivitySchema = Schema.Number.pipe(Schema.between(0.1, 5.0), Schema.brand('Sensitivity'))

const CameraDistanceSchema = Schema.Number.pipe(Schema.between(1, 50), Schema.brand('CameraDistance'))

const MouseDeltaSchema = Schema.Number.pipe(Schema.between(-100, 100), Schema.brand('MouseDelta'))
...
const PitchSchema = Schema.Number.pipe(Schema.between(-90, 90), Schema.brand('Pitch'))

const YawSchema = Schema.Number.pipe(Schema.between(0, 360), Schema.brand('Yaw'))

const AngleSchema = Schema.Number.pipe(Schema.between(-360, 360), Schema.brand('Angle'))
 ...
 * 距離補間
 */
const interpolateDistance = (
  from: CameraDistance,
  to: CameraDistance,
  t: number
): Effect.Effect<CameraDistance, never> =>
  Effect.sync(() => {
...
describe('Camera Domain - Brand Type Boundary Testing', () => {
  describe('FOV Brand型境界値テスト', () => {
    it.effect('FOV有効範囲内での検証', () =>
      Effect.gen(function* () {
        yield* TestUtils.fcAssert(
          TestUtils.fcProperty(fovArbitrary, (fov) =>
            Effect.gen(function* () {
    ...
    it.effect('FOV Property-based境界値テスト', () =>
      Effect.gen(function* () {
        yield* TestUtils.fcAssert(
          effectProperty(fc.integer({ min: -100, max: 300 }), (value) =>
            Effect.gen(function* () {
              const result = yield* Effect.either(validateFOV(value))
  ...
  describe('Sensitivity Brand型境界値テスト', () => {
    it.effect('Sensitivity有効範囲内での検証', () =>
      Effect.gen(function* () {
        yield* TestUtils.fcAssert(
          TestUtils.fcProperty(sensitivityArbitrary, (sensitivity) =>
            Effect.gen(function* () {
    ...
    it.effect('マウス感度計算の数学的性質', () =>
      Effect.gen(function* () {
        yield* TestUtils.fcAssert(
          effectProperty(fc.tuple(mouseDeltaArbitrary, sensitivityArbitrary), ([delta, sensitivity]) =>
            Effect.gen(function* () {
  ...
  describe('CameraDistance Brand型境界値テスト', () => {
    it.effect('CameraDistance有効範囲内での検証', () =>
      Effect.gen(function* () {
        yield* TestUtils.fcAssert(
          TestUtils.fcProperty(cameraDistanceArbitrary, (distance) =>
            Effect.gen(function* () {
  ...
  describe('角度系Brand型境界値テスト', () => {
    it.effect('Pitch制限の数学的性質', () =>
      Effect.gen(function* () {
        yield* TestUtils.fcAssert(
          effectProperty(fc.float({ min: Math.fround(-180), max: Math.fround(180), noNaN: true }), (angle) =>
            Effect.gen(function* () {
  ...
  describe('DeltaTime Brand型境界値テスト', () => {
    it.effect('DeltaTime有効範囲での検証', () =>
      Effect.gen(function* () {
        yield* TestUtils.fcAssert(
          TestUtils.fcProperty(deltaTimeArbitrary, (deltaTime) =>
            Effect.gen(function* () {
