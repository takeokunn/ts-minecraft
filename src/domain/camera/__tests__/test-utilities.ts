...
import { Brand, Effect, Exit, pipe, Ref, Schema } from 'effect'
 ...
 * ADT タグ アサーション
 */
export const expectADTTag = <T extends { _tag: string }>(adt: T, expectedTag: T['_tag']): Effect.Effect<void, never> =>
  Effect.sync(() => {
 ...
 * パフォーマンス期待値アサーション
 */
export const expectPerformance = <A, E, R>(effect: Effect.Effect<A, E, R>, maxTimeMs: number): Effect.Effect<A, E, R> =>
  pipe(
    measureTime(effect),
 ...
 * テスト用状態管理
 */
export const createTestState = <T>(
  initialValue: T
): Effect.Effect<
  {
