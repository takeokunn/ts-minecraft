...
import type { CameraRotation, MouseDelta, Pitch, Yaw } from '../../value_object/camera_rotation/types'
import type { FOV, Sensitivity } from '../../value_object/camera_settings/types'
...
export const fovArbitrary = fc.integer({ min: 30, max: 120 }).map((n) => Brand.nominal<FOV>()(n))
...
export const boundingBoxArbitrary = fc.tuple(position3DArbitrary, position3DArbitrary).map(([pos1, pos2]) => {
...
export const cameraErrorArbitrary: fc.Arbitrary<CameraError> = fc.oneof(
  ...
  fc
    ...
    .map(({ operation }) => CameraNotInitialized({ operation })),
  ...
  fc
    ...
    .map(({ mode, validModes }) => InvalidMode({ mode, validModes })),
...
export const viewModeArbitrary = fc.oneof(
  ...
  fc
    ...
    .map((settings) => ViewMode.FirstPerson({ settings })),
  ...
  fc
    ...
    .map(({ settings, distance }) => ViewMode.ThirdPerson({ settings, distance })),
  ...
  fc
    ...
    .map((settings) => ViewMode.Spectator({ settings })),
  ...
  fc
    ...
    .map(({ settings, timeline }) => ViewMode.Cinematic({ settings, timeline }))
)
...
export const approximatelyEqual = (a: number, b: number, epsilon = EPSILON): boolean => Math.abs(a - b) < epsilon

export const position3DApproximatelyEqual = (pos1: Position3D, pos2: Position3D, epsilon = EPSILON): boolean =>
 ...
 */
export const assertEffectSuccess = <T, E, R>(effect: Effect.Effect<T, E, R>): Effect.Effect<T, E, R> =>
  pipe(
    effect,
    Effect.tap((result) =>
      Effect.sync(() => {
...
export const assertEffectFailure = <T, E, R>(effect: Effect.Effect<T, E, R>): Effect.Effect<E, never, R> =>
  pipe(
    effect,
    Effect.flip,
    Effect.tap((error) =>
