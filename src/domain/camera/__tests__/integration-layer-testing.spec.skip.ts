...
interface AnimationService {
  readonly startAnimation: (
    from: CameraState,
    to: CameraState,
    duration: number
  ) => Effect.Effect<AnimationId, AnimationError>
...
interface CollisionService {
  ...
  ) => Effect.Effect<Position3D, CollisionError>
...
const CameraServiceLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const render = () => Effect.succeed({ success: true, frameTime: 16.67 } as RenderResult)
              ...
              (operations) =>
                Effect.gen(function* () {
                  ...
                  const concurrentOps = operations.map((op) =>
                    Effect.gen(function* () {
                      // 有効な操作のみ実行
                      if (op.position.x >= -1000 && op.position.x <= 1000 && op.fov >= 30 && op.fov <= 120) {
                        yield* cameraService.updatePosition(op.position)
                        yield* cameraService.updateRotation(op.rotation)
                      }
                    })
  ...
  describe('Error Handling Integration', () => {
    it.effect('Layer間エラー伝播テスト', () =>
      Effect.gen(function* () {
        ...
        // 初期化前の操作は失敗
        const uninitializedResult = yield* Effect.either(cameraService.updatePosition({ x: 0, y: 0, z: 0 }))
        ...
        // 無効な位置での衝突エラー
        const collisionResult = yield* Effect.either(cameraService.updatePosition({ x: 10000, y: 10000, z: 10000 }))
        expect(collisionResult._tag).toBe('Left')

        // 有効な操作は成功
        const validResult = yield* Effect.either(cameraService.updatePosition({ x: 10, y: 10, z: 10 }))
