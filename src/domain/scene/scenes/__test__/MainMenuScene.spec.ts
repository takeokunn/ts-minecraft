...
describe('MainMenuScene', () => {
  ...
  describe('初期化', () => {
  it.effect('シーンデータが正しく設定される', () => Effect.gen(function* () {
  ...
  Effect.gen(function* () {
    ...
    it.effect('二重初期化でエラーになる', () => Effect.gen(function* () {
    const scene = yield* createScene()
    ...
    describe('更新処理', () => {
  it.effect('初期化前のupdateは何もしない', () => Effect.gen(function* () {
    const scene = yield* createScene()
  ...
  Effect.gen(function* () {
    ...
    it.effect('異なるdeltaTimeで動作する', () => Effect.gen(function* () {
    const scene = yield* createScene()
    yield* scene.initialize()
    // 異なるフレームレートでの更新
    ...
    describe('描画処理', () => {
  it.effect('初期化前のrenderは何もしない', () => Effect.gen(function* () {
    const scene = yield* createScene()
    yield* scene.render()
    // エラーなく完了することを確認
})
  ...
  describe('シーンライフサイクル', () => {
  it.effect('onEnterが正常に動作する', () => Effect.gen(function* () {
    const scene = yield* createScene()
    yield* scene.onEnter()
    // エラーなく完了することを確認
})
  ...
  Effect.gen(function* () {
    ...
    it.effect('完全なライフサイクルが動作する', () => Effect.gen(function* () {
    const scene = yield* createScene()
    // 入場
    yield* scene.onEnter()
    yield* scene.initialize()
  ...
  Effect.gen(function* () {
        it.effect('初期化前のcleanupがエラーになる', () => Effect.gen(function* () {
    const scene = yield* createScene()
    ...
    it.effect('初期化後のcleanupが成功する', () => Effect.gen(function* () {
    const scene = yield* createScene()
    yield* scene.initialize()
    yield* scene.cleanup()
    // エラーなく完了することを確認
    ...
    describe('メニュー固有の機能', () => {
  it.effect('ゲームタイトルが正しく設定される', () => Effect.gen(function* () {
    const scene = yield* createScene()
  ...
  Effect.gen(function* () {
    ...
    it.effect('メニューアイテムが設定される', () => Effect.gen(function* () {
    const scene = yield* createScene()
