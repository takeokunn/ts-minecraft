...
export const createDevConsole = (world: World) => Effect.gen(function* () {
  ...
  const createConsoleUI = () => Effect.gen(function* () {
  ...
  const navigateHistory = (direction: number) => Effect.gen(function* () {
    const state = yield* Ref.get(stateRef)
  ...
  const autocomplete = () => Effect.gen(function* () {
    const state = yield* Ref.get(stateRef)
  ...
  const executeCommand = (input: string) => Effect.gen(function* () {
  ...
  const print = (message: string, color: string = '#fff') => Effect.gen(function* () {
    const state = yield* Ref.get(stateRef)
  ...
  const printWelcomeMessage = () => Effect.gen(function* () {
  ...
  // 外部からコマンドを追加する機能
  const addCommand = (command: ConsoleCommand) => Effect.gen(function* () {
    yield* Ref.update(stateRef, state => {
  ...
  // コマンドを削除する機能
  const removeCommand = (name: string) => Effect.gen(function* () {
    yield* Ref.update(stateRef, state => {
      const newCommands = new Map(state.commands)
