...
import { InventoryAPIService } from '../../../application/inventory/api-service'
import { InventoryService } from '../../../domain/inventory/service'
import type { PlayerId } from '../../../domain/inventory/types'
import { Inventory, ItemStack } from '../../../domain/inventory/types'
import { InventoryStateManager } from './state-manager'
...
export const InventoryReactiveSystem = Context.GenericTag<InventoryReactiveSystem>(
  '@minecraft/presentation/inventory/ReactiveSystem'
)
      ...
      Effect.gen(function* () {
        ...
        yield* pipe(
          ...
          Option.match({
            ...
            onSome: (inv) =>
              Effect.gen(function* () {
                const slot = inv.slots[update.slotIndex]
                const item = slot?.itemStack
                  ...
                  Match.when(true, () =>
                    stateManager.updateState(update.playerId, (inventory) => ({
                      ...
                      slots: inventory.slots.map((slot, idx) =>
                        idx === update.slotIndex && slot?.itemStack
                          ? {
                              ...slot,
                              itemStack: {
                                ...slot.itemStack,
                                metadata: {
                                  ...slot.itemStack.metadata,
        ...
        stateManager.events.pipe(
          ...
          Stream.mapEffect(() =>
            stateManager
              ...
              .pipe(
                Effect.map(
                  Option.map((inv) => Option.fromNullable(inv.slots[slotIndex]?.itemStack ?? null))
                )
              )
          ),
            ...
            stateManager
              ...
              .pipe(
                Effect.map(
                  Option.map(
                    (inv) =>
                      inv.hotbar.map((idx) => inv.slots[idx]?.itemStack ?? null) as ReadonlyArray<ItemStack | null>
                  )
                )
              )
          ),
              ...
              onSome: (inv) =>
                Effect.gen(function* () {
                  const foodSlot = inv.hotbar.find((slotIdx) => {
                    const item = inv.slots[slotIdx]?.itemStack
