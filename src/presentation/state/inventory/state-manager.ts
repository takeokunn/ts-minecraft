...
import { ItemManagerService } from '../../../application/inventory/item-manager-service'
import { InventoryError, InventoryService } from '../../../domain/inventory/service'
import type { PlayerId } from '../../../domain/inventory/types'
import type { InventoryAggregate as Inventory } from '../../../domain/inventory/aggregate/inventory/types'
...
export const InventoryStateManager = Context.GenericTag<InventoryStateManager>(
  '@minecraft/presentation/inventory/StateManager'
)
          ...
          yield* pipe(
            ...
            Match.when('success', () =>
              Effect.gen(function* () {
                ...
                const slotIndex = inventory.slots.findIndex((slot) => slot?.itemStack?.itemId === item.itemId)
    ...
    manager.getAllStates().pipe(
      Effect.map((states) => {
        ...
        states.forEach((inventory, playerId) => {
          const hasItem = inventory.slots.some((slot) => slot?.itemStack?.itemId === itemId)
