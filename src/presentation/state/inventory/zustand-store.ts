...
import type { PlayerId } from '../../../domain/inventory/types'
import type { InventoryAggregate as Inventory, InventorySlot } from '../../../domain/inventory/aggregate/inventory/types'
import type { ItemStackEntity as ItemStack } from '../../../domain/inventory/aggregate/item_stack/types'
    ...
    persist(
      (set, get) => ({
        ...
        updateSlot: (slotIndex: number, itemStack: ItemStack | null) => {
          ...
          newSlots[slotIndex] = itemStack ? { itemStack, metadata: {} } : null
        ...
        removeItem: (slotIndex: number, amount?: number) => {
          ...
          const currentSlot = state.currentInventory.slots[slotIndex]
          if (!currentSlot?.itemStack) return

          const currentItem = currentSlot.itemStack
        ...
        moveItem: (fromSlot: number, toSlot: number) => {
          ...
          const fromSlotValue = state.currentInventory.slots[fromSlot]
          const toSlotValue = state.currentInventory.slots[toSlot]

          if (!fromSlotValue?.itemStack) return

          const fromItem = fromSlotValue.itemStack
          const toItem = toSlotValue?.itemStack ?? null
        ...
        swapItems: (slot1: number, slot2: number) => {
          ...
          const item1 = state.currentInventory.slots[slot1]?.itemStack ?? null
          const item2 = state.currentInventory.slots[slot2]?.itemStack ?? null
        ...
        getSelectedItem: () => {
          ...
          if (selectedHotbarSlot === undefined) {
            return null
          }
          const slot = state.currentInventory.slots[selectedHotbarSlot]
        ...
        findItemSlots: (itemId: string) => {
          ...
          return state.currentInventory.slots
            .map((slot, index) => (slot?.itemStack?.itemId === itemId ? index : -1))
        ...
        hasSpaceForItem: (itemStack: ItemStack) => {
          ...
          for (const slotIndex of compatibleSlots) {
            ...
            if (
              existingItem?.itemStack &&
              JSON.stringify(existingItem.itemStack.metadata) === JSON.stringify(itemStack.metadata) &&
              existingItem.itemStack.count < 64
            ) {
...
export const inventorySelectors = {
  hotbar: (state: InventoryZustandState) =>
    state.currentInventory
      ? state.currentInventory.hotbar.map((slotIndex) => state.currentInventory!.slots[slotIndex]?.itemStack ?? null)
      : [],

  armor: (state: InventoryZustandState) =>
    state.currentInventory
      ? {
          helmet: state.currentInventory.armor.helmet?.itemStack ?? null,
          chestplate: state.currentInventory.armor.chestplate?.itemStack ?? null,
          leggings: state.currentInventory.armor.leggings?.itemStack ?? null,
          boots: state.currentInventory.armor.boots?.itemStack ?? null,
        }
      : {
          helmet: null,
          chestplate: null,
  ...
  offhand: (state: InventoryZustandState) => state.currentInventory?.offhand?.itemStack ?? null,
