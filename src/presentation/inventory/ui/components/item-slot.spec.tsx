...
import { FastCheck, describe, expect, it } from '@effect/vitest'
...
describe('presentation/inventory/ui/ItemSlot', () => {
  ...
  it('handles quick move when handler is provided', () =>
    Effect.sync(() =>
      FastCheck.assert(
        FastCheck.property(FastCheck.boolean(), (shouldProvide) => {
          const slotIndex = Effect.runSync(Schema.decode(SlotIndexSchema)(0))
          const slotType = Effect.runSync(Schema.decode(SlotTypeSchema)('normal'))
          const position = Effect.runSync(
            slotGridPosition({ index: 0, columns: 9, spacing: 2, slotSize: 32 })
          )
          const slot = makeInventorySlot({
            index: slotIndex,
            section: 'main',
          ...
          const quickMoved: Array<number> = []

          const quickMoveHandler = pipe(
            shouldProvide,
            Match.value,
            Match.when(true, () => (current: typeof slot) => quickMoved.push(current.index)),
          ...
          try {
            ...
            pipe(
              ...
              Match.value,
              Match.when(true, () => {
                expect(quickMoved).toHaveLength(1)
                expect(quickMoved[0]).toBe(slotIndex)
              }),
              Match.orElse(() => {
                expect(quickMoved).toHaveLength(0)
              })
            )
          } finally {
            unmount()
