...
import { Chunk, Effect, Fiber, HashMap, Match, Option, pipe, Schema, Stream } from 'effect'
  ...
  it.effect('emits updates via streamByPlayer', () =>
    sampleView.pipe(
      Effect.flatMap(({ playerId, view }) =>
        Effect.gen(function* () {
          const store = yield* InventoryStateStoreTag
          const fiber = yield* store.streamByPlayer(playerId).pipe(Stream.take(1), Stream.runCollect, Effect.fork)
          yield* store.set(playerId, view)
