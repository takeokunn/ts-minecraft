...
import { InventoryViewModelTag } from '@mc/bc-inventory/interface/view-models/inventory-view-model'
...
describe('presentation/inventory/state/reactive-system', () => {
  const makeLayer = Effect.gen(function* () {
    ...
    const viewModelLayer = Layer.succeed(InventoryViewModelTag, {
      handleEvent: (): Effect.Effect<void, never> => Effect.void,
      ...
      handler: (): InventoryEventHandler => () => Effect.void,
    })
    ...
    makeLayer.pipe(
      Effect.flatMap(({ viewModelLayer, playerId }) =>
        Effect.gen(function* () {
          ...
          Effect.provide(InventoryReactiveSystemLive),
          Effect.provide(viewModelLayer),
