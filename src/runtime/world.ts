...
export const WorldLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const getComponent = <T extends ComponentName>(entityId: EntityId, componentName: T) => Effect.map(Ref.get(worldStateRef), (world) => getComponentPure(world, entityId, componentName))

    const updateComponent = <T extends ComponentName>(entityId: EntityId, componentName: T, componentData: Components[T]) => update((world) => updateComponentPure(world, entityId, componentName, componentData))
