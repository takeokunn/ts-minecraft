...
describe('application/lifecycle', () => {
  it.effect('allows declared transitions only', () =>
    Effect.forEach(lifecycleStates, (state) =>
      Effect.forEach(permittedTargets(state), (target) =>
        guardLifecycleTransition(state, target)
      )
    )
  )
  ...
  it.effect('rejects disallowed transitions', () =>
    Effect.forEach(lifecycleStates, (state) =>
      Effect.forEach(
        lifecycleStates.filter(
          (candidate) => !permittedTargets(state).includes(candidate)
        ),
        (target) =>
