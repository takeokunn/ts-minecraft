...
describe('application/config', () => {
  it.effect('merges partial patch deeply', () =>
    Effect.gen(function* () {
      ...
      expect(patched.rendering.targetFps).toEqual(
        Schema.decodeSync(TargetFramesPerSecond)(120)
      )
  ...
  it.effect('rejects invalid configuration with descriptive error', () =>
    Effect.gen(function* () {
      ...
      const decoded = Schema.decodeSync(GameApplicationConfig)(
        DEFAULT_GAME_APPLICATION_CONFIG
      )
      expect(decoded.rendering.targetFps).toEqual(
        DEFAULT_GAME_APPLICATION_CONFIG.rendering.targetFps
      )
    })
  )
