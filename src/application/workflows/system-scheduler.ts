...
export interface SystemScheduler {
  readonly registerSystem: (config: SystemConfig, system: SystemFunction) => Effect.Effect<void, ValidationError | SystemInitializationError>
  readonly unregisterSystem: (systemId: string) => Effect.Effect<void, SystemInitializationError>
  readonly executeFrame: () => Effect.Effect<void, Schema.Schema.Type<typeof SchedulerError>, World | ClockPort>
  readonly getSystemMetrics: (systemId: string) => Effect.Effect<Option.Option<SystemMetrics>, never, never>
 ...
 */
export const SystemScheduler = Context.GenericTag<SystemScheduler>('SystemScheduler')
 ...
 */
export const SystemSchedulerLive = (config: SchedulerConfig = defaultSchedulerConfig) =>
  Layer.effect(
    SystemScheduler,
    Effect.gen(function* () {
 ...
 */
export const createSystemScheduler = (config: Partial<SchedulerConfig> = {}) => SystemSchedulerLive({ ...defaultSchedulerConfig, ...config })
