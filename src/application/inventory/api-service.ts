...
import {
  ...
  type ItemStackEntity as ItemStack,
  type InventoryAggregate as Inventory,
  type PlayerId,
  parseItemId
} from '../../domain/inventory/index.js'
import { LocalStorageInventoryService } from '../../infrastructure/inventory/persistence/local-storage'
import { InventoryStorageService } from '../../infrastructure/inventory/storage-service'
import { InventoryIntegrationService, InventoryIntegrationServiceLive } from './integration-layer'
import { ItemManagerService, ItemManagerServiceLive } from './item-manager-service'
...
const createInventoryStats = (inventory: Inventory) => {
  ...
  const occupied = slots.filter((slot): slot is { readonly itemStack: ItemStack } => slot?.itemStack !== undefined)
  const empty = slots.length - occupied.length
  const uniqueItems = new Set(occupied.map((slot) => slot.itemStack.itemId)).size

  return {
    totalSlots: slots.length,
    usedSlots: occupied.length,
    emptySlots: empty,
    uniqueItems,
  }
}
...
export const InventoryAPIServiceLive = Layer.effect(
  ...
  Effect.gen(function* () {
    // すべての依存関係を先に取得
    const inventoryService = yield* InventoryManagerService
    ...
    return InventoryAPIService.of({
      ...
      deletePlayerInventory: (playerId: PlayerId) =>
        pipe(
          ...
          Effect.map(() => createSuccessResponse<void>(undefined)),
          Effect.catchAll((error) =>
      ...
      addItem: (playerId: PlayerId, itemId: string, count = 1) =>
        pipe(
          Effect.gen(function* () {
            const parsedItemId = yield* parseItemId(itemId)
            const item = yield* itemManager.createItem(parsedItemId, count)
          ...
          Effect.catchAll((error) => Effect.succeed(createErrorResponse<ItemOperationResult>(`Failed to add item: ${String(error)}`)))
        ),
        ...
        pipe(
          ...
          Effect.map((inventory) => {
            const matchingItems = inventory.slots
              ...
              .filter(({ slot }) => slot?.itemStack?.itemId.toLowerCase().includes(query.toLowerCase()))
              .map(({ slot, index }) => ({ slotIndex: index, item: slot!.itemStack! }))
      ...
      getItemsByCategory: (playerId: PlayerId, category: string) =>
        pipe(
          Effect.gen(function* () {
            ...
            const matchingItems = inventory.slots
              .map((slot, index) => ({ slot, index }))
              .filter(({ slot }) => slot?.itemStack && categoryItemIds.has(slot.itemStack.itemId as any))
              .map(({ slot, index }) => ({ slotIndex: index, item: slot!.itemStack! }))
              ...
              (slot, index) =>
                pipe(
                  ...
                  Option.match({
                    ...
                    onSome: (stack) =>
                      pipe(
                        inventoryService.setSlotItem(targetPlayerId, index, stack),
                        Effect.map(() => true),
      ...
      mergeInventories: (playerId1: PlayerId, playerId2: PlayerId, targetPlayerId: PlayerId) =>
        pipe(
          Effect.gen(function* () {
            ...
            const allItems: ReadonlyArray<ItemStack> = [
              ...inventory1.slots.flatMap((slot) => (slot?.itemStack ? [slot.itemStack] : [])),
              ...inventory2.slots.flatMap((slot) => (slot?.itemStack ? [slot.itemStack] : [])),
            ]
