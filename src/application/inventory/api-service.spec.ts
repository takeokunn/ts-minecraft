...
import { InventoryService, InventoryServiceError } from '../../domain/inventory/inventory-service'
import {
  ItemId,
  ItemStack,
  PlayerId,
...
const sortSlots = (slots: Slots): Slots => {
  ...
  const nullSlots = Match.value(difference).pipe(
    ...
    Match.orElse(() => ReadonlyArray.replicate(null satisfies Slots[number], difference))
  )
...
const withApi = <A, E>(program: Effect.Effect<A, E, typeof InventoryAPIService>): Effect.Effect<A, E> =>
  Effect.scoped(
    Layer.build(ApiLayer).pipe(
      Effect.flatMap((context) =>
        program.pipe(Effect.provideService(InventoryAPIService, Context.get(context, InventoryAPIService)))
      )
    )
  )

describe('application/inventory/api-service', () => {
  const itemIdArbitrary = FastCheck.stringMatching(/^[a-z]{1,12}$/).map((value) => ItemId(`minecraft:${value}`))
  ...
  it.effect('creates snapshot without sorting', () =>
    withApi(
      Effect.gen(function* () {
        ...
        expect(snapshot.uniqueItemIds).toEqual(['minecraft:apple', 'minecraft:diamond', 'minecraft:stone'])
        expect(HashSet.size(HashSet.fromIterable(snapshot.uniqueItemIds))).toBe(snapshot.uniqueItemIds.length)
      })
    )
  )
