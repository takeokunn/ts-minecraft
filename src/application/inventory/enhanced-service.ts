...
import { SlotManager } from '../../domain/inventory/helpers/slot-manager'
import { StackProcessor } from '../../domain/inventory/helpers/stack-processor'
import { InventoryError, InventoryService } from '../../domain/inventory/service'
import { createEmptyInventory, Inventory, InventoryState, ItemStack, type PlayerId } from '../../domain/inventory/types'
    ...
    return InventoryService.of({
      ...
      dropAllItems: (playerId: PlayerId) =>
        Effect.gen(function* () {
          ...
          // Collect armor items
          Object.values(inventory.armor).forEach((armorSlot) => {
            if (armorSlot?.itemStack) {
              droppedItems.push(armorSlot.itemStack)
            }
          })
          ...
          pipe(
            ...
            Option.match({
              onNone: () => {},
              onSome: (offhand) => {
                if (offhand?.itemStack) {
                  droppedItems.push(offhand.itemStack)
                }
              },
            })
          )
