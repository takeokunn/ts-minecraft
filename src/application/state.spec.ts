...
describe('application/state', () => {
  ...
  it('tickState increments frame count while running', () => {
    const initial = synchronizeLifecycle(createInitialState(DEFAULT_GAME_APPLICATION_CONFIG), 'Running')
    const updated = tickState(initial, toMilliseconds(16))

    expect(updated.systems.gameLoop.frameCount).toEqual(initial.systems.gameLoop.frameCount + 1)
  ...
  it('tickState is idempotent when lifecycle is paused', () => {
    const paused = synchronizeLifecycle(createInitialState(DEFAULT_GAME_APPLICATION_CONFIG), 'Paused')
  ...
  it('applyConfig updates target FPS while preserving counters', () => {
    const running = synchronizeLifecycle(createInitialState(DEFAULT_GAME_APPLICATION_CONFIG), 'Running')
    ...
    expect(applied.systems.gameLoop.frameCount).toEqual(running.systems.gameLoop.frameCount)
  })

  it('computeHealth reflects lifecycle derived system status', () => {
    const base = synchronizeLifecycle(createInitialState(DEFAULT_GAME_APPLICATION_CONFIG), 'Running')
  ...
  it('tickState accumulates frames monotonically (property)', () => {
    FastCheck.assert(
      FastCheck.property(FastCheck.integer({ min: 1, max: 120 }), (steps) => {
        ...
        const result = ReadonlyArray.reduce(deltas, running, (state, delta) => tickState(state, delta))
        expect(result.systems.gameLoop.frameCount).toEqual(running.systems.gameLoop.frameCount + steps)
      })
    )
  })
})
