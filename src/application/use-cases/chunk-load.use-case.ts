    ...
    execute: (command) =>
      Effect.gen(function* (_) {
        ...
        const isLoaded = yield* _(
          worldService.isChunkLoaded(command.chunkX, command.chunkZ)
        )
        ...
        const isInQueue = yield* _(
          worldService.isChunkInLoadingQueue(command.chunkX, command.chunkZ)
        )
        ...
        for (let x = centerChunkX - radius; x <= centerChunkX + radius; x++) {
          for (let z = centerChunkZ - radius; z <= centerChunkZ + radius; z++) {
            const distance = Math.sqrt(
              Math.pow(x - centerChunkX, 2) + Math.pow(z - centerChunkZ, 2)
            )
...
const processChunkLoading = (
  ...
  Effect.gen(function* (_) {
    ...
    const terrainData = yield* _(
      worldService.generateChunkTerrain(command.chunkX, command.chunkZ)
    )

    // Generate structures (trees, ores, etc.)
    const structureData = yield* _(
      worldService.generateChunkStructures(command.chunkX, command.chunkZ, terrainData)
    )
    ...
    yield* _(
      worldService.markChunkAsLoaded(command.chunkX, command.chunkZ)
    )

    yield* _(
      Effect.log(`Chunk ${command.chunkX}, ${command.chunkZ} loaded successfully`)
    )
  })
