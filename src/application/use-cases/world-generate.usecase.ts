...
export interface WorldGenerateUseCaseService {
  readonly execute: (command: WorldGenerateCommand) => Effect.Effect<void, ValidationError | TerrainGenerationError | MeshGenerationError | WorldStateError>
  readonly generateBiome: (chunkX: number, chunkZ: number, biomeType: string) => Effect.Effect<void, ValidationError | TerrainGenerationError | WorldStateError>
  readonly generateStructure: (position: { x: number; y: number; z: number }, structureType: string) => Effect.Effect<void, ValidationError | InvalidPositionError | WorldStateError>
}
  ...
  Effect.gen(function* () {
    ...
    const generateStructure = (position: { x: number; y: number; z: number }, structureType: string) =>
      Effect.gen(function* (_) {
        ...
        if (!canPlace) {
          return yield* _(Effect.fail(new InvalidPositionError({ 
            message: `Cannot place structure ${structureType} at position`,
            position 
          })))
        }
...
const generateBaseTerrain = (command: WorldGenerateCommand, terrainGenerator: TerrainGeneratorPort): Effect.Effect<void, TerrainGenerationError> =>
  Effect.gen(function* (_) {
...
const generateBiomes = (command: WorldGenerateCommand, terrainGenerator: TerrainGeneratorPort): Effect.Effect<void, TerrainGenerationError> =>
  Effect.gen(function* (_) {
