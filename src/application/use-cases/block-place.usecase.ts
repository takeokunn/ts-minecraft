...
export interface BlockPlaceUseCaseService {
  readonly execute: (command: BlockInteractionCommand) => Effect.Effect<void, ValidationError | InvalidPositionError | EntityNotFoundError | WorldStateError | RaycastError>
}
    ...
    Effect.gen(function* () {
      ...
      if (!canPlace) {
        return yield* Effect.fail(new InvalidPositionError({ 
          message: 'Cannot place block at this location',
...
const validateBlockPlacement = (command: BlockInteractionCommand, worldService: WorldDomainService, entityService: EntityDomainService): Effect.Effect<boolean, EntityNotFoundError | WorldStateError> =>
  Effect.gen(function* () {
