...
import { PhysicsService, type RigidBodyId, type ConstraintId, type PhysicsMaterialId } from '../../domain/services/physics.service'
import { RenderService } from '../../infrastructure/gpu/webgpu/renderer.layer'
import { InputService } from '@/application/services/input.service'
...
export const MockEntityService = Layer.succeed(
  ...
  {
    createEntity: () => Effect.succeed(1 as EntityId),
    ...
    deserializeEntity: () => Effect.succeed(1 as EntityId),
...
export const MockPhysicsService = Layer.succeed(
  ...
  {
    createRigidBody: () => Effect.succeed('body1' as RigidBodyId),
    ...
    createConstraint: () => Effect.succeed('constraint1' as ConstraintId),
    destroyConstraint: () => Effect.succeed(undefined),
    updateConstraint: () => Effect.succeed(undefined),
    createMaterial: () => Effect.succeed('material1' as PhysicsMaterialId),
...
export const createServiceLayer = (config: ServiceLayerConfig) => {
  ...
  return serviceLayers.reduce((acc, layer) => Layer.mergeAll(acc, layer), Layer.empty)
}
