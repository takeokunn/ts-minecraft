...
import { ValidationError, QueryExecutionError } from '@domain/errors'
...
interface QueryHandlersService {
  readonly getPlayerState: (query: PlayerQuery) => Effect.Effect<PlayerQueryResult, ValidationError | QueryExecutionError, never>
  readonly getChunkState: (query: ChunkQuery) => Effect.Effect<ChunkQueryResult, ValidationError | QueryExecutionError, never>
  readonly getWorldState: (query: WorldStateQuery) => Effect.Effect<WorldStateQueryResult, ValidationError | QueryExecutionError, never>
  readonly findEntities: (query: EntityQuery) => Effect.Effect<PlayerQueryResult[], ValidationError | QueryExecutionError, never>
  readonly getLoadedChunks: () => Effect.Effect<ChunkQueryResult[], QueryExecutionError, never>
  readonly getNearbyEntities: (position: { x: number; y: number; z: number }, radius: number) => Effect.Effect<PlayerQueryResult[], ValidationError | QueryExecutionError, never>
}
...
export const QueryHandlersLive: Layer.Layer<QueryHandlers, never, typeof WorldDomainService | typeof EntityDomainService | typeof PhysicsDomainService> = Layer.effect(
  QueryHandlers,
  Effect.gen(function* () {
    // Dependencies are available through context injection
    yield* Effect.succeed(undefined)
    return {
      getPlayerState: (query: PlayerQuery): Effect.Effect<PlayerQueryResult, ValidationError | QueryExecutionError, never> =>
        Effect.gen(function* () {
      ...
      getChunkState: (query: ChunkQuery): Effect.Effect<ChunkQueryResult, ValidationError | QueryExecutionError, never> =>
        Effect.gen(function* () {
          // Validate query
      ...
      getWorldState: (query: WorldStateQuery): Effect.Effect<WorldStateQueryResult, ValidationError | QueryExecutionError, never> =>
        Effect.gen(function* () {
      ...
      findEntities: (query: EntityQuery): Effect.Effect<PlayerQueryResult[], ValidationError | QueryExecutionError, never> =>
        Effect.gen(function* () {
          // Validate query
      ...
      getLoadedChunks: (): Effect.Effect<ChunkQueryResult[], QueryExecutionError, never> =>
        Effect.gen(function* () {
      ...
      getNearbyEntities: (_position: { x: number; y: number; z: number }, _radius: number): Effect.Effect<PlayerQueryResult[], ValidationError | QueryExecutionError, never> =>
        Effect.gen(function* () {
...
const validatePlayerQuery = (query: PlayerQuery): Effect.Effect<void, ValidationError, never> =>
  Effect.gen(function* () {
    if (!query.entityId) {
...
const validateChunkQuery = (query: ChunkQuery): Effect.Effect<void, ValidationError, never> =>
  Effect.gen(function* () {
    if (typeof query.chunkX !== 'number' || typeof query.chunkZ !== 'number') {
      return yield* Effect.fail(new ValidationError({
        field: 'chunkCoordinates',
        message: 'Valid chunk coordinates are required'
      }))
    }
  })

const validateWorldStateQuery = (_query: WorldStateQuery): Effect.Effect<void, ValidationError, never> =>
  Effect.gen(function* () {
    // World state query is always valid as all fields are optional
    yield* Effect.succeed(undefined)
  })

const validateEntityQuery = (query: EntityQuery): Effect.Effect<void, ValidationError, never> =>
  Effect.gen(function* () {
    if (query.position && typeof query.position.radius !== 'number') {
      return yield* Effect.fail(new ValidationError({
        field: 'position.radius',
        message: 'Radius is required when position is specified'
      }))
    }
  })
