...
import { Context, Data, Effect, Layer, Match, Option, Ref, pipe } from 'effect'
            ...
            Effect.gen(function* () {
              ...
              yield* system
                ...
                .pipe(
                  ...
                  Effect.matchEffect({
                    onFailure: (error) =>
                      recordFailure(system.name, error).pipe(Effect.andThen(Effect.fail(error))),
                    onSuccess: () =>
                      Effect.sync(() => Date.now()).pipe(
                        Effect.flatMap((endTime) =>
                          recordSuccess(system.name, endTime - startTime, endTime)
                        )
                      ),
                  })
                )
