...
describe('Entity ECS Architecture', () => {
  ...
  describe('ComponentStorage - Structure of Arrays', () => {
    ...
    it.effect('should clear all data', () =>
      Effect.gen(function* () {
        const storage = createComponentStorage<TestComponent>()
        const entities = Array.from({ length: 5 }, (_, i) => createEntityId(i))

        for (const [index, entity] of entities.entries()) {
          yield* storage.insert(entity, { x: index, y: index * 2, z: index * 3 })
        }

        const beforeClear = yield* storage.getAll()
        expect(beforeClear).toHaveLength(entities.length)

        yield* storage.clear()

        const afterClear = yield* storage.getAll()
        expect(afterClear).toHaveLength(0)

        for (const entity of entities) {
          const hasComponent = yield* storage.has(entity)
          expect(hasComponent).toBe(false)

          const component = yield* storage.get(entity)
          expect(Option.isNone(component)).toBe(true)
        }

        const raw = yield* storage.getRawData()
        expect(raw.data.length).toBe(0)
        expect(raw.entities.length).toBe(0)
      }).pipe(Effect.provide(TestContext.TestContext))
    )
  })
