...
export interface CoordinateTransforms {
  // ブロック ⟷ チャンク変換
  readonly blockToChunk: (coord: BlockCoordinate) => import('./world-types').ChunkCoordinate
  readonly chunkToBlock: (coord: import('./world-types').ChunkCoordinate) => BlockCoordinate
  ...
  // チャンク ⟷ リージョン変換
  readonly chunkToRegion: (coord: import('./world-types').ChunkCoordinate) => RegionCoordinate
  readonly regionToChunk: (coord: RegionCoordinate) => import('./world-types').ChunkCoordinate

  // ローカル ⟷ ワールド変換
  readonly localToWorld: (local: ChunkLocalPosition, chunkPos: import('./world-types').ChunkPosition) => BlockPosition
  readonly worldToLocal: (world: BlockPosition) => { local: ChunkLocalPosition; chunkPos: import('./world-types').ChunkPosition }
