...
export interface StatisticalAnalyzerService {
  ...
  readonly calculatePercentile: (values: ReadonlyArray<number>, percentile: number) => Effect.Effect<number, GenerationError>
}
...
export const makeStatisticalAnalyzerService = (): StatisticalAnalyzerService => ({
  calculateMean: (values) => {
    if (values.length === 0) return Effect.fail({ type: 'StatisticalError', message: 'Empty array' } as GenerationError)
