...
import type {
  ClimateData,
} from '../biome_classification/climate-calculator.js'
import type {
  BiomeMappingResult,
} from '../biome_classification/biome-mapper.js'
import type {
  WorldSeed,
...
export const ConsistencyCheckResultSchema = Schema.Struct({
  ...
  // 修正提案
  corrections: Schema.Array(Schema.Struct({
    type: Schema.String,
...
export const makeConsistencyCheckerService = (): ConsistencyCheckerService => ({
  ...
  checkBiomeValidity: (biomeMapping, climateData) =>
      Effect.gen(function* () {
        ...
        // バイオーム-気候整合性チェック
        const isValidCombination = validateBiomeClimateMatch(
