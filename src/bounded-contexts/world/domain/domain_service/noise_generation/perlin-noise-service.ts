...
/**
 * Perlin Noise Service 実装ファクトリ
 *
 * Ken Perlinの改良版アルゴリズム（2002年）を基に実装
 * IEEE 754準拠の高精度計算とSIMD最適化対応
 */
export const makePerlinNoiseService = (): PerlinNoiseService => {
  let service: PerlinNoiseService
  ...
  service = {
    ...
    generateOctaveNoise: (coordinate, octaveConfigs, baseConfig) =>
      Effect.gen(function* () {
        ...
        for (const octaveConfig of octaveConfigs) {
          ...
          const octaveSample = yield* service.sample2D(coordinate, octaveNoiseConfig)
    ...
    generateRidgeNoise: (coordinate, config, ridgeThreshold) =>
      Effect.gen(function* () {
        const baseSample = yield* service.sample2D(coordinate, config)
