...
import type {
  OctaveConfig,
} from '../../value_object/noise_configuration/octave-config.js'
import type {
  WorldSeed,
...
import {
  ...
  type NoiseSample,
  type NoiseField,
} from './perlin-noise-service.js'
import {
  SimplexNoiseService,
  makeSimplexNoiseService,
} from './simplex-noise-service.js'
 ...
 * GPU並列処理とSIMD最適化に対応
 */
export const makeFractalNoiseService = (): FractalNoiseService => {
  const perlinNoise = makePerlinNoiseService()
  const simplexNoise = makeSimplexNoiseService()

  const service: FractalNoiseService = {
      sample2D: (coordinate, config) =>
        Effect.gen(function* () {
          switch (config.type) {
            case 'brownian_motion':
              return yield* service.generateBrownianMotion(coordinate, config)
            case 'turbulence':
              return yield* service.generateTurbulence(coordinate, config)
            case 'ridged_multifractal':
              return yield* service.generateRidgedMultifractal(coordinate, config)
            case 'warped':
              return yield* service.generateDomainWarped(coordinate, config, config)
            default:
              return yield* service.generateBrownianMotion(coordinate, config)
          }
        }),
      ...
      sample3D: (coordinate, config) =>
        Effect.gen(function* () {
          ...
          const baseSample = yield* service.sample2D(coord2D, config)
      ...
      generateDomainWarped: (coordinate, config, warpConfig) =>
        Effect.gen(function* () {
          ...
          // ワープベクトルの計算
          const warpX = yield* service.generateBrownianMotion(
            { x: coordinate.x + 1000, z: coordinate.z },
            warpConfig
          )
          const warpY = yield* service.generateBrownianMotion(
          ...
          // ワープされた座標でノイズをサンプリング
          const warpedSample = yield* service.generateBrownianMotion(warpedCoordinate, config)
