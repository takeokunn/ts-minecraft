...
import type * as BiomeSystem from "../../aggregate/biome_system/biome-system.js"
...
const validateBiomeSystem = (
  ...
  Effect.gen(function* () {
    ...
    if (level === 'exhaustive') {
      // 詳細パフォーマンス分析
      const processingTime = await measureProcessingTime(system)
...
const measureProcessingTime = async (system: BiomeSystem.BiomeSystem): Promise<number> => {
  // 処理時間測定（ミリ秒）
