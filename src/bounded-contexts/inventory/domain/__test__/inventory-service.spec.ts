...
import { ItemRegistry } from '@mc/bc-inventory/domain/item-registry'
import { InventoryServiceLive } from '@mc/bc-inventory/application/services/inventory-service.live'
import { ItemRegistryLive } from '@mc/bc-inventory/application/services/item-registry.live'


const run = <A>(body: (service: InventoryService) => Effect.Effect<A>) =>
  Effect.scoped(
    Effect.gen(function* () {
      const layer = Layer.provideMerge(ItemRegistryLive)(InventoryServiceLive)
...
describe('InventoryServiceLive', () => {
  ...
  it.effect('addItem preserves quantity accounting across arbitrary counts', () => {
    const slotCountsArb = fc.array(fc.integer({ min: 0, max: 64 }), {
      minLength: 36,
      maxLength: 36,
    })
    const addCountArb = fc.integer({ min: 0, max: 64 * 36 })

    const property = fc.asyncProperty(slotCountsArb, addCountArb, async (slotCounts, addCount) => {
        ...
        run((service) =>
          Effect.gen(function* () {
            ...
            for (let index = 0; index < slotCounts.length; index += 1) {
              ...
              if (count > 0) {
                yield* service.setSlotItem(testPlayer, index, makeStack(dirt, count))
              }
            }

            const initialTotal = slotCounts.reduce((sum, value) => sum + value, 0)
            const result = yield* service.addItem(testPlayer, makeStack(dirt, addCount))
            const inventory = yield* service.getInventory(testPlayer)
            const finalTotal = inventory.slots.reduce((sum, slot) => {
              if (slot && slot.itemId === dirt) {
                return sum + slot.count
              }
              return sum
            }, 0)

            expect(result.addedItems + result.remainingItems).toBe(addCount)
            expect(finalTotal).toBe(initialTotal + result.addedItems)
            expect(new Set(result.affectedSlots).size).toBe(result.affectedSlots.length)
            result.affectedSlots.forEach((slotIndex) => {
              expect(slotIndex).toBeGreaterThanOrEqual(0)
              expect(slotIndex).toBeLessThan(36)
              const slot = inventory.slots[slotIndex]
              expect(slot).not.toBeNull()
              expect(slot?.itemId).toBe(dirt)
            })
          })
        )
      )
    ...
    return Effect.promise(() => fc.assert(property, { numRuns: 25 }))
  })
})
