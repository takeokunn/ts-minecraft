...
const EMISSIVE_LEVEL_OVERRIDES = {
    LAVA: 15,
    REDSTONE_BLOCK: 15,
    REDSTONE_ORE: 9,
...
export const isTransparent = (blockType) => TRANSPARENCY_TABLE[blockType];
export const emissiveLightLevel = (blockType) => EMISSIVE_TABLE[blockType];
export const isTransparentIndex = (blockIdx) => (TRANSPARENCY_BY_INDEX[blockIdx] ?? 0) === 1;
export const emissiveLevelByIndex = (blockIdx) => EMISSIVE_BY_INDEX[blockIdx] ?? 0;
export const createLightBuffer = () => new Uint8Array(LIGHT_BYTE_LENGTH);
const voxelIndex = (lx, y, lz) => blockIndexUnsafe(lx, y, lz);
// Caller must ensure 0 ≤ lx < CHUNK_SIZE, 0 ≤ y < CHUNK_HEIGHT, 0 ≤ lz < CHUNK_SIZE.
export const getLightAt = (grid, lx, y, lz) => {
...
export const setLightAt = (grid, lx, y, lz, value) => {
...
export const computeBlockLight = (blocks, lightGrid) => {
    ...
    while (head < queue.length) {
        ...
        for (let i = 0; i < NEIGHBOR_OFFSETS.length; i++) {
            ...
            /* c8 ignore next */
            if ((TRANSPARENCY_BY_INDEX[nBlock] ?? 0) === 0)
                continue;
...
export const computeSkyLight = (blocks, lightGrid) => {
    lightGrid.fill(0);
    const queue = [];
    ...
    for (let x = 0; x < CHUNK_SIZE; x++) {
        for (let z = 0; z < CHUNK_SIZE; z++) {
            for (let y = CHUNK_HEIGHT - 1; y >= 0; y--) {
                ...
                const blockIdx = blocks[vi] ?? 0;
                /* c8 ignore next */
                if ((TRANSPARENCY_BY_INDEX[blockIdx] ?? 0) === 0)
                    break;
