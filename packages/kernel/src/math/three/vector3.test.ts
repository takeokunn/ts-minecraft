...
describe('Vector3Schema', () => {
  ...
  it('should fail on string x', () => {
    expect(() => Schema.decodeUnknownSync(Vector3Schema)({ x: 'not a number', y: 0, z: 0 })).toThrow()
  })
...
describe('add', () => {
  ...
  it('should add zero vector without changing original', () => {
...
describe('subtract', () => {
  it('should subtract two vectors', () => {
    expect(subtract({ x: 5, y: 7, z: 9 }, { x: 4, y: 5, z: 6 })).toEqual({ x: 1, y: 2, z: 3 })
  })
...
describe('cross', () => {
  ...
  it('right x up = backward (z+)', () => {
    const result = cross(right, up)
    expect(result).toEqual({ x: 0, y: 0, z: 1 })
  })
...
describe('normalize', () => {
  ...
  it('normalizing zero vector should return zero', () => {
...
describe('toJSON / fromJSON', () => {
  ...
  it('fromJSON(toJSON(v)) roundtrip should equal v', () => {
    const v = { x: 1.5, y: -2.5, z: 3.5 }
