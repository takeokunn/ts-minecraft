...
import { AIR_BLOCK_INDEX, WOOD_BLOCK_INDEX, LEAVES_BLOCK_INDEX } from './surface-resolver';
import { ROUND_OAK_TRUNK, TALL_BIRCH_TRUNK, SPRUCE_TRUNK, TALL_CANOPY_TRUNK, ROUND_OAK_TIP_RNG_SCALE, ROUND_OAK_TIP_THRESHOLD, SPRUCE_LOWER_RADIUS_RNG_SCALE, SPRUCE_LOWER_RADIUS_THRESHOLD, TALL_CANOPY_TIP_RNG_SCALE, TALL_CANOPY_TIP_THRESHOLD, TREE_RNG_X_SCALE, TREE_RNG_Z_SCALE, TREE_RNG_AMPLITUDE, ARCHETYPE_ROLL_RNG_SCALE, TREE_MIN_SURFACE_Y, TREE_SURFACE_Y_HEADROOM, } from './tree-placer.config';
export const TREE_CANOPY_MARGIN = 2;
const computeTrunkHeight = (config, treeRng) => config.baseHeight + Math.floor(fract(treeRng * config.rngScale) * config.heightRange);
const setWoodBlock = (blocks, lx, y, lz) => {
    if (lx < 0 || lx >= CHUNK_SIZE || lz < 0 || lz >= CHUNK_SIZE || y < 0 || y >= CHUNK_HEIGHT)
        return;
    blocks[chunkBlockIndexUnchecked(lx, y, lz)] = WOOD_BLOCK_INDEX;
};
const setLeafBlock = (blocks, lx, y, lz) => {
    if (lx < 0 || lx >= CHUNK_SIZE || lz < 0 || lz >= CHUNK_SIZE || y < 0 || y >= CHUNK_HEIGHT)
        return;
...
const placeTrunk = (blocks, lx, lz, surfaceY, trunkHeight) => {
    for (let offset = 1; offset <= trunkHeight; offset++) {
        setWoodBlock(blocks, lx, surfaceY + offset, lz);
    }
};
const placeLeafLayer = (blocks, lx, y, lz, radius) => {
    if (radius === 0) {
    ...
    for (let dx = -radius; dx <= radius; dx++) {
        for (let dz = -radius; dz <= radius; dz++) {
            if (dx * dx + dz * dz > distanceLimit)
                continue;
            setLeafBlock(blocks, lx + dx, y, lz + dz);
        }
    }
};
const placeRoundOakTree = (blocks, lx, lz, surfaceY, treeRng) => {
...
const placeTallBirchTree = (blocks, lx, lz, surfaceY, treeRng) => {
...
const placeSpruceTree = (blocks, lx, lz, surfaceY, treeRng) => {
...
const placeTallCanopyTree = (blocks, lx, lz, surfaceY, treeRng) => {
...
export const selectTreeArchetype = (biome, surfaceY, treeRng) => {
...
export const shouldPlaceTree = (treeDensity, surfaceY, wx, wz) => {
    if (treeDensity <= 0
...
export const placeTree = (blocks, lx, lz, surfaceY, biome, treeRng) => {
...
//# sourceMappingURL=../../../../dist/packages/terrain/domain/terrain/tree-placer.js.map