...
const sampleOreY = (cfg, yMin, yMax, rngState) => {
...
export const ORE_REGULAR_INDICES = Arr.map(ORE_CONFIGS, (cfg) => blockTypeToIndex(`${cfg.name}_ORE`));
export const ORE_DEEPSLATE_INDICES = Arr.map(ORE_CONFIGS, (cfg) => blockTypeToIndex(`DEEPSLATE_${cfg.name}_ORE`));
// Stack-based random-walk vein growth. Replaces only STONE/DEEPSLATE; threads rngState for deterministic output.
export const growVein = (blocks, seedX, seedY, seedZ, targetSize, oreIndex, deepslateOreIndex, stoneBlockIndex, deepslateBlockIndex, yMin, yMax, rngState) => {
    let state = rngState;
    let placed = 0;
    // Stack of candidate (lx, y, lz) positions — seeded with the start voxel.
    const stack = [seedX, seedY, seedZ];
    while (placed < targetSize && stack.length > 0) {
        ...
        if (current !== stoneBlockIndex && current !== deepslateBlockIndex)
            continue;
...
export const placeOres = (blocks, baseWorldX, baseWorldZ, oreIndices) => {
    ...
    for (let oreIdx = 0; oreIdx < ORE_CONFIGS.length; oreIdx++) {
        ...
        for (let v = 0; v < count; v++) {
            ...
            for (let attempt = 0; attempt < MAX_SEED_ATTEMPTS && !placed; attempt++) {
                ...
                if (seedCurrent !== stoneBlockIndex && seedCurrent !== deepslateBlockIndex)
                    continue;
                state = growVein(blocks, seedX, seedY, seedZ, veinSize, oreBlock, deepslateOreBlock, stoneBlockIndex, deepslateBlockIndex, yMin, yMax, state);
                placed = true;
            }
        }
    }
};
//# sourceMappingURL=../../../../dist/packages/terrain/domain/terrain/ore-generator.js.map