...
import { LAKE_NOISE_SCALE, LAKE_THRESHOLD, LAKE_SHORE_WIDTH, VARIANT_THRESHOLD, OVERHANG_NOISE_SCALE, OVERHANG_BAND_HEIGHT, OVERHANG_THRESHOLD, } from './constants';
...
export const buildColumnStates = ({ blocks, baseWorldX, baseWorldZ, biomeColumns, terrainChannels, initialSurfaceYs, lakeNoiseVals, graniteNoiseVals, dioriteNoiseVals, andesiteNoiseVals, treeColumnContextCache, blockIndices, }) => {
    const columnStates = [];
    ...
    for (let lx = 0; lx < CHUNK_SIZE; lx++) {
        for (let lz = 0; lz < CHUNK_SIZE; lz++) {
            ...
            const ruggedness = computeRuggedness(terrainChannels.erosion[terrainIndex], terrainChannels.jaggedness[terrainIndex]);
...
export const collectOverhangTargets = (blocks, baseWorldX, baseWorldZ, columnStates, airBlockIndex) => {
    const overhangXs = [];
    const overhangYs = [];
    const overhangZs = [];
    const overhangTargets = [];
    for (let lx = 0; lx < CHUNK_SIZE; lx++) {
        for (let lz = 0; lz < CHUNK_SIZE; lz++) {
            const columnIndex = lx * CHUNK_SIZE + lz;
            ...
            for (let y = surfaceY + 2; y <= bandTop; y++) {
                ...
                if (blocks[blockIndex] !== airBlockIndex)
                    continue;
...
export const applyOverhangNoise = (blocks, overhangTargets, overhangNoiseVals, columnStates, stoneBlockIndex, airBlockIndex) => {
...
export const createTreeColumnContextResolver = ({ biomeService, noiseService, treeColumnContextCache, blockIndices, }) => (wx, wz) => {
...
export const placeChunkTrees = (blocks, baseWorldX, baseWorldZ, resolveTreeColumnContext) => Effect.forEach(Arr.makeBy(CHUNK_SIZE + TREE_CANOPY_MARGIN * 2, (index) => index - TREE_CANOPY_MARGIN), (originLx) => Effect.forEach(Arr.makeBy(CHUNK_SIZE + TREE_CANOPY_MARGIN * 2, (index) => index - TREE_CANOPY_MARGIN), (originLz) => Effect.gen(function* () {
...
//# sourceMappingURL=../../../../dist/packages/terrain/domain/terrain/generator-pipeline.js.map