...
describe('ChunkService.worldToChunkCoord', () => {
  ...
  it.effect('maps (16, 0) to chunk { x: 1, z: 0 }', () =>
    Effect.gen(function* () {
      const cs = yield* ChunkService
  ...
  it.effect('floors negative world coordinates to the correct chunk', () =>
    Effect.gen(function* () {
      const cs = yield* ChunkService
      const coord = yield* cs.worldToChunkCoord(-1, -1)
      expect(coord).toEqual({ x: -1, z: -1 })
    }).pipe(Effect.provide(ChunkServiceLive))
  )

  it.effect('maps arbitrary positive coordinates correctly', () =>
    Effect.gen(function* () {
      const cs = yield* ChunkService
      // worldX=37 → chunk x = floor(37/16) = 2; worldZ=50 → chunk z = floor(50/16) = 3
      const coord = yield* cs.worldToChunkCoord(37, 50)
...
// ---------------------------------------------------------------------------
// ChunkService.chunkToWorldCoord
// ---------------------------------------------------------------------------

describe('ChunkService.chunkToWorldCoord', () => {
  it.effect('maps chunk { x: 0, z: 0 } local (0, 0) to world (0, 0)', () =>
    Effect.gen(function* () {
      const cs = yield* ChunkService
  ...
  it.effect('maps chunk { x: 1, z: 0 } local (0, 0) to world (16, 0)', () =>
    Effect.gen(function* () {
      const cs = yield* ChunkService
  ...
  it.effect('adds localX and localZ offsets to the chunk origin', () =>
    Effect.gen(function* () {
      const cs = yield* ChunkService
  ...
  it.effect('worldToChunkCoord and chunkToWorldCoord are inverse for chunk origins', () =>
    Effect.gen(function* () {
      const cs = yield* ChunkService
      const worldX = 48
      const worldZ = 80
      ...
      const restored = yield* cs.chunkToWorldCoord(chunkCoord, localX, localZ)
      expect(restored).toEqual({ x: worldX, z: worldZ })
    }).pipe(Effect.provide(ChunkServiceLive))
  )
})
