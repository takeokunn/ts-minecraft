...
import { AIR_INDEX, FLUID_TICK_BUDGET, FLOW_OFFSETS, INITIAL_STATE, LAVA_INDEX, LAVA_TICK_INTERVAL, NOTIFY_OFFSETS, WATER_INDEX, blockIndexFor, blockKey, chunkCoordsForPosition, enqueue, getBlockIndex, maxLevelFor, parseKey, } from '@ts-minecraft/world-state';
import { ChunkCacheKey } from '@ts-minecraft/kernel';
import { setFluidBlockIfLoaded, setAirBlockIfLoaded, setSolidBlockIfLoaded, } from './fluid-chunk-ops';
...
export class FluidService extends Effect.Service()('@minecraft/application/FluidService', {
    effect: Effect.all([
        ChunkManagerService,
        ...
        const isAirAt = (loaded, position) => Option.match(HashMap.get(loaded, ChunkCacheKey.make(chunkCoordsForPosition(position))), {
            onNone: () => false,
            onSome: (chunk) => {
        ...
        const blockAt = (loaded, position) => Option.flatMap(HashMap.get(loaded, ChunkCacheKey.make(chunkCoordsForPosition(position))), (chunk) => {
            const idx = getBlockIndex(position);
            /* c8 ignore next */
            return idx >= 0 ? Option.fromNullable(chunk.blocks[idx]) : Option.none();
        });
        const writeFluid = (loaded, position, cell) => setFluidBlockIfLoaded(HashMap.get(loaded, ChunkCacheKey.make(chunkCoordsForPosition(position))), position, cell, chunkManagerService);
        const writeAir = (loaded, position) => setAirBlockIfLoaded(HashMap.get(loaded, ChunkCacheKey.make(chunkCoordsForPosition(position))), position, chunkManagerService);
        const writeSolid = (loaded, position, blockType) => setSolidBlockIfLoaded(HashMap.get(loaded, ChunkCacheKey.make(chunkCoordsForPosition(position))), position, blockType, chunkManagerService);
        const getLoaded = Ref.get(loadedCacheRef).pipe(Effect.flatMap((cached) => HashMap.size(cached) > 0
            ? Effect.succeed(cached)
            : chunkManagerService.getLoadedChunks().pipe(Effect.map((chunks) => HashMap.fromIterable(Arr.map(chunks, (chunk) => [ChunkCacheKey.make(chunk.coord), chunk]))))));
        const seedFluid = (type) => (position) => Effect.gen(function* () {
            const normalized = {
            ...
            const cell = { level: 0, source: true, type };
        ...
        const removeFluid = (position) => Effect.gen(function* () {
            const normalized = {
                x: Math.floor(position.x),
                y: Math.floor(position.y),
                z: Math.floor(position.z),
        ...
        const resolveNeighborContact = (loaded, stateRefLocal, position, cell) => Effect.gen(function* () {
            ...
            return yield* Option.match(hit, {
                ...
                onSome: (offset) => Effect.gen(function* () {
                    const neighborPos = { x: position.x + offset.x, y: position.y + offset.y, z: position.z + offset.z };
                    const neighborCell = Option.getOrElse(HashMap.get(state.cells, blockKey(neighborPos)), () => ({ level: 0, source: true, type: 'water' }));
                    /* c8 ignore next 3 */
                    const [lavaCell, lavaPos, waterCell, waterPos] = cell.type === 'lava'
                        ? [cell, position, neighborCell, neighborPos]
                        : [neighborCell, neighborPos, cell, position];
        ...
        const tryFlowDownward = (loaded, tickStateRef, position, cell, nextLevel) => {
            ...
            return Ref.modify(tickStateRef, (s) => {
                ...
                /* c8 ignore next */
                if (Option.exists(existing, (e) => e.type === cell.type && e.level <= nextLevel))
                    return [false, s];
                const newCell = { level: nextLevel, source: false, type: cell.type };
                const next = setCell(s, below, newCell);
                return [true, { ...next, frontier: HashSet.add(next.frontier, targetKey) }];
            }).pipe(Effect.flatMap((shouldWrite) => 
            /* c8 ignore next */
            shouldWrite ? writeFluid(loaded, below, { level: nextLevel, source: false, type: cell.type }) : Effect.void), Effect.as(true));
        };
        const flowLaterally = (loaded, tickStateRef, position, cell, nextLevel) => Effect.forEach(FLOW_OFFSETS, (offset) => Effect.gen(function* () {
            const target = {
            ...
            if (shouldWrite)
                yield* writeFluid(loaded, target, { level: nextLevel, source: false, type: cell.type });
        }), { concurrency: 1 });
        const processFluidCell = (loaded, tickStateRef, position, cell) => Option.match(HashMap.get(loaded, ChunkCacheKey.make(chunkCoordsForPosition(position))), {
            /* c8 ignore next */
            onNone: () => Effect.void,
            onSome: (currentChunk) => Effect.gen(function* () {
                ...
                if (!cell.source && cell.level >= maxLevel)
                    return;
                const flowedDown = yield* tryFlowDownward(loaded, tickStateRef, position, cell, nextLevel);
                if (!flowedDown)
                    yield* flowLaterally(loaded, tickStateRef, position, cell, nextLevel);
            }),
        });
        return {
            notifyBlockChanged: (position) => Ref.update(stateRef, (state) => ({
                ...state,
            ...
            syncLoadedChunks: (chunks) => Effect.all([
                Ref.set(stateRef, Arr.reduce(chunks, INITIAL_STATE, hydrateChunk)),
                Ref.set(loadedCacheRef, HashMap.fromIterable(Arr.map(chunks, (chunk) => [ChunkCacheKey.make(chunk.coord), chunk]))),
            ...
            tick: () => Effect.gen(function* () {
                ...
                const workWithCells = Arr.filterMap(queued, (key) => Option.map(HashMap.get(state.cells, key), (cell) => ({ key, cell })));
                ...
                const tickStateRef = yield* Ref.make({
                    ...state,
                    tickCounter,
                    frontier: HashSet.fromIterable([...carry, ...retainedLavaFrontier]),
                });
                yield* Effect.forEach(work, ({ key, cell }) => processFluidCell(loaded, tickStateRef, parseKey(key), cell), { concurrency: 1 });
...
//# sourceMappingURL=../../../dist/packages/terrain/application/fluid-service.js.map