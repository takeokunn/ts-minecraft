...
export const chunkLoadHistogram = Metric.histogram('chunk_load_ms', MetricBoundaries.linear({ start: 0, width: 50, count: 20 }), 'Chunk load duration in milliseconds');
// ---------------------------------------------------------------------------
...
const makePureNoisePortLayer = (seed) => Layer.succeed(NoiseServicePort, buildNoisePortFromPrimitives(createNoisePrimitives(seed), seed));
...
export const buildTerrainLayer = (seed) => {
    const noisePort = makePureNoisePortLayer(seed);
    return Layer.mergeAll(ChunkService.Default, BiomeService.Default.pipe(Layer.provide(noisePort)), noisePort);
};
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
const computeInitialLightGrids = (blocks) => {
...
export const createTerrainNoiseCoordinates = (coord) => {
    const baseX = coord.x * CHUNK_SIZE;
    const baseZ = coord.z * CHUNK_SIZE;
    return Arr.flatMap(Arr.makeBy(CHUNK_SIZE, (lx) => lx), (lx) => Arr.makeBy(CHUNK_SIZE, (lz) => ({ wx: baseX + lx, wz: baseZ + lz })));
};
// ---------------------------------------------------------------------------
...
export const buildTerrainProgram = (coord) => Effect.gen(function* () {
