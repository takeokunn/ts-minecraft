...
import { Array as Arr, Cause, Effect, MutableHashMap, MutableRef, Option, Schema, } from 'effect';
import { CHUNK_HEIGHT, CHUNK_SIZE } from '@ts-minecraft/kernel';
import { LIGHT_BYTE_LENGTH } from '@ts-minecraft/world-state';
import { generateTerrainBlocks } from '../application/terrain-generation';
import { TerrainGenerationError } from '../application/terrain-worker-pool-port';
...
const generateTerrainSync = (chunk, options) => Effect.try({
...
// Service
// -----------------------------------------------------------------------------
export class TerrainWorkerPool extends Effect.Service()('@minecraft/infrastructure/terrain/TerrainWorkerPool', {
    scoped: Effect.gen(function* () {
        ...
        if (typeof Worker === 'undefined') {
            return {
                generateTerrain: (chunk, options) => generateTerrainSync(chunk, options),
                workerCount: 0,
                queueDepth: () => 0,
            };
        }
        ...
        const poolWorkersRef = MutableRef.make([]);
        const terminateWorkerSafely = (worker) => {
            try {
                worker.terminate();
            }
            catch {
        ...
        const formatWorkerErrorDetails = (event) => {
            const details = [
            ...
            if (event.error !== undefined && event.error !== null) {
                details.push(`error=${event.error instanceof Error ? event.error.message : String(event.error)}`);
            }
            return details.join(', ');
        };
        const degradeToSync = (reason) => {
        ...
        const makePoolWorker = () => {
            ...
            worker.onerror = (event) => {
                ...
                Effect.runFork(Effect.logError(`${reason}; degrading to synchronous terrain generation (${formatWorkerErrorDetails(event)})`));
        ...
        return {
            generateTerrain: (chunk, options) => {
                ...
                }), Effect.catchAll((err) => Effect.logWarning(`TerrainWorkerPool falling back to sync generation for chunk (${chunk.x},${chunk.z}): ${err.reason}`).pipe(Effect.zipRight(generateTerrainSync(chunk, options)))));
            },
            workerCount,
            queueDepth: () => {
                if (MutableRef.get(degradedRef)) {
