...
const computeSpatial = (listener, source) => {
...
export class SoundManager extends Effect.Service()('@minecraft/audio/SoundManager', {
    effect: Effect.all([
        AudioEnginePort,
        Ref.make(true),
        Ref.make(0.8),
        Ref.make(1.0),
        Ref.make(DEFAULT_LISTENER_POSITION),
        ...
        setListenerPosition: (position) => Ref.set(listenerPositionRef, position),
        playEffect: (effect, options) => Effect.gen(function* () {
            const enabled = yield* Ref.get(enabledRef);
            if (!enabled) {
                return;
            }
            const definition = SOUND_LIBRARY[effect];
            const [listenerPosition, masterVolume, sfxVolume] = yield* Effect.all([Ref.get(listenerPositionRef), Ref.get(masterVolumeRef), Ref.get(sfxVolumeRef)], { concurrency: 'unbounded' });
            const spatial = Option.match(Option.fromNullable(options?.position), {
                onNone: () => ({ gain: 1, pan: 0 }),
                onSome: (pos) => computeSpatial(listenerPosition, pos),
            });
            const finalGain = clamp01(definition.baseGain
                * masterVolume
                * sfxVolume
                * spatial.gain
