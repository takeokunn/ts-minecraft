...
import { PlayerService, MovementService, PlayerCameraStateService } from '@ts-minecraft/player';
import { DeltaTimeSecs, DeltaTimeSecsSchema, DEFAULT_PLAYER_ID, FIRST_FRAME_DELTA_SECS, PLAYER_HALF_WIDTH, PLAYER_HALF_HEIGHT, CHUNK_SIZE } from '@ts-minecraft/kernel';
...
export class GameStateService extends Effect.Service()('@minecraft/application/GameStateService', {
    effect: Effect.all([
        PlayerService,
        PhysicsService,
        MovementService,
        ...
        Ref.make([null, null, null, null, null, null, null, null, null]),
        // Last refreshed player chunk coord. Refresh the 9-cell cache only when this changes.
        Ref.make({ cx: Number.NaN, cz: Number.NaN }),
        ...
        return {
            initialize: (spawnPosition) => Effect.gen(function* () {
            ...
            }).pipe(Effect.catchTag('PhysicsServiceError', (e) => Effect.fail(new GameStateError({ operation: 'initialize', reason: e.operation, cause: e }))), Effect.catchTag('PlayerError', (e) => Effect.fail(new GameStateError({ operation: 'initialize', reason: e.reason, cause: e })))),
            update: (deltaTime) => Effect.gen(function* () {
                ...
                /* c8 ignore next 3 */
                const currentVel = yield* physicsService.getVelocity(playerBodyId).pipe(Effect.catchTag('PhysicsServiceError', (e) => Effect.logWarning(`getVelocity fallback: ${e.message ?? String(e)}`).pipe(Effect.as(ZERO_VEC3))));
                ...
                /* c8 ignore next */
                const physPos = yield* physicsService.getPosition(playerBodyId).pipe(Effect.catchTag('PhysicsServiceError', () => Effect.succeed(ZERO_VEC3)));
                const physVel = yield* physicsService.getVelocity(playerBodyId).pipe(Effect.catchTag('PhysicsServiceError', () => Effect.succeed(ZERO_VEC3)));
                ...
                const { position: resolvedPos, velocity: resolvedVel, isGrounded: newIsGrounded } = resolveBlockCollisions(physPos, physVel, PLAYER_HALF_WIDTH, PLAYER_HALF_HEIGHT, (wx, wy, wz) => isBlockSolid(wx, wy, wz, chunkCache, playerCx, playerCz));
                yield* physicsService.setPosition(playerBodyId, resolvedPos).pipe(Effect.catchTag('PhysicsServiceError', () => Effect.void));
                yield* physicsService.setVelocity(playerBodyId, resolvedVel).pipe(Effect.catchTag('PhysicsServiceError', () => Effect.void));
                yield* Ref.set(isGroundedRef, newIsGrounded);
                yield* playerService.updatePosition(playerId, resolvedPos);
            ...
            }).pipe(Effect.catchTag('PhysicsServiceError', (e) => 
            /* c8 ignore next -- defensive re-raise: only fires if physics service faults */
            Effect.fail(new GameStateError({ operation: 'update', reason: e.operation, cause: e }))), Effect.catchTag('PlayerError', (e) => 
            ...
            respawn: (spawnPosition) => Effect.gen(function* () {
                const playerBodyId = yield* Option.match(yield* Ref.get(playerBodyIdRef), {
            ...
            /* c8 ignore next -- defensive re-raise: only fires if physics service faults */
            Effect.fail(new GameStateError({ operation: 'respawn', reason: e.operation, cause: e }))), Effect.catchTag('PlayerError', (e) => 
            /* c8 ignore next -- defensive re-raise: only fires if player service faults */
            ...
            getCameraRotation: () => cameraState.getRotation(),
            isPlayerGrounded: () => Ref.get(isGroundedRef),
        };
    })),
}) {
}
