...
describe('domain/light', () => {
  ...
  describe('transparency / emissive lookups', () => {
    ...
    it('isTransparentIndex returns false for out-of-range index (exercises ?? 0 fallback)', () => {
      expect(isTransparentIndex(9999)).toBe(false)
    })

    it('emissiveLevelByIndex returns 0 for out-of-range index (exercises ?? 0 fallback)', () => {
      expect(emissiveLevelByIndex(9999)).toBe(0)
    })

    it('isTransparentIndex returns true for AIR index', () => {
      expect(isTransparentIndex(AIR)).toBe(true)
    })

    it('isTransparentIndex returns false for STONE index', () => {
      expect(isTransparentIndex(STONE)).toBe(false)
    })

    it('emissiveLevelByIndex returns 15 for LAVA index', () => {
      expect(emissiveLevelByIndex(LAVA)).toBe(15)
    })

    it('emissiveLevelByIndex returns 0 for non-emissive STONE index', () => {
      expect(emissiveLevelByIndex(STONE)).toBe(0)
    })
  })
