...
describe('InventoryError', () => {
  it('has _tag === "InventoryError"', () => {
    const err = new InventoryError({ operation: 'addItem' })
    expect(err._tag).toBe('InventoryError')
  })

  it('message without cause matches format "Inventory error [<op>]"', () => {
    const err = new InventoryError({ operation: 'addItem' })
    expect(err.message).toBe('Inventory error [addItem]')
  })

  it('message with cause includes operation and cause string', () => {
    const err = new InventoryError({ operation: 'addItem', cause: new Error('slot full') })
    expect(err.message).toContain('addItem')
    expect(err.message).toContain('slot full')
  })

  it('message with plain-string cause includes that string', () => {
    const err = new InventoryError({ operation: 'removeItem', cause: 'index out of bounds' })
    expect(err.message).toContain('index out of bounds')
  })
})

describe('RecipeError', () => {
  it('has _tag === "RecipeError"', () => {
    const err = new RecipeError({ operation: 'craft' })
    expect(err._tag).toBe('RecipeError')
  })

  it('message without cause matches format "Recipe error [<op>]"', () => {
    const err = new RecipeError({ operation: 'craft' })
    expect(err.message).toBe('Recipe error [craft]')
  })

  it('message with cause includes operation and cause string', () => {
    const err = new RecipeError({ operation: 'craft', cause: new Error('missing ingredient') })
    expect(err.message).toContain('craft')
    expect(err.message).toContain('missing ingredient')
  })

  it('message with plain-string cause includes that string', () => {
    const err = new RecipeError({ operation: 'smelt', cause: 'no fuel' })
    expect(err.message).toContain('no fuel')
  })
})
