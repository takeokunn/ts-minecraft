...
export class PlayerHealth extends Schema.Class('PlayerHealth')({
...
export const PlayerHealthInvariant = PlayerHealth.pipe(Schema.filter((h) => h.current <= h.max
    ? true
    : `current health (${h.current}) must not exceed max health (${h.max})`, { identifier: 'PlayerHealthInvariant' }));
//# sourceMappingURL=../../../dist/packages/player/domain/player-health.js.map