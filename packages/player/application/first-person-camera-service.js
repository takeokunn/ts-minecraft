...
export class FirstPersonCameraService extends Effect.Service()('@minecraft/application/FirstPersonCameraService', {
    effect: Effect.all([PlayerInputService, PlayerCameraStateService], { concurrency: 'unbounded' }).pipe(Effect.map(([inputService, cameraState]) => ({
        update: (camera, sensitivity = 0.5) => Effect.gen(function* () {
        ...
        attachToPlayer: (camera) => Effect.gen(function* () {
            const rotation = yield* cameraState.getRotation();
            yield* Effect.sync(() => { camera.rotation.set(rotation.pitch, rotation.yaw, 0, 'YXZ'); });
        }),
    })))
