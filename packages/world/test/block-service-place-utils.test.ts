...
import { BlockType,Position,SlotIndex } from '@ts-minecraft/core'
import {
BlockService,
BlockServiceError,
} from '../application/block-service'
...
describe('BlockService.placeBlock', () => {
  ...
  it.effect('rolls back and fails when inventory has no matching block', () => {
    ...
    const layer = createTestLayer(
      ...
      createMockPlayerService({ x: 100, y: 0, z: 100 }),
      undefined,
      createMockInventoryService({ removeBlock: () => Effect.fail(new InventoryError({ operation: 'removeBlock' })) }),
    )
    return Effect.gen(function* () {
  ...
  it.effect('fails with BlockServiceError when the item type is not a placeable block (e.g. a tool)', () => {
    const pos: Position = { x: 0, y: 0, z: 0 }
    const layer = createTestLayer(
      createMockChunkManagerService().service,
      createMockPlayerService({ x: 100, y: 0, z: 100 }),
    ...
    return Effect.gen(function* () {
      // 'UNKNOWN_BLOCK_XYZ' is not in NON_PLACEABLE_ITEM_TYPES and not in BlockTypeSchema —
      // reaches the Schema.decodeUnknownEither check and fails there (lines 157-161).
      const result = yield* Effect.either(
        Effect.flatMap(BlockService, (svc) =>
          svc.placeBlock(pos, 'UNKNOWN_BLOCK_XYZ' as unknown as BlockType),
        ),
      )
      const err = assertLeft(result) as BlockServiceError
      expect(err.operation).toBe('placeBlock')
      expect(err.message).toContain('cannot be placed in the world')
    }).pipe(Effect.provide(layer))
  })
