...
import { BlockService, BlockServiceError, BlockServiceLive, ChunkManagerService, FluidService } from '@ts-minecraft/world'
import { ChunkServiceLive } from '@ts-minecraft/world/application/chunk-service'
import { FurnaceService, HotbarService, InventoryService } from '@ts-minecraft/inventory'
import { PlayerService } from '@ts-minecraft/entity'
import { CHUNK_HEIGHT, CHUNK_SIZE, Position, SlotIndex, blockTypeToIndex } from '@ts-minecraft/core'
...
const buildLayer = (chunk: ReturnType<typeof makeChunk>, fluidSvc: Layer.Layer<FluidService>) => BlockServiceLive.pipe(
  Layer.provide(makeChunkManagerLayer(chunk)), Layer.provide(ChunkServiceLive), Layer.provide(fluidSvc),
