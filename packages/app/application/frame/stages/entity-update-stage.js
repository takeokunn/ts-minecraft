...
export const entityUpdateStage = (deps, services, refs, inputs) => Effect.gen(function* () {
    ...
    if (mobsPhysicsEnabled && typeof applyPhysics === 'function') {
        ...
        if (chunkCoordChanged || loadedChunksChanged || hasMissingChunk) {
            ...
            const nextChunkCache = refreshAllChunks
                ? [
                    null, null, null,
                    null, null, null,
                    null, null, null,
                ...
                : [...chunkCache];
            yield* Effect.forEach(ENTITY_PHYSICS_CHUNK_OFFSETS, ([dx, dz, index]) => {
        ...
        yield* logErrors(applyPhysics(inputs.deltaTime, (position, velocity) => resolveBlockCollisions(position, velocity, MOB_HALF_WIDTH, MOB_HALF_HEIGHT, isBlockSolid)), 'Entity physics error');
    }
    yield* logErrors(Effect.all([services.entityManager.getEntities(), services.entityManager.getStructureVersion()], { concurrency: 'unbounded' }).pipe(Effect.flatMap(([entitiesSnapshot, structureVersion]) => Ref.get(refs.lastEntityStructureVersionRef).pipe(Effect.flatMap((lastStructureVersion) => {
        if (!mobsRenderEnabled) {
            return lastStructureVersion === RENDER_DISABLED_STRUCTURE_VERSION
                ? Effect.void
                : services.entityRenderer.syncEntities([], deps.scene).pipe(Effect.andThen(Ref.set(refs.lastEntityStructureVersionRef, RENDER_DISABLED_STRUCTURE_VERSION)));
        }
        return (lastStructureVersion === structureVersion
            ? Effect.void
            : services.entityRenderer.syncEntities(entitiesSnapshot, deps.scene).pipe(Effect.andThen(Ref.set(refs.lastEntityStructureVersionRef, structureVersion)))).pipe(Effect.andThen(services.entityRenderer.updateEntityTransforms(entitiesSnapshot, inputs.totalTimeSecs, inputs.deltaTime)));
    })))), 'Entity render error');
    if (debugFlags['simulation.redstone']) {
        yield* logErrors(Ref.modify(refs.redstoneTickAccumulatorRef, (accumulated) => {
            const { ticks, remainder } = advanceFixedStep(accumulated, inputs.deltaTime, REDSTONE_TICK_INTERVAL_SECS);
            return [ticks, remainder];
        }).pipe(Effect.flatMap((ticksToRun) => ticksToRun === 1
            ...
            : ticksToRun > 1
                ...
                : Effect.void)), 'Redstone system error');
    }
    if (debugFlags['simulation.fluid']) {
        yield* logErrors(Ref.modify(refs.fluidTickAccumulatorRef, (accumulated) => {
            ...
            : ticksToRun > 1
                ...
                : Effect.void)), 'Fluid system error');
    }
...
//# sourceMappingURL=../../../../../dist/packages/app/application/frame/stages/entity-update-stage.js.map