...
export const cameraStage = (deps, services, refs, inputs) => Effect.gen(function* () {
    // Handle view toggle: F5 switches first/third person before raycasting
    yield* logErrors(services.inputService.consumeKeyPress(KeyMappings.CAMERA_TOGGLE).pipe(Effect.flatMap((pressed) => (pressed ? services.playerCameraState.toggleMode() : Effect.void))), 'Camera toggle error');
    // Camera perspective + position sync (use current mode before raycasting)
    yield* Effect.all([services.playerCameraState.getRotation(), services.playerCameraState.getMode()], { concurrency: 'unbounded' }).pipe(Effect.flatMap(([rotation, cameraMode]) => cameraMode === 'firstPerson'
        ? Effect.sync(() => {
    ...
    Effect.andThen(Ref.modify(refs.lastRenderDistanceRef, (lastRd) => lastRd === inputs.renderDistance ? [false, lastRd] : [true, inputs.renderDistance])), Effect.flatMap((changed) => changed
        ? Effect.sync(() => {
            ...
            const halfExtent = Math.min(Math.ceil(inputs.renderDistance * CHUNK_SIZE * 0.5), MAX_SHADOW_HALF_EXTENT);
...
//# sourceMappingURL=../../../../../dist/packages/app/application/frame/stages/camera-stage.js.map