...
const handleEscape = (deps, services) => Effect.gen(function* () {
...
const handleInventoryKey = (deps, services) => Effect.gen(function* () {
...
const handleTradeKeys = (deps, services, inputs) => Effect.gen(function* () {
        ...
        if (tradeOpen) {
            ...
            if (!isInvOpen && !isSettingsOpen) {
                ...
                yield* Option.match(villagerOption, {
                    onNone: () => Effect.void,
                    onSome: (villager) => services.tradingPresentation.open(villager.villagerId).pipe(Effect.flatMap((opened) => (opened ? Ref.set(deps.gamePausedRef, true) : Effect.void))),
                });
            }
        }
    }
...
const syncDayLength = (services, inputs) => Effect.gen(function* () {
...
export const inputStage = (deps, services, inputs) => Effect.gen(function* () {
    // Update camera rotation from mouse look (suppressed when a modal is open)
    yield* Ref.get(deps.gamePausedRef).pipe(Effect.flatMap((paused) => paused ? Effect.void : services.firstPersonCamera.update(deps.camera, inputs.mouseSensitivity)));
    yield* logErrors(Effect.all([
    ...
    ], { discard: true }), 'Overlay error');
});
//# sourceMappingURL=../../../../../dist/packages/app/application/frame/stages/input-stage.js.map