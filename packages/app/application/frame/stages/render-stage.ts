...
export const renderStage = (
  ...
  services.debugFeatureFlags.getFlags().pipe(
    Effect.flatMap((debugFlags) =>
      Effect.zipWith(
        Ref.get(refs.totalTimeSecsRef),
        Ref.get(refs.attackSwingStateRef),
        (totalTimeSecs, swingState) => ({ totalTimeSecs, swingState, debugFlags }),
      ),
    ),
    Effect.flatMap(({ totalTimeSecs, swingState, debugFlags }) =>
      Effect.sync(() => {
    ...
    Effect.flatMap(() => services.perfHud.setDrawCalls(deps.renderer.info.render.calls)),
  )
