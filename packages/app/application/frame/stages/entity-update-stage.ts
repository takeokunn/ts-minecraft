...
import { isBlockSolid as isCachedBlockSolid, resolveBlockCollisionsInto } from '@ts-minecraft/game'
...
export const entityUpdateStage = (
  ...
  services: Pick<
    ...
    'chunkManagerService' | 'entityManager' | 'entityRenderer' | 'redstoneService' | 'fluidService' | 'particleSystem' | 'inventoryService' | 'equipmentService' | 'xpService'
  >,
  refs: Pick<
    FrameStageRefs,
  ...
  Effect.gen(function* () {
    ...
    if (mobsAiEnabled) {
      ...
      if (births > 0) {
        yield* logErrors(addExperienceWithMending(births * BREED_XP_REWARD, services), 'Breeding XP error')
      }
    }
