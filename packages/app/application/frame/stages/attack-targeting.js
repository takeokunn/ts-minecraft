...
export const findAttackableEntity = (entities, camera, maxDistance) => {
    const cameraDirection = new THREE.Vector3();
    camera.getWorldDirection(cameraDirection);
    cameraDirection.normalize();
    const rayOrigin = camera.position;
    const radiusSq = PLAYER_ATTACK_RADIUS * PLAYER_ATTACK_RADIUS;
    const closest = Arr.reduce(entities, Option.none(), (acc, entity) => {
        const toEntity = new THREE.Vector3(entity.position.x - rayOrigin.x, entity.position.y + ENTITY_CENTER_Y_OFFSET - rayOrigin.y, entity.position.z - rayOrigin.z);
        const alongRay = toEntity.dot(cameraDirection);
        if (alongRay < 0 || alongRay > PLAYER_ATTACK_REACH)
            return acc;
        /* c8 ignore next */
        if (Option.match(maxDistance, { onNone: () => false, onSome: (d) => alongRay > d }))
            return acc;
...
//# sourceMappingURL=../../../../../dist/packages/app/application/frame/stages/attack-targeting.js.map