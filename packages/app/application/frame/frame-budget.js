...
export const TargetFpsSchema = Schema.Number.pipe(Schema.finite(), Schema.positive(), Schema.brand('TargetFps'));
export const TargetFps = {
    make: (n) => Schema.decodeUnknownSync(TargetFpsSchema)(n),
    toNumber: (fps) => fps,
};
const fpsValue = (fps) => typeof fps === 'number' ? fps : TargetFps.toNumber(fps);
/**
 * Total frame budget in milliseconds for the given target FPS.
 * 60 FPS → 16.667 ms, 120 FPS → 8.333 ms, 240 FPS → 4.167 ms.
 */
export const computeFrameBudgetMs = (targetFps) => 1000 / fpsValue(targetFps);
/**
 ...
 */
export const computeChunkSyncBudgetMs = (targetFps) => 240 / fpsValue(targetFps);
/**
 ...
 */
export const computeMaxChunkUpdatesPerFrame = (targetFps) => Math.ceil((8 * fpsValue(targetFps)) / 60);
/**
 * Hard-cap on dirty chunk remeshes flushed per frame.
 * 60 FPS → 4, 120 FPS → 8, 240 FPS → 16.
 */
export const computeMaxDirtyChunkUpdatesPerFrame = (targetFps) => Math.ceil((4 * fpsValue(targetFps)) / 60);
/**
 ...
 */
export const DEFAULT_TARGET_FPS = TargetFps.make(120);
//# sourceMappingURL=../../../../dist/packages/app/application/frame/frame-budget.js.map