...
export const getNormalizedLookDirection = (camera) => {
    const direction = new THREE.Vector3();
    camera.getWorldDirection(direction);
    direction.normalize();
    return direction;
};
export const projectBlockAhead = (camera, distance) => {
...
export const getChunkAccessForWorldPosition = (position) => {
    const chunkCoord = {
...
export const NEARBY_BLOCK_DY_OFFSETS = [-1, 0, 1, 2];
export const scanNearbyBlock = (playerPos, searchRadius, targetBlockIndex, getChunk) => {
    ...
    const searchCoords = Arr.flatMap(dxRange, (dx) => Arr.flatMap(dyRange, (dy) => Arr.map(dzRange, (dz) => ({ dx, dy, dz }))));
    return Effect.iterate({ found: false, i: 0 }, {
        ...
        body: (s) => {
            ...
            return getChunk(chunkCoord).pipe(Effect.map((chunkOpt) => {
                const chunk = Option.getOrNull(chunkOpt);
                if (chunk === null)
                    return { found: false, i: s.i + 1 };
...
//# sourceMappingURL=../../../../dist/packages/app/application/main/qa-spatial.js.map