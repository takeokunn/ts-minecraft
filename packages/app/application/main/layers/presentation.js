...
import { ChunkManagerLayer, CameraStateLayer, FurnaceLayer, GameLayer, HotbarLayer, HotbarRendererLayer, InventoryLayer, PlayerInputLayer, RaycastingLayer, TradingLayer, VillageLayer, } from './game-logic';
// Level 3: Crosshair depends on DomOperations
export const CrosshairLayer = CrosshairLive.pipe(Layer.provide(DomOperationsLive));
// BlockHighlight depends on RaycastingService
export const BlockHighlightLayer = BlockHighlightLive.pipe(Layer.provide(RaycastingLayer));
// FirstPersonCameraService depends on PlayerInputService and PlayerCameraState
export const FirstPersonCameraLayer = FirstPersonCameraServiceLive.pipe(Layer.provide(PlayerInputLayer), Layer.provide(CameraStateLayer));
// ThirdPersonCameraService depends on PlayerCameraState
export const ThirdPersonCameraLayer = ThirdPersonCameraServiceLive.pipe(Layer.provide(CameraStateLayer));
// SettingsService: reads/writes localStorage and queries EnvironmentPort for localhost detection
export const SettingsLayer = SettingsServiceLive.pipe(Layer.provide(EnvironmentLayer));
// SettingsOverlay depends on SettingsService and DomOperations
export const SettingsOverlayLayer = SettingsOverlayLive.pipe(Layer.provide(SettingsLayer), Layer.provide(DomOperationsLive));
// ConfirmDialog depends only on DomOperations — generic modal reusable by
// PauseMenu (Save & Quit) and W2a MainMenu (world delete).
export const ConfirmDialogLayer = ConfirmDialogLive.pipe(Layer.provide(DomOperationsLive));
// PauseMenu depends on DomOperations, SettingsOverlay, ChunkManager, ConfirmDialog.
export const PauseMenuLayer = PauseMenuLive.pipe(Layer.provide(DomOperationsLive), Layer.provide(SettingsOverlayLayer), Layer.provide(ChunkManagerLayer), Layer.provide(ConfirmDialogLayer));
...
export const MainMenuLayer = MainMenuLive.pipe(Layer.provide(StorageServiceLive), Layer.provide(DomOperationsLive), Layer.provide(ConfirmDialogLayer));
...
export const DeathScreenLayer = DeathScreenLive.pipe(Layer.provide(DomOperationsLive), Layer.provide(GameLayer), Layer.provide(GameModeServiceLive), Layer.provide(HealthServiceLive), Layer.provide(InventoryLayer));
...
export const LoadingScreenLayer = LoadingScreenLive.pipe(Layer.provide(DomOperationsLive));
...
export const InventoryRendererLayer = InventoryRendererLive.pipe(Layer.provide(InventoryLayer), Layer.provide(HotbarLayer), Layer.provide(RecipeServiceLive), Layer.provide(FurnaceLayer), Layer.provide(GameLayer), Layer.provide(ChunkManagerLayer), Layer.provide(DomOperationsLive));
// Trading overlay depends on trading + village services and DOM operations
export const TradingPresentationLayer = TradingPresentationLive.pipe(Layer.provide(TradingLayer), Layer.provide(VillageLayer), Layer.provide(DomOperationsLive));
// Top-level bundle re-exported by index.ts
export const PresentationLayers = Layer.mergeAll(CrosshairLayer, BlockHighlightLayer, FirstPersonCameraLayer, ThirdPersonCameraLayer, HotbarRendererLayer, SettingsLayer, SettingsOverlayLayer, ConfirmDialogLayer, PauseMenuLayer, MainMenuLayer, DeathScreenLayer, LoadingScreenLayer, DebugOverlayLayer, InventoryRendererLayer, TradingPresentationLayer, FPSCounterService.Default);
//# sourceMappingURL=../../../../../dist/packages/app/application/main/layers/presentation.js.map