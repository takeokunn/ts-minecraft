...
import { StoragePortLayer, NoisePortLayer, TerrainWorkerPoolPortLayer, BiomeLayer, PhysicsLayer, } from './infrastructure';
// PlayerInputServiceLive: adapter bridging InputService (presentation) to PlayerInputService (application)
export const PlayerInputLayer = PlayerInputServiceLive.pipe(Layer.provide(InputServiceLive));
// MovementService depends on PlayerInputService
export const MovementLayer = MovementServiceLive.pipe(Layer.provide(PlayerInputLayer));
...
export const ChunkManagerLayer = ChunkManagerServiceLive.pipe(Layer.provide(Layer.mergeAll(ChunkServiceLive, StoragePortLayer, BiomeLayer, NoisePortLayer, LightEngineLayer, TerrainWorkerPoolPortLayer)));
// Fluid simulation service depends on chunk and world state
export const FluidLayer = FluidServiceLive.pipe(Layer.provide(ChunkManagerLayer));
...
export const InventoryLayer = InventoryServiceLive.pipe(Layer.provide(BlockRegistryLive));
...
export const GameLayer = GameStateServiceLive.pipe(Layer.provide(PlayerServiceLive), Layer.provide(PhysicsLayer), Layer.provide(MovementLayer), Layer.provide(CameraStateLayer), Layer.provide(ChunkManagerLayer), Layer.provide(GameModeServiceLive), Layer.provide(InventoryLayer));
// Level 4: WorldRendererService depends on ChunkMeshService and SceneService
export const WorldRendererLayer = WorldRendererServiceLive.pipe(Layer.provide(ChunkMeshServiceLive), Layer.provide(SceneServiceLive));
// Level 4: EntityRendererService depends on SceneService — mirrors WorldRendererLayer
export const EntityRendererLayer = EntityRendererLive.pipe(Layer.provide(SceneServiceLive));
// Level 4: ParticleSystemService depends on ChunkMeshService (atlas texture)
export const ParticleSystemLayer = ParticleSystemServiceLive.pipe(Layer.provide(ChunkMeshServiceLive));
// Audio infrastructure and managers
export const AudioEngineLayer = AudioEngineLive;
export const AudioEnginePortLayer = AudioEnginePortLive.pipe(Layer.provide(AudioEngineLayer));
export const SoundLayer = SoundManagerLive.pipe(Layer.provide(AudioEnginePortLayer));
export const MusicLayer = MusicManagerLive.pipe(Layer.provide(AudioEnginePortLayer));
...
export const MobSpawnerLayer = MobSpawnerLive.pipe(Layer.provide(EntityLayer), Layer.provide(TimeServiceLive));
// HotbarService depends on PlayerInputService and InventoryService
export const HotbarLayer = HotbarServiceLive.pipe(Layer.provide(PlayerInputLayer), Layer.provide(InventoryLayer));
export const FurnaceLayer = FurnaceServiceLive.pipe(Layer.provide(RecipeServiceLive), Layer.provide(InventoryLayer), Layer.provide(PlayerServiceLive), Layer.provide(ChunkManagerLayer));
// Level 4: BlockService depends on ChunkManagerService, ChunkService, PlayerService, InventoryService (all independent peers)
export const BlockLayer = BlockServiceLive.pipe(Layer.provide(Layer.mergeAll(ChunkManagerLayer, ChunkServiceLive, FluidLayer, PlayerServiceLive, InventoryLayer, FurnaceLayer, HotbarServiceLive.pipe(Layer.provide(PlayerInputLayer), Layer.provide(InventoryLayer)))));
// HotbarRenderer depends on RendererService
export const HotbarRendererLayer = HotbarRendererLive.pipe(Layer.provide(RendererServiceLive), Layer.provide(TextureServiceLive));
...
export const TradingLayer = TradingServiceLive.pipe(Layer.provide(VillageLayer), Layer.provide(InventoryLayer));
...
export const GameLogicLayers = Layer.mergeAll(GameLayer, ChunkManagerLayer, BlockLayer, HotbarLayer, InventoryLayer, SoundLayer, MusicLayer, EntityLayer, TimeServiceLive, VillageLayer, TradingLayer, RedstoneLayer, FluidLayer, LightEngineLayer, MobSpawnerLayer, HealthServiceLive, RecipeServiceLive, FurnaceLayer, PlayerInputLayer, MovementLayer, CameraStateLayer, DebugFeatureFlagsLayer, RaycastingLayer, WorldRendererLayer, // moved from InfrastructureLayers: orchestrates application→infrastructure
EntityRendererLayer, ParticleSystemLayer, GameModeServiceLive);
//# sourceMappingURL=../../../../../dist/packages/app/application/main/layers/game-logic.js.map