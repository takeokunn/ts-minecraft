...
// Three.js services
RendererServiceLive, SceneServiceLive, PerspectiveCameraServiceLive, TextureServiceLive, BlockMeshServiceLive, BlockRegistryLive, 
// Domain services
PlayerServiceLive, 
// Input and DOM
InputServiceLive, DomOperationsLive, 
// FPS counter presentation wiring lives in PresentationLayers.
StorageServiceLive, ChunkServiceLive, ChunkMeshServiceLive, 
// Game loop service (no external service dependencies)
GameLoopServiceLive);
...
export const NoisePortLayer = Layer.effect(NoiseServicePort, Effect.map(NoiseService, (noise) => {
...
export const StoragePortLayer = Layer.effect(StorageServicePort, Effect.map(StorageService, (storage) => {
    return StorageServicePort.of({
        _tag: '@minecraft/application/storage/StorageServicePort',
...
export const BiomeLayer = BiomeServiceLive.pipe(Layer.provide(NoisePortLayer));
// Level 2: PhysicsService depends on PhysicsWorldPort, RigidBodyPort, ShapePort (all independent peers, satisfied by bridges)
export const PhysicsLayer = PhysicsServiceLive.pipe(Layer.provide(Layer.mergeAll(PhysicsWorldPortLayer, RigidBodyPortLayer, ShapePortLayer)));
...
export const InfrastructureLayers = Layer.mergeAll(BaseLayer, NoiseLayer, BiomeLayer, PhysicsLayer);
//# sourceMappingURL=../../../../../dist/packages/app/application/main/layers/infrastructure.js.map