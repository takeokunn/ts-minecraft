import { Cause, Effect, Fiber, MutableRef, Option } from 'effect';
...
export class DeathScreenService extends Effect.Service()('@minecraft/presentation/DeathScreen', {
    scoped: Effect.flatMap(Effect.all([DomOperationsService, GameStateService, GameModeService, HealthService], { concurrency: 'unbounded' }), ([dom, gameState, gameMode, healthService]) => Effect.acquireRelease(Effect.sync(() => {
    ...
    })).pipe(Effect.map(({ backdropEl, respawnBtn, quitBtn }) => {
        ...
        const captureDeathContext = () => gameState.getPlayerPosition(DEFAULT_PLAYER_ID).pipe(Effect.map((deathPos) => ({ deathPos })), Effect.catchAllCause(() => Effect.succeed({ deathPos: { x: 0, y: 64, z: 0 } })));
        const performRespawn = (spawnPosition) => captureDeathContext().pipe(Effect.flatMap(({ deathPos }) => Effect.log(`Player died at (${deathPos.x}, ${deathPos.y}, ${deathPos.z}); respawning at (${spawnPosition.x}, ${spawnPosition.y}, ${spawnPosition.z})`)), Effect.andThen(healthService.reset()), 
        // `gameState.respawn` is mode-aware: clears inventory in
        // survival, preserves it in creative.
        Effect.andThen(gameState.respawn(spawnPosition).pipe(Effect.catchAllCause((cause) => Effect.logError(`Death-screen respawn error: ${Cause.pretty(cause)}`)))), Effect.asVoid);
        return {
            // Scope-managed: fiber is interrupted and DOM torn down when the surrounding session scope closes.
            attach: (control, spawnPosition) => Effect.acquireRelease(Effect.gen(function* () {
                ...
                const deathWatcher = Effect.gen(function* () {
                ...
                }).pipe(Effect.forever, Effect.catchAllCause((cause) => Effect.logError(`Death-screen watcher error: ${Cause.pretty(cause)}`)));
...
//# sourceMappingURL=../../../../dist/packages/app/presentation/menu/death-screen.js.map