import { Cause, Duration, Effect, Fiber, MutableRef, Option, Schedule } from 'effect';
...
import { requestQuitToTitle, setPaused, } from '@ts-minecraft/app/main/session-control';
...
const SAVE_QUIT_CONFIRM_MESSAGE = 'Save and return to title?\nLast 5s of progress will be flushed first.';
export class PauseMenuService extends Effect.Service()('@minecraft/presentation/PauseMenu', {
    scoped: Effect.flatMap(Effect.all([DomOperationsService, SettingsOverlayService, ConfirmDialogService], { concurrency: 'unbounded' }), ([dom, settingsOverlay, confirmDialog]) => Effect.acquireRelease(Effect.sync(() => {
    ...
    }), ({ backdropEl }) => Effect.sync(() => {
        Option.map(backdropEl, (el) => {
            Option.map(dom.getParentNode(el), () => dom.removeChild(el));
        });
    })).pipe(Effect.map(({ backdropEl, resumeBtn, settingsBtn, saveQuitBtn }) => {
        ...
        const activeControlRef = MutableRef.make(Option.none());
        const showMenu = () => {
        ...
        return {
            ...
            attach: (control) => Effect.acquireRelease(Effect.sync(() => {
                MutableRef.set(activeControlRef, Option.some(control));
                const handleResumeClick = () => {
                    if (!MutableRef.get(isOpenRef))
                        return;
                ...
                const watchdogFiberRef = MutableRef.make(Option.none());
                const stopWatchdog = () => Effect.gen(function* () {
                ...
                const startWatchdog = () => Effect.gen(function* () {
                    ...
                    yield* stopWatchdog();
                    const fiber = yield* settingsOverlay.isOpen().pipe(Effect.flatMap((isSettingsOpen) => !isSettingsOpen && MutableRef.get(isOpenRef)
                        ? Effect.sync(() => { showMenu(); }).pipe(Effect.andThen(stopWatchdog()))
                        : Effect.void), Effect.catchAllCause(() => Effect.void), Effect.repeat(Schedule.spaced(Duration.millis(100))), 
                ...
                const openSettingsFromMenu = () => {
                    ...
                    hideMenu();
                    Effect.runFork(settingsOverlay.isOpen().pipe(Effect.flatMap((open) => open ? Effect.void : settingsOverlay.toggle().pipe(Effect.asVoid)), Effect.andThen(startWatchdog()), Effect.catchAllCause((cause) => Effect.logError(`Pause→Settings open error: ${Cause.pretty(cause)}`))));
                };
                ...
                const handleSaveQuitClick = () => {
                    ...
                    Effect.runFork(confirmDialog
                        ...
                        : Effect.sync(() => {
                            if (MutableRef.get(isOpenRef))
                                showMenu();
                        })), Effect.catchAllCause((cause) => Effect.logError(`Save & Quit dialog error: ${Cause.pretty(cause)}`))));
                };
            ...
            isOpen: () => Effect.sync(() => MutableRef.get(isOpenRef)),
        };
    }))),
}) {
}
export const PauseMenuLive = PauseMenuService.Default;
//# sourceMappingURL=../../../../dist/packages/app/presentation/menu/pause-menu.js.map