...
export class InputService extends Effect.Service()('@minecraft/presentation/InputService', {
    scoped: Effect.all([
        // Refs for all mutable input state — DOM handlers use Effect.runSync for synchronous access
        Effect.sync(() => MutableRef.make(HashSet.empty())),
        ...
        Effect.sync(() => MutableRef.make(HashMap.empty())),
    ...
    ], { concurrency: 'unbounded' }).pipe(Effect.flatMap(([pressedKeysRef, justPressedKeysRef, mouseDeltaRef, mouseButtonsRef, justClickedButtonsRef, wheelDeltaRef, pointerLockFallbackRef]) => {
        ...
        return registerListeners.pipe(Effect.as({
            isKeyPressed: (key) => Effect.sync(() => HashSet.has(MutableRef.get(pressedKeysRef), key)),
            consumeKeyPress: (key) => Effect.sync(() => {
            ...
            getMouseDelta: () => Effect.sync(() => {
                const delta = MutableRef.get(mouseDeltaRef);
                MutableRef.set(mouseDeltaRef, { x: 0, y: 0 });
                return delta;
            }),
            isMouseDown: (button) => Effect.sync(() => Option.getOrElse(HashMap.get(MutableRef.get(mouseButtonsRef), button), () => false)),
            ...
            isPointerLocked: () => Effect.sync(() => document.pointerLockElement instanceof HTMLCanvasElement || MutableRef.get(pointerLockFallbackRef)),
            consumeMouseClick: (button) => Effect.sync(() => {
...
export const PlayerInputServiceLive = Layer.effect(PlayerInputService, Effect.map(InputService, (input) => ({
    _tag: '@minecraft/application/PlayerInputService',
    isKeyPressed: (key) => input.isKeyPressed(key),
    consumeKeyPress: (key) => input.consumeKeyPress(key),
    consumeWheelDelta: () => input.consumeWheelDelta(),
    getMouseDelta: () => input.getMouseDelta(),
    isPointerLocked: () => input.isPointerLocked(),
})));
//# sourceMappingURL=../../../../dist/packages/app/presentation/input/input-service.js.map