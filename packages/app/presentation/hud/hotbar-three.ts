...
export class HotbarRendererService extends Effect.Service<HotbarRendererService>()(
  '@minecraft/presentation/HotbarRenderer',
  {
    scoped: Effect.all([
      ...
      Ref.make<{ slotsKey: string; selected: SlotIndex }>({ slotsKey: '', selected: SlotIndex.make(0) }),
    ], { concurrency: 'unbounded' }).pipe(
      Effect.flatMap(([rendererService, textureService, { hudScene, borderMesh }, prevStateRef]) =>
        Effect.gen(function* () {
