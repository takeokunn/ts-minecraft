...
const debugFeatureBadges = (entry: DebugFeatureCatalogEntry): ReadonlyArray<DebugFeatureBadge> =>
...
type GroupSectionNodes = {
  readonly group: DebugFeatureFlagGroup
  readonly section: HTMLDivElement
}

const badgeStyles: Record<'danger' | 'reload' | 'perf', string> = {
...
export class DebugOverlayService extends Effect.Service<DebugOverlayService>()(
  '@minecraft/presentation/DebugOverlayService',
  {
    scoped: Effect.sync(() => {
      ...
      const setStatus = (dom: DomNodes, message: string): void => {
        dom.statusText.nodeValue = message
      }

      const updateToggleRowsFromFlags = (dom: DomNodes, flags: DebugFeatureFlags): void => {
        let enabledCount = 0
      ...
      const filterToggleRows = (dom: DomNodes): void => {
      ...
      const runPanelEffect = (dom: DomNodes, effect: Effect.Effect<void, never>): void => {
      ...
      const buildDom = (deps: DebugOverlayDeps): DomNodes => {
        ...
        for (const group of DEBUG_FEATURE_GROUP_ORDER) {
          ...
          for (const entry of entries) {
            ...
            const activate = (): void => {
              const nextEffect = deps.debugFeatureFlags.isEnabled(entry.id).pipe(
                Effect.flatMap((enabled) => deps.debugFeatureFlags.setEnabled(entry.id, !enabled)),
                Effect.flatMap((changed) => refreshTogglePanel(
                  MutableRef.get(domRef) ?? dom,
                  deps,
                  changed ? `${entry.label} toggled` : `${entry.label} unchanged`,
                )),
              )
          ...
          resetGroupButton.addEventListener('click', () => {
            ...
            runPanelEffect(
              dom,
              deps.debugFeatureFlags.resetGroup(group).pipe(
                Effect.andThen(refreshTogglePanel(dom, deps, `${debugFeatureGroupLabels[group]} reset`)),
              ),
            )
          })
        ...
        resetAllButton.addEventListener('click', () => {
          runPanelEffect(
            dom,
            deps.debugFeatureFlags.resetAll().pipe(
              Effect.andThen(refreshTogglePanel(dom, deps, 'All debug toggles reset')),
            ),
          )
        })
