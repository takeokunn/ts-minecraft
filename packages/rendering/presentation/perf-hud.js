...
const computePercentiles = (ring, scratch, validLength) => {
    if (validLength === 0)
        return { p50Ms: 0, p99Ms: 0 };
...
export class PerfHudService extends Effect.Service()('@minecraft/infrastructure/perf/PerfHudService', {
    ...
    scoped: Effect.gen(function* () {
        ...
        if (!enabled) {
            const noop = {
                recordFrame: (_dtSecs) => Effect.void,
                setWorkerQueueDepth: (_n) => Effect.void,
                setChunkCount: (_n) => Effect.void,
                setDrawCalls: (_n) => Effect.void,
            };
            return noop;
        }
        // -------------------------------------------------------------------
        ...
        // ---------------------------------------------------------------
        const { textNodes } = yield* Effect.acquireRelease(Effect.sync(() => {
            ...
            const initialValues = ['0.0', '0.0ms', '0.0ms', '0', '0', '0'];
            const nodes = [];
            for (let i = 0; i < 6; i++) {
            ...
            overlay.remove();
            Reflect.deleteProperty(window, '__perfHud__');
        }));
        // ---------------------------------------------------------------
        ...
        const buildSnapshot = () => {
            ...
            const samples = Arr.makeBy(validLength, (i) => ring[i]);
            return {
        ...
        yield* Effect.sync(() => {
            Reflect.set(window, '__perfHud__', { snapshot: buildSnapshot });
        });
        // ---------------------------------------------------------------
        // Hot-path methods.
        // ---------------------------------------------------------------
        const recordFrame = (dtSecs) => Effect.sync(() => {
            if (!Number.isFinite(dtSecs) || dtSecs <= 0)
                return;
            ...
            if (nowMs - lastUpdate < DOM_UPDATE_INTERVAL_MS)
                return;
        ...
        const setWorkerQueueDepth = (n) => Effect.sync(() => {
            MutableRef.set(workerQueueDepthRef, n);
        });
        const setChunkCount = (n) => Effect.sync(() => {
            MutableRef.set(chunkCountRef, n);
        });
        const setDrawCalls = (n) => Effect.sync(() => {
...
export const installPerfHudCounters = (perfHud, chunkManager, queueDepthSource) => {
    if (!isPerfEnabled())
        return Effect.void;
    return Effect.forkDaemon(Effect.repeat(Effect.gen(function* () {
