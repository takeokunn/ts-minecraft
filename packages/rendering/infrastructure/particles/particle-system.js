...
import { getTileIndex, getTileUVs } from '../textures/block-texture-map';
...
const buildParticleGeometry = (atlasTileFraction) => {
    ...
    arr[0] = 0;
    arr[1] = atlasTileFraction;
    arr[2] = atlasTileFraction;
    arr[3] = atlasTileFraction;
    arr[4] = 0;
    arr[5] = 0;
    arr[6] = atlasTileFraction;
    arr[7] = 0;
    uv.needsUpdate = true;
    return geom;
};
// Uses top face tile (most visually representative). Unknown blockId → dirt (tile 0).
export const getParticleUvOffset = (blockId, faceDir = 'top') => {
    const tileIndex = getTileIndex(blockId, faceDir);
    const { u0, v0 } = getTileUVs(tileIndex);
    return { u: u0, v: v0 };
};
export class ParticleSystemService extends Effect.Service()('@minecraft/infrastructure/three/ParticleSystemService', {
    scoped: Effect.gen(function* () {
        ...
        material.onBeforeCompile = (shader) => {
            ...
            if (!shader.vertexShader.includes('#include <uv_pars_vertex>') ||
                ...
                throw new Error('particle-system: Three.js vertex shader tokens for uvOffset injection not found');
            }
        ...
        // ── Service API ────────────────────────────────────────────────────────
        const attach = (scene) => Effect.acquireRelease(Effect.sync(() => {
            scene.add(mesh);
        }), () => Effect.sync(() => {
            scene.remove(mesh);
        }));
        const spawnBurst = (x, y, z, uvU, uvV, count = 6) => Effect.sync(() => {
        ...
        const update = (dtSecs) => Effect.sync(() => {
        ...
        const getActiveCount = () => Effect.sync(() => MutableRef.get(activeCountRef));
        return {
            attach,
            spawnBurst,
            update,
...
//# sourceMappingURL=../../../../dist/packages/rendering/infrastructure/particles/particle-system.js.map