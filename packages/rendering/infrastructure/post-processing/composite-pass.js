...
`;
const buildDefines = (flags) => {
    const defines = {};
    if (flags.bloom)
        defines['BLOOM_ENABLED'] = '';
    if (flags.godRays)
        defines['GODRAYS_ENABLED'] = '';
    if (flags.bokeh)
        defines['BOKEH_ENABLED'] = '';
    return defines;
};
const buildUniforms = (flags, resolution, inputs) => {
    const uniforms = {
        tDiffuse: { value: null },
...
export class CompositePass extends Pass {
    ...
    material;
    fsQuad;
    constructor(width, height, flags, inputs = {}) {
        super();
        this.flags = flags;
...
export const compositeFlagsAnyEnabled = (flags) => flags.bloom || flags.godRays || flags.bokeh;
//# sourceMappingURL=../../../../dist/packages/rendering/infrastructure/post-processing/composite-pass.js.map