...
const snapInterval = (min, len, step) => {
    ...
    return snapMin === snapMax ? [snapMin, snapMin + step] : [snapMin, snapMax];
};
 ...
 */
export const simplifyMesh = (meshed, lodLevel) => {
    if (lodLevel === 0 || meshed.indices.length === 0)
        return meshed;
    ...
    for (let q = 0; q < quadCount; q += 1) {
        ...
        const pickX = (vx) => (vx === box.minX ? p0x : p2x);
        const pickY = (vy) => (vy === box.minY ? p0y : p2y);
        const pickZ = (vz) => (vz === box.minZ ? p0z : p2z);
        ...
        positions[o3 + 8] = nv2z;
        positions[o3 + 9] = nv3x;
        positions[o3 + 10] = nv3y;
        positions[o3 + 11] = nv3z;
 ...
 */
export const lodForDistance = (distanceChunks) => {
    if (distanceChunks < LOD1_DISTANCE_CHUNKS)
        return 0;
    if (distanceChunks < LOD2_DISTANCE_CHUNKS)
        return 1;
    return 2;
};
//# sourceMappingURL=../../../../dist/packages/rendering/infrastructure/meshing/lod-simplification.js.map