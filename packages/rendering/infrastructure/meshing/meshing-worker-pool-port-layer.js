...
export const MeshingWorkerPoolPortLayer = Layer.effect(MeshingWorkerPoolPort, Effect.map(MeshingWorkerPool, (pool) => {
    return MeshingWorkerPoolPort.of({
        _tag: '@minecraft/application/rendering/MeshingWorkerPoolPort',
        meshChunk: (chunk, options) => pool.meshChunk(chunk, options),
    });
})).pipe(Layer.provide(MeshingWorkerPool.Default));
//# sourceMappingURL=../../../../dist/packages/rendering/infrastructure/meshing/meshing-worker-pool-port-layer.js.map