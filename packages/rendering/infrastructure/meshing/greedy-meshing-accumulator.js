...
import { getTileIndex } from '../textures/block-texture-map';
...
const AO_COLOR_BY_LEVEL = [255, 204, 153, 102];
...
export const ensureCapacity = (acc, additionalQuads) => {
...
export const addQuad = (acc, v0, v1, v2, v3, normal, blockId, ao, skyLight, blockLight, faceDir, du, dv) => {
    ensureCapacity(acc, 1);
    const base = acc.vertexCount;
    const pi = base * 3;
    // Positions — 4 vertices × 3 components
    acc.positions[pi] = v0[0];
    acc.positions[pi + 1] = v0[1];
    acc.positions[pi + 2] = v0[2];
    acc.positions[pi + 3] = v1[0];
    acc.positions[pi + 4] = v1[1];
    acc.positions[pi + 5] = v1[2];
    acc.positions[pi + 6] = v2[0];
    acc.positions[pi + 7] = v2[1];
    acc.positions[pi + 8] = v2[2];
    acc.positions[pi + 9] = v3[0];
    acc.positions[pi + 10] = v3[1];
    acc.positions[pi + 11] = v3[2];
    // Normals — same normal for all 4 vertices
    acc.normals[pi] = normal[0];
    acc.normals[pi + 1] = normal[1];
    acc.normals[pi + 2] = normal[2];
    acc.normals[pi + 3] = normal[0];
    acc.normals[pi + 4] = normal[1];
    acc.normals[pi + 5] = normal[2];
    acc.normals[pi + 6] = normal[0];
    acc.normals[pi + 7] = normal[1];
    acc.normals[pi + 8] = normal[2];
    acc.normals[pi + 9] = normal[0];
    acc.normals[pi + 10] = normal[1];
    acc.normals[pi + 11] = normal[2];
    ...
    acc.colors[pi] = aoR0;
    acc.colors[pi + 1] = skG0;
    acc.colors[pi + 2] = blB0;
    acc.colors[pi + 3] = aoR1;
    acc.colors[pi + 4] = skG1;
    acc.colors[pi + 5] = blB1;
    acc.colors[pi + 6] = aoR2;
    acc.colors[pi + 7] = skG2;
    acc.colors[pi + 8] = blB2;
    acc.colors[pi + 9] = aoR3;
    acc.colors[pi + 10] = skG3;
    acc.colors[pi + 11] = blB3;
    ...
    acc.indices[ii] = base;
    acc.indices[ii + 1] = base + 1;
    acc.indices[ii + 2] = base + 2;
    acc.indices[ii + 3] = base;
    acc.indices[ii + 4] = base + 2;
    acc.indices[ii + 5] = base + 3;
    acc.vertexCount += 4;
    acc.indexCount += 6;
};
//# sourceMappingURL=../../../../dist/packages/rendering/infrastructure/meshing/greedy-meshing-accumulator.js.map