...
export const getBlock = (blocks, lx, y, lz) => {
    if (lx < 0 || lx >= CHUNK_SIZE || y < 0 || y >= CHUNK_HEIGHT || lz < 0 || lz >= CHUNK_SIZE)
        return AIR;
    return blocks[y + lz * CHUNK_HEIGHT + lx * CHUNK_HEIGHT * CHUNK_SIZE];
};
export const isAir = (blocks, lx, y, lz) => getBlock(blocks, lx, y, lz) === AIR;
export const aoXPos = (blocks, lx, y, lz) => {
    if (lx >= CHUNK_SIZE - 1)
        return 0;
    const xOffset = (lx + 1) * CHUNK_HEIGHT * CHUNK_SIZE;
    const zOffset = lz * CHUNK_HEIGHT;
    let count = 0;
    if (y > 0 && blocks[y - 1 + zOffset + xOffset] !== AIR)
        count++;
    if (y < CHUNK_HEIGHT - 1 && blocks[y + 1 + zOffset + xOffset] !== AIR)
        count++;
    if (lz > 0 && blocks[y + (lz - 1) * CHUNK_HEIGHT + xOffset] !== AIR)
        count++;
    if (lz < CHUNK_SIZE - 1 && blocks[y + (lz + 1) * CHUNK_HEIGHT + xOffset] !== AIR)
        count++;
    return count > 3 ? 3 : count;
};
export const aoXNeg = (blocks, lx, y, lz) => {
    if (lx <= 0)
        return 0;
    const xOffset = (lx - 1) * CHUNK_HEIGHT * CHUNK_SIZE;
    const zOffset = lz * CHUNK_HEIGHT;
...
export const aoYPos = (blocks, lx, y, lz) => {
    if (y >= CHUNK_HEIGHT - 1)
        return 0;
    const yOffset = y + 1;
    ...
    const zOffset = lz * CHUNK_HEIGHT;
    let count = 0;
    if (lx + 1 < CHUNK_SIZE && blocks[yOffset + zOffset + (lx + 1) * xBase] !== AIR)
        count++;
    if (lx > 0 && blocks[yOffset + zOffset + (lx - 1) * xBase] !== AIR)
        count++;
    if (lz + 1 < CHUNK_SIZE && blocks[yOffset + (lz + 1) * CHUNK_HEIGHT + lx * xBase] !== AIR)
        count++;
    if (lz > 0 && blocks[yOffset + (lz - 1) * CHUNK_HEIGHT + lx * xBase] !== AIR)
        count++;
    return count > 3 ? 3 : count;
};
export const aoYNeg = (blocks, lx, y, lz) => {
    if (y <= 0)
        return 0;
    const yOffset = y - 1;
    const xBase = CHUNK_HEIGHT * CHUNK_SIZE;
...
export const aoZPos = (blocks, lx, y, lz) => {
    if (lz >= CHUNK_SIZE - 1)
        return 0;
    const zOffset = (lz + 1) * CHUNK_HEIGHT;
    const xBase = CHUNK_HEIGHT * CHUNK_SIZE;
    let count = 0;
    if (lx + 1 < CHUNK_SIZE && blocks[y + zOffset + (lx + 1) * xBase] !== AIR)
        count++;
    if (lx > 0 && blocks[y + zOffset + (lx - 1) * xBase] !== AIR)
        count++;
    if (y + 1 < CHUNK_HEIGHT && blocks[y + 1 + zOffset + lx * xBase] !== AIR)
        count++;
    if (y > 0 && blocks[y - 1 + zOffset + lx * xBase] !== AIR)
        count++;
    return count > 3 ? 3 : count;
};
export const aoZNeg = (blocks, lx, y, lz) => {
    if (lz <= 0)
        return 0;
    const zOffset = (lz - 1) * CHUNK_HEIGHT;
    const xBase = CHUNK_HEIGHT * CHUNK_SIZE;
...
export const sampleVoxelLight = (grids, lx, y, lz) => {
    /* c8 ignore next */
    if (grids === undefined)
...
export const sampleCornerLight = (grids, airX, airY, airZ, t1x, t1y, t1z, t2x, t2y, t2z, du, dv) => {
    if (grids === undefined)
        return 0xf0; /* sky=15,block=0 */
...
//# sourceMappingURL=../../../../dist/packages/rendering/infrastructure/meshing/greedy-meshing-ao.js.map