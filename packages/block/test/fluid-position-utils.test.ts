...
describe('positionFromChunk', () => {
  ...
  it('idx=0 maps to {x: chunkX*16, y:0, z: chunkZ*16}', () => {
    ...
    const pos = positionFromChunk(chunk.coord,0)
  ...
  it('idx=1 maps to y=1 (y is innermost dimension)', () => {
    ...
    const pos = positionFromChunk(chunk.coord,1)
    expect(pos.y).toBe(1)
    expect(pos.x).toBe(0)
    expect(pos.z).toBe(0)
  })
  ...
  it('idx=CHUNK_HEIGHT maps to z=1 (z is second dimension)', () => {
    const chunk = makeChunk(0, 0)
    const pos = positionFromChunk(chunk.coord,CHUNK_HEIGHT)
    expect(pos.z).toBe(1)
    expect(pos.y).toBe(0)
    expect(pos.x).toBe(0)
  })
  ...
  it('positionFromChunk round-trip: position encodes back to the same idx', () => {
    ...
    const pos = positionFromChunk(chunk.coord,testIdx)
