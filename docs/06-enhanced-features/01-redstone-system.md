...
const CircuitAnalyzer = Context.GenericTag<CircuitAnalyzerInterface>("@app/CircuitAnalyzer")
```
...
export const SignalPropagationEngineLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const propagateSignal = (
      ...
      // Phase 1: Process scheduled updates
      const updatedNetwork = yield* processScheduledUpdates(network, tick)

      // Phase 2: Calculate new signal strengths
      const powerSources = yield* CircuitAnalyzer.pipe(
        Effect.flatMap(analyzer => analyzer.findPowerSources(updatedNetwork))
      )

      // Phase 3: Breadth-first signal propagation
      let currentNetwork = updatedNetwork
      ...
      while (queue.length > 0) {
        ...
        const component = currentNetwork.components.get(componentId)
        if (!component) continue

        const neighbors = currentNetwork.connections.get(componentId) ?? []
        const inputSignals = yield* Effect.forEach(
          neighbors,
          (neighborId) => getSignalStrength(currentNetwork, neighborId),
          { concurrency: "unbounded" }
        )

        const outputSignal = yield* calculateComponentOutput(component, inputSignals)

        // Update component with new signal
        const updatedComponent = updateComponentSignal(component, outputSignal)
        currentNetwork = {
          ...currentNetwork,
          components: currentNetwork.components.set(componentId, updatedComponent)
        }

        // Schedule updates for connected components
    ...
    const calculateComponentOutput = (
      ...
      return yield* Match.value(component).pipe(
        Match.tag("RedstoneDust", () =>
          Effect.succeed(Math.max(0, Math.max(...inputs) - 1) as RedstoneSignalStrength)
        ),
        Match.tag("RedstoneRepeater", () => {
