# Block System - ブロック管理システム

## 概要

Block Systemは、TypeScript Minecraft cloneの基本要素であるブロックの完全な管理システムです。Effect-TS 3.17+の最新パターン（Schema.Struct、@app/ServiceNameネームスペース）とDDDの値オブジェクトパターンを活用し、400+種類のブロックを純粋関数型で管理します。
  ...
  const registry = yield* Ref.make(new Map<BlockId, BlockDefinition>())
    ...
    const register = (block: BlockDefinition) =>
      Effect.gen(function* () {
        ...
        if (current.has(block.id)) {
          return yield* Effect.fail(new BlockAlreadyRegisteredError(block.id))
        }

        yield* Ref.update(registry, (map) => new Map(map).set(block.id, block))
      })
    ...
    const getByMaterial = (material: string) =>
      Effect.gen(function* () {
        ...
        return all.filter(block => block.material === material)
      })

    const getByProperty = (predicate: (props: BlockProperties) => boolean) =>
      Effect.gen(function* () {
        const all = yield* getAll()
        return all.filter(block => predicate(block.properties))
      })
        ...
        Effect.gen(function* () {
          ...
          yield* processUpdate(update).pipe(
            Effect.catchAll((error) =>
              Effect.log(`Block update error: ${error}`)
            )
          )
        })
      )
    ...
    const processUpdate = (update: BlockUpdate) =>
      Effect.gen(function* () {
        ...
        yield* pipe(
          update,
          Match.value,
          Match.tag("neighbor_changed", (update) =>
            handleNeighborChange(update.position, update.source!)
          ),
          Match.tag("random_tick", (update) =>
            handleRandomTick(update.position, definition)
          ),
          Match.tag("scheduled_tick", (update) =>
            handleScheduledTick(update.position, definition)
          ),
          Match.tag("block_event", (update) =>
            handleBlockEvent(update.position, update.data)
          ),
          Match.exhaustive
        )
      })
        ...
        yield* pipe(
          ...
          Match.when(
            ...
            () => Effect.gen(function* () {
              ...
              yield* physics.addFallingBlock(position, block)
              yield* world.setBlock(position, "minecraft:air" as BlockId)
            })
          ),
          Match.when(
            (id) => isRedstoneComponent(id),
