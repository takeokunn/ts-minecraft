...
const findHardBlocks = Effect.gen(function* () {
  ...
  // 硬度が2.0以上のブロックを取得
  const hardBlocks = yield* blockRegistry.getByProperty(
    props => props.hardness >= 2.0
  )
...
// 条件付きブロック配置
const placeBlockWithValidation = (
  position: WorldPosition,
  blockId: BlockId,
  player: PlayerId
) =>
...
const breakBlock = (
  ...
  Effect.gen(function* () {
    ...
    const drops = yield* interactionService.onBlockBreak(
      position,
      player,
      tool
    )
...
const processRandomTick = (position: WorldPosition) =>
  Effect.gen(function* () {
    ...
    yield* Match.value(block.id).pipe(
      Match.when("minecraft:grass_block", () =>
        processGrassSpread(position)
      ),
      Match.when("minecraft:farmland", () =>
        processWaterAbsorption(position)
      ),
      Match.when("minecraft:ice", () =>
        processIceMelting(position)
      ),
      Match.when(isCrop, () =>
        processCropGrowth(position)
      ),
...
const handleNeighborChange = (
  ...
  Effect.gen(function* () {
    ...
    yield* Match.value(block.id).pipe(
      Match.when(isGravityAffected, () =>
        checkGravitySupport(position)
      ),
      Match.when(isRedstoneComponent, () =>
        updateRedstoneNetwork(position)
      ),
      Match.when("minecraft:water", () =>
        updateWaterFlow(position, source)
      ),
      Match.when("minecraft:torch", () =>
        checkTorchSupport(position)
      ),
      Match.orElse(() => Effect.void)
    )
  })
  ...
  Effect.gen(function* () {
    ...
    for (const neighborPos of neighbors) {
      ...
      if (canWaterFlowTo(neighbor)) {
        ...
        if (targetLevel > 0) {
          yield* world.setBlock(
            neighborPos,
            "minecraft:water" as BlockId,
            { level: targetLevel }
          )
...
interface MaterialIntegration {
  readonly getMaterialProperties: (
    blockId: BlockId
  ) => Effect.Effect<MaterialProperties, never>
...
const processBlockCollision = (
  ...
  Effect.gen(function* () {
    ...
    const collision = yield* materials.calculateCollision(
      block.materialId,
      entityMaterial,
      velocity
    )
    ...
    if (collision.soundEffect) {
      yield* playSound(
        collision.soundEffect.type,
        position,
        collision.soundEffect.volume
      )
    }

    // パーティクル効果
    if (collision.particleEffect) {
      yield* spawnParticles(
        collision.particleEffect.material,
        position,
        collision.particleEffect.count
      )
    }
...
// 高性能ブロックアクセス
const getBlockOptimized = (
  storage: OptimizedBlockStorage,
  index: number
): BlockData => ({
...
const findLuminousBlocks = Effect.gen(function* () {
  ...
  const lightSources = yield* registry.getByProperty(
    props => props.luminance > 0
  )
...
const buildRedstoneCircuit = Effect.gen(function* () {
  ...
  yield* placeBlock(
    { x: 0, y: 64, z: 0 },
    "minecraft:redstone_torch" as BlockId,
    player
  )
  ...
  for (let i = 1; i <= 5; i++) {
    yield* placeBlock(
      { x: i, y: 64, z: 0 },
      "minecraft:redstone_wire" as BlockId,
      player
    )
  }
...
```typescript
// 包括的なエラーハンドリング
const safeBlockOperation = (
  operation: Effect.Effect<void, BlockError>
) =>
  Effect.gen(function* () {
    yield* operation.pipe(
      Effect.catchTag("BlockNotFoundError", (error) =>
        Effect.log(`ブロックが見つかりません: ${error.blockId}`)
      ),
      Effect.catchTag("PlaceError", (error) =>
        Effect.log(`配置エラー: ${error.message}`)
      ),
      Effect.catchTag("BreakError", (error) =>
        Effect.log(`破壊エラー: ${error.message}`)
      ),
      Effect.catchTag("PhysicsError", (error) =>
        Effect.log(`物理エラー: ${error.message}`)
      ),
      Effect.catchAllDefect((defect) =>
        Effect.log(`予期しないエラー: ${defect}`)
      )
    )
  })

// リトライ機能付きブロック操作
const retryableBlockOperation = <A, E>(
