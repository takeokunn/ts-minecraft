...
- 弓でのキルにもメレーと同じ Mob XP orb が spawn されることを確認する（`DroppedXpOrbService.spawn`）
...
- 雨天時に Zombie の日光燃焼が `WeatherService.getWeather()` 由来の `daylightBurnProtected` で抑制されることを `entity-manager-internal-update.ts` と `entity-manager-update.test.ts` で確認する
...
- survival respawn 時に防具が world item 化され、その後 equipment が reset されることを確認する
...
- **クリエイティブモードで落下ダメージが無効か確認する**（`physics-stage-health.ts` が `processFallDamage` で落下状態更新を維持しつつ、Creative では適用ダメージを 0 にする）
...
| 22 | mineral block レシピ | 実装済み: COAL/IRON/GOLD/DIAMOND/REDSTONE/LAPIS/EMERALD の素材×9→ブロックと 1 ブロック→素材×9 を `MISC_RECIPES` に追加し、`misc-recipes.test.ts` で圧縮・逆変換を検証 | `packages/inventory/application/recipes/misc-recipes.ts`, `packages/inventory/test/misc-recipes.test.ts` |
| 23 | Q キー（アイテムドロップ入力）| **実装済み** — `DROP_ITEM='KeyQ'` を定義し、ゲームプレイ中の Q で選択ホットバーのアイテムを 1 個消費して `DroppedItemService` に world item を spawn する。frame stage で浮遊描画・pickup delay 後の自動回収へ接続済み | `packages/entity/domain/key-mappings.ts`, `packages/app/application/frame/stages/input-stage.ts`, `packages/app/application/frame/stages/entity-update-stage.ts`, `packages/rendering/infrastructure/entity/dropped-item-renderer.ts` |
...
| 25 | クリエイティブ落下ダメージ無効 | **実装済み** — `processFallDamage` による落下状態更新は継続しつつ、Creative では適用ダメージを 0 にして `healthService.applyDamage` と hurt 音を発火させない | `packages/app/application/frame/stages/physics-stage-health.ts`, `packages/app/application/frame/stages/physics-stage.test.ts` |
...
| 45 | 植物ブロック欠如 | TALL_GRASS / FERN は BlockType と基本挙動、通常破壊の WHEAT_SEEDS 1/8 抽選、SHEARS 回収に接続済み。SUGAR_CANE / CACTUS / LILY_PAD も BlockType / codec index 85-87 / 初期ブロック / 支え判定 / 水破壊（SUGAR_CANE・CACTUS）/ 非衝突（SUGAR_CANE・LILY_PAD）/ スポーン面除外 / 仮タイル描画に接続済み。SUGAR_CANE / CACTUS は最大3段の自動成長と砂浜・水辺・砂漠での自然生成に接続済み。草・花・LILY_PAD はバイオーム別の deterministic 自然生成に接続済み。草・花・キノコ・SUGAR_CANE はクロスモデル、CACTUS は 1/16 inset 柱、LILY_PAD は薄板モデルで描画される | `packages/core/domain/block-type.ts`, `packages/world/domain/block-support.ts`, `packages/world/domain/terrain/plant-placer.ts`, `packages/rendering/infrastructure/textures/block-texture-map.config.ts`, `packages/rendering/infrastructure/meshing/plant-mesh.ts` |
...
| 52 | 雨天 Zombie 日光保護 | **実装済み** — `WeatherService.getWeather()` を frame stage で `daylightBurnProtected` に変換し、雨/雷天候中の Zombie 日光ダメージを抑制 | `packages/app/application/frame/frame-stage-executor-helpers.ts`, `packages/entity/application/mob/entity-manager-internal-update.ts`, `packages/entity/test/mob/entity-manager-update.test.ts` |
...
| 56 | SLAB / STAIRS は基本定義済み、階段の向き/段差形状は未完成 | STONE_SLAB と OAK_STAIRS を BlockType / codec / block config / texture / item / hotbar / recipe に追加。STONE_SLAB は半ブロック衝突、OAK_STAIRS は現状向きなしフルブロック近似 | `packages/core/domain/block-type.ts`, `packages/game/domain/block-collision-predicates.ts`, `packages/inventory/application/recipes/misc-recipes.ts` |
| 57 | FLOWER / MUSHROOM ブロック実装済み | **実装済み** — DANDELION/POPPY/BROWN_MUSHROOM/RED_MUSHROOM を BlockType / codec index 79-82 / 初期ブロック / 非固体衝突 / 窒息除外 / スポーン面除外 / 支え判定 / 水破壊 / 自己ドロップ / 仮タイル描画 / クロスモデル描画に接続。DANDELION/POPPY は草と同じ地表植物生成で自然配置される。BROWN_MUSHROOM/RED_MUSHROOM は日陰の森林・湿地・タイガ・ジャングル地表生成と、光量12以下の設置制約に接続済み | `packages/core/domain/block-type.ts`, `packages/world/domain/block-support.ts`, `packages/world/domain/terrain/plant-placer.ts`, `packages/world/application/block-service-place-plan.ts`, `packages/rendering/infrastructure/textures/block-texture-map.config.ts`, `packages/rendering/infrastructure/meshing/plant-mesh.ts` |
| 58 | アイテム拾得距離定数（domain）| `ITEM_PICKUP_DISTANCE=1.5` / `ITEM_PICKUP_DISTANCE_SQUARED` / `DROPPED_ITEM_LIFETIME_TICKS=6000` と二乗距離 pickup 判定を追加。Q ドロップの浮遊 item entity 化は #80 の未実装差分として継続 | `packages/entity/domain/dropped-item.ts`, `packages/entity/test/dropped-item.test.ts` |
...
| 68 | ANVIL 基本ブロック実装済み / 修繕UI未実装 | `ANVIL` を BlockType / codec index 92 / 初期ブロック / 鉄ブロック仮タイル / ホットバー / ピッケル採掘 / `3 IRON_BLOCK + 4 IRON_INGOT -> ANVIL` レシピに対応。アイテム修繕・名前変更・耐久消費UIは未実装 | `packages/core/domain/block-type.ts`, `packages/core/domain/block-codec.ts`, `packages/block/domain/blocks.config.crafted.ts`, `packages/rendering/infrastructure/textures/block-texture-map.config.ts`, `packages/rendering/infrastructure/textures/item-texture-map.config.ts`, `packages/world/domain/harvestable-blocks.ts`, `packages/inventory/application/recipes/misc-recipes.ts` |
| 69 | CAULDRON 基本ブロック実装済み / 水保持未実装 | `CAULDRON` を BlockType / codec index 93 / 初期ブロック / 鉄ブロック仮タイル / ホットバー / ピッケル採掘 / `7 IRON_INGOT -> CAULDRON` レシピに対応。バケツでの水充填・瓶/ポーション保持・専用形状は未実装 | `packages/core/domain/block-type.ts`, `packages/core/domain/block-codec.ts`, `packages/block/domain/blocks.config.crafted.ts`, `packages/rendering/infrastructure/textures/block-texture-map.config.ts`, `packages/rendering/infrastructure/textures/item-texture-map.config.ts`, `packages/world/domain/harvestable-blocks.ts`, `packages/inventory/application/recipes/misc-recipes.ts` |
| 70 | PRESSURE_PLATE は基本ブロック＋踏下信号を実装済み | 感圧板（PRESSURE_PLATE）は BlockType / codec index 89 / 初期ブロック / 支持・水破壊 / 高さ 1/16 の薄い衝突形状 / 仮タイル描画 / 石 2 個レシピに対応。配置時に RedstoneComponent.PressurePlate へ登録され、プレイヤーまたは mob が上にいる間は隣接導体へ信号を出す。item/重量差の踏下検知は未実装 | `packages/core/domain/block-type.ts`, `packages/world/domain/block-support.ts`, `packages/game/domain/block-collision-predicates.ts`, `packages/inventory/application/recipes/misc-recipes.ts`, `packages/entity/application/redstone/redstone-service.ts`, `packages/app/application/frame/stages/entity-update-stage.ts` |
| 71 | CHICKEN Mob 実装済み | `ChickenDefinition` を追加し、EntityType・PASSIVE_MOBS・昼間スポーン対象・FEATHER/RAW_CHICKEN ドロップ・WHEAT_SEEDS 繁殖・RAW_CHICKEN→COOKED_CHICKEN 精錬・翼付き二足 Mob 描画に対応 | `packages/entity/domain/mob/mobs/chicken.ts`, `packages/entity/domain/mob/mob-categories.ts`, `packages/inventory/application/recipes/misc-recipes.ts`, `packages/rendering/infrastructure/entity/mob-geometry.ts` |
| 72 | 重力ブロック落下を実装済み / エンティティ化なし | loaded chunk 内の SAND・GRAVEL は直下が AIR の場合、world maintenance で 1 tick あたり最大 32 個を 1 ブロック下へ移動する。公式の falling_block エンティティ化・落下アニメーション・着地イベントは未実装 | `packages/world/domain/falling-block.ts`, `packages/world/application/falling-block-maintenance.ts` |
| 73 | 溶岩脱出後の炎上実装済み | LAVA 接触中に炎上残時間を付与し、脱出後も `FIRE_DAMAGE_INTERVAL_SECS` ごとに Fire Protection 軽減込みの `FIRE_DAMAGE` を適用 | `packages/app/application/frame/stages/physics-stage-survival/environment-hazards.ts`, `packages/entity/domain/environment-hazard.ts` |
| 74 | ENDER_PEARL 右クリックテレポート実装済み / 投射物なし | ENDER_PEARL は右クリックで視線方向へ即時テレポートし、Survival では選択ホットバースロットから 1 個消費、Creative では消費しない。投射物エンティティ・飛翔時間・落下地点衝突は未実装 | `packages/app/application/frame/stages/interaction-item-use-handler/ender-pearl.ts`, `packages/app/application/frame/stages/interaction-ender-pearl-handler.test.ts` |
...
| 78 | FIRE 基本ブロック実装済み / 延焼未実装 | FIRE を BlockType/codec/config/light/texture に追加し、FLINT_AND_STEEL で隣接 AIR に着火可能。延焼・自然消火・専用炎テクスチャは未実装 | `packages/core/domain/block-type.ts`, `packages/app/application/frame/stages/interaction-flint-steel-fire.ts` |
...
| 80 | アイテムドロップ（Qキー）| Q 入力は選択スロットから 1 個を減らし、`DroppedItemService` に world item を spawn する。`DroppedItemRendererService` で浮遊描画し、frame stage で tick/collectWithin/inventory add を実行する。インベントリ満杯時は pickup delay 付きで再スポーンする。ブロック破壊ドロップ、モブ死亡時のアイテムドロップ、死亡時インベントリ/装備散布も同じ world item 経路に接続済み。XP orb runtime/source spawn は §12 #279/#280 で別管理 | `packages/entity/domain/key-mappings.ts`, `packages/entity/application/dropped-item-service.ts`, `packages/rendering/infrastructure/entity/dropped-item-renderer.ts`, `packages/app/application/frame/stages/input-stage.ts`, `packages/app/application/frame/stages/entity-update-stage.ts`, `packages/app/application/frame/stages/interaction-break-handler.ts`, `packages/app/application/frame/stages/interaction-mob-drops.ts`, `packages/app/application/frame/stages/interaction-melee-handler.ts`, `packages/app/application/frame/stages/interaction-bow-handler.ts`, `packages/game/application/game-state-service.ts` |
...
| 90 | モブアイテム/XP ドロップは world entity 化済み | 近接・弓でのモブ撃破時アイテムは `interaction-mob-drops.ts` 経由で `DroppedItemService.spawn()` へ流し、frame の自動 pickup 経路で回収する。XP は `spawnMobXpOrb` から `DroppedXpOrbService.spawn()` へ流し、回収時に Mending 修理を先に通して残 XP を `XPService` に加算する | `packages/app/application/frame/stages/interaction-mob-drops.ts`, `packages/app/application/frame/stages/interaction-melee-handler.ts`, `packages/app/application/frame/stages/interaction-bow-handler.ts`, `packages/app/application/frame/stages/entity-update-stage.ts` |
...
| 93 | 火打石と鋼の一般着火は実装済み / 延焼なし | FLINT_AND_STEEL はネザーポータル点火に加え、隣接 AIR への FIRE 生成に対応済み。延焼・自然消火・専用炎テクスチャは未実装 | `packages/app/application/frame/stages/interaction-flint-steel-handler.ts`, `packages/app/application/frame/stages/interaction-flint-steel-fire.ts` |
| 94 | 落雷ダメージ | `thunder` かつ空に露出したプレイヤーへ周期落雷ダメージを適用し、屋根下では抑止。`weather-service.ts` は状態遷移のみで、survival 環境ハザード側が処理 | `packages/app/application/frame/stages/physics-stage-survival/environment.test.ts`, `packages/app/application/frame/stages/physics-stage-survival/environment-hazards.test.ts`, `packages/entity/domain/environment-hazard.ts` |
...
| 102 | コンパレータ・オブザーバー・レッドストーンランプ未実装 | `block-type.ts` のレッドストーン関連は REDSTONE_WIRE/TORCH/LEVER/STONE_BUTTON/REPEATER/PRESSURE_PLATE の 6 種のみ。COMPARATOR・OBSERVER・REDSTONE_LAMP なし | `packages/core/domain/block-type.ts:53` |
...
| 105 | インベントリ右クリック半スタック実装済み | スロット右クリックで空カーソル時にスタック半量（奇数は切り上げ）を持ち上げる。左クリックの全スタック移動と同じカーソル状態に統合済み | `packages/presentation/inventory/inventory-cursor-click.ts`, `packages/presentation/inventory/inventory-renderer-click-handler.ts`, `packages/presentation/inventory/inventory-cursor-click.test.ts` |
| 106 | インベントリ右クリック 1 個置き実装済み | 保持中スタックをスロット右クリックすると空スロットへ 1 個配置、同種スタックには最大数まで 1 個加算する。異種スロットでは何もしない | `packages/presentation/inventory/inventory-cursor-click.ts`, `packages/presentation/inventory/inventory-renderer-click-handler.ts`, `packages/presentation/inventory/inventory-cursor-click.test.ts` |
...
| 108 | SUGAR_CANE・CACTUS 基本ブロック実装済み | **一部実装済み** — SUGAR_CANE は砂/土/草/自身の上かつ隣接水あり、CACTUS は砂/自身の上かつ横 4 方向が空気のとき配置可能。BlockType / codec / 初期ブロック / 支え判定 / 水破壊 / 仮タイル描画 / サボテン接触ダメージ / 1/16 inset の細い当たり判定 / 最大3段の自動成長 / 砂浜・水辺・砂漠での自然生成に接続済み | `packages/world/application/block-service-place-plan.ts`, `packages/world/application/crop-growth-maintenance.ts`, `packages/world/domain/plant-growth.ts`, `packages/world/domain/terrain/plant-placer.ts`, `packages/world/test/block-service.test.ts`, `packages/world/test/plant-growth.test.ts`, `packages/world/test/place-chunk-plants.test.ts`, `packages/app/application/frame/stages/physics-stage-survival/environment.ts`, `packages/app/application/frame/stages/physics-stage-survival/environment-hazards.ts`, `packages/game/domain/aabb-collision.ts` |
...
| 122 | ステップ高 0.6 ブロック対応済み | `aabb-collision.ts` の `MAX_STEP_UP = 0.6`。0.6 ブロックの side overlap は昇段し、0.6 超の side overlap は接地扱いしない境界テストで検証済み | `packages/game/domain/aabb-collision.ts`, `packages/game/test/aabb-collision.test.ts` |
...
| 130 | シフトクリックで防具を自動装備 | インベントリの shift-click は近接チェストへの quick move を優先し、チェスト非選択時は防具アイテムを空の対応装備スロットへ 1 個だけ装備する。対象スロットが埋まっている場合や非防具は従来の hotbar/main inventory quick move にフォールバック | `packages/presentation/inventory/inventory-renderer-click-handler.ts:25`, `packages/inventory/application/equipment-service-state.ts:33`, `packages/presentation/inventory/inventory-renderer-click-handler.test.ts:52` |
...
| 140 | 死亡時インベントリ/装備アイテムの地面ドロップ実装済み | survival の `respawn()` は死亡地点の所持スロットと装備スロットを `DroppedItemService` でワールドアイテム化してから inventory/equipment を clear。creative respawn は所持品と装備を保持 | `packages/game/application/game-state-service.ts:102`, `packages/app/application/frame/stages/physics-stage-health/death.ts` |
| 141 | 無限水源を実装済み | 非水源の水セルは、下が水で置換可能でない場合に水平隣接 2 個以上の水源を検出すると `level=0/source=true` に更新。支持なしの水は水源化せず落下を継続 | `packages/world/application/fluid-service-flow.ts:110`, `packages/world/test/fluid-service-tick.test.ts:228` |
...
| 167 | ブロック破壊ドロップの満杯消失は解消済み | Survival のブロック破壊は `breakBlock(..., { dropItems: false })` で world 層の直接インベントリ加算を抑止し、通常ドロップ・Silk Touch・草種・作物・葉ボーナス・Fortune 追加分を `DroppedItemService.spawn()` へ流す。満杯時も frame pickup 経路で再スポーンされ、地面の world item として残る | `packages/app/application/frame/stages/interaction-break-handler.ts`, `packages/app/application/frame/stages/entity-update-stage.ts`, `packages/entity/application/dropped-item-service.ts` |
...
| 169 | エンチャント台のクラフトレシピ実装済み | `BOOK` 1 + `DIAMOND` 2 + `OBSIDIAN` 4 から `ENCHANTING_TABLE` 1 を作成。公式の素材数と一致（ただしレシピは無形式）| `packages/inventory/application/recipes/misc-recipes.ts`, `packages/inventory/test/misc-recipes.test.ts` |
| 170 | ブラウザキーショートカット抑止実装済み | **実装済み** — gameplay 中の `keydown` で Space/矢印/PageUp/PageDown に加え F1/F11/Ctrl+W/Meta+W を `preventDefault()` する。入力欄/textarea/select/contentEditable 中は従来通り抑止しない | `packages/presentation/input/input-service.ts`, `packages/presentation/input/input-service.test.ts` |
...
| 213 | エンダーパールの即時テレポート実装済み / 投射物なし | ENDER_PEARL は右クリックでカメラ forward と照準距離からテレポート先を計算し、`GameStateService.setPlayerPosition` で物理 body とプレイヤー位置を同期する。公式の投射物飛翔・落下地点衝突・着弾ダメージは未実装 | `packages/app/application/frame/stages/interaction-item-use-handler/ender-pearl.ts` |
| 214 | ポーション/醸造システムなし | POTION アイテム・BREWING_STAND ブロックともに未定義。ポーション効果（速度・再生・毒など）のシステムなし | `packages/core/domain/` |
| 215 | BOOK アイテム実装済み / 書き込み本なし | `BOOK` はエンチャント台レシピ素材として定義済み。BOOK_AND_QUILL/WRITTEN_BOOK とテキスト編集 UI は未実装 | `packages/core/domain/item-type.ts`, `packages/core/domain/item-type.test.ts` |
...
| 232 | サトウキビ・サボテン基本ブロック実装済み | SUGAR_CANE・CACTUS は BlockType / codec / 初期ブロック / 支え判定 / 水破壊 / 仮タイル描画 / サボテン接触ダメージ / 1/16 inset の細い当たり判定 / 最大3段の自動成長 / 砂浜・水辺・砂漠での自然生成に接続済み | `packages/core/domain/block-type.ts`, `packages/world/domain/block-support.ts`, `packages/world/domain/plant-growth.ts`, `packages/world/domain/terrain/plant-placer.ts`, `packages/world/test/place-chunk-plants.test.ts`, `packages/world/application/crop-growth-maintenance.ts`, `packages/rendering/infrastructure/textures/block-texture-map.config.ts`, `packages/game/domain/aabb-collision.ts` |
...
| 242 | 花・草・水辺装飾ブロック一部実装済み | DANDELION/POPPY/TALL_GRASS/FERN/SUGAR_CANE/CACTUS/LILY_PAD は BlockType/codec/初期ブロック/支え判定/スポーン面除外/仮タイル描画に接続済み。草の破壊時の WHEAT_SEEDS 1/8 抽選と SHEARS 回収は実装済み。SUGAR_CANE・LILY_PAD は非衝突、CACTUS は 1/16 inset の細い当たり判定に接続済み。SUGAR_CANE/CACTUS/草/花/LILY_PAD の自然生成は実装済み。DANDELION/POPPY/TALL_GRASS/FERN/SUGAR_CANE はクロスモデル、CACTUS は 1/16 inset 柱、LILY_PAD は薄板モデルで描画される。CHORUS_FLOWER は End 専用として既存 | `packages/core/domain/block-type.ts`, `packages/world/domain/block-support.ts`, `packages/world/domain/terrain/plant-placer.ts`, `packages/world/test/place-chunk-plants.test.ts`, `packages/game/domain/aabb-collision.ts`, `packages/rendering/infrastructure/meshing/plant-mesh.ts` |
| 243 | 汎用ハーフブロックは最小対応 | STONE_SLAB を定義し、石テクスチャ・レシピ・半ブロック衝突を追加。OAK_SLAB 等の追加バリエーションは未対応 | `packages/core/domain/block-type.ts`, `packages/game/domain/aabb-collision.ts` |
...
| 247 | 感圧板の信号メカニクス一部実装済み | PRESSURE_PLATE は基本ブロックとして配置時に RedstoneComponent.PressurePlate へ登録され、プレイヤーまたは mob がブロック上にいる間 active になり隣接導体へ信号を出す。STONE_PRESSURE_PLATE/WOODEN_PRESSURE_PLATE の分離、item/重量差の踏下検知は未実装 | `packages/entity/domain/redstone/redstone-model.ts`, `packages/entity/domain/redstone/redstone-simulation.ts`, `packages/entity/application/redstone/redstone-service.ts`, `packages/app/application/frame/stages/entity-update-stage.ts`, `packages/app/application/frame/stages/interaction-placement-handler.ts` |
| 248 | 炎（FIRE）基本ブロック実装済み / 延焼なし | FIRE は `block-type.ts` に定義済みで、FLINT_AND_STEEL による隣接 AIR 着火に対応。ブロック延焼・木材への自動着火・自然消火は未実装 | `packages/core/domain/block-type.ts`, `packages/app/application/frame/stages/interaction-flint-steel-fire.ts` |
...
| 250 | 金床（ANVIL）は基本ブロックのみ実装済み | ANVIL は `block-type.ts` に定義済み。ツール修繕・アイテムリネーム・耐久消費 UI は未実装 | `packages/core/domain/block-type.ts`, `packages/inventory/application/recipes/misc-recipes.ts` |
| 251 | 大釜（CAULDRON）は基本ブロックのみ実装済み | CAULDRON は `block-type.ts` に定義済み。バケツで水を充填・瓶/ポーション保持・専用形状は未実装 | `packages/core/domain/block-type.ts`, `packages/inventory/application/recipes/misc-recipes.ts` |
...
| 279 | XP オーブランタイムと主要発生元接続・描画は実装済み | `DroppedXpOrbService` が XP orb の spawn・tick・pickup delay・寿命・吸引移動・距離回収を管理し、frame 回収時は mending 修理を先に通して残 XP を `XPService` に加算する。近接・弓でのモブ撃破 XP と鉱石ブロック XP は orb spawn 経路に接続済み。繁殖報酬は直接 mending/XP 加算経路のまま。`DroppedXpOrbRendererService` が回収後の live orb を frame stage から同期し、浮遊発光表現を表示する | `packages/entity/application/dropped-xp-orb-service.ts`, `packages/entity/domain/dropped-xp-orb.ts`, `packages/app/application/frame/stages/entity-update-stage.ts`, `packages/rendering/infrastructure/entity/dropped-xp-orb-renderer.ts`, `packages/entity/test/dropped-xp-orb-service.test.ts`, `packages/rendering/test/dropped-xp-orb-renderer.test.ts` |
| 280 | 浮遊アイテムエンティティと XP orb ランタイム/描画は実装済み | Q ドロップ、Survival ブロック破壊ドロップ、近接・弓でのモブ死亡時アイテムドロップ、死亡時インベントリ/装備散布は `DroppedItemService` + `DroppedItemRendererService` + frame auto pickup に接続済み。ブロック破壊では直接インベントリ加算を抑止し、通常ドロップ・Silk Touch・草種・作物・葉ボーナス・Fortune 追加分を world item として spawn する。XP orb は runtime/pickup/mending 経路、プレイヤー吸引、モブ/鉱石 XP 発生元 spawn、`DroppedXpOrbRendererService` による浮遊発光描画まで接続済み | `packages/app/application/frame/stages/interaction-break-handler.ts`, `packages/app/application/frame/stages/interaction-mob-drops.ts`, `packages/app/application/frame/stages/interaction-melee-handler.ts`, `packages/app/application/frame/stages/interaction-bow-handler.ts`, `packages/app/application/frame/stages/entity-update-stage.ts`, `packages/rendering/infrastructure/entity/dropped-item-renderer.ts`, `packages/rendering/infrastructure/entity/dropped-xp-orb-renderer.ts`, `packages/entity/application/dropped-xp-orb-service.ts`, `packages/game/application/game-state-service.ts` |
| 281 | 死亡時インベントリ/装備ドロップ実装済み | サバイバル `respawn()` 時に死亡地点のインベントリ占有スロットと防具スロットを pickup delay 付き world item として散布し、その後 inventory/equipment を reset する。死亡ステージでは装備を先に reset せず、respawn 経路がドロップを materialize できる状態を保つ | `packages/game/application/game-state-service.ts`, `packages/app/application/frame/stages/physics-stage-health/death.ts` |
...
| 291 | アイテムホバーツールチップ実装済み | インベントリ/チェスト内アイテムの slot title/data-tooltip/aria-label に表示名・個数・エンチャント一覧を反映し、空スロットでは stale tooltip を消去 | `packages/presentation/inventory/inventory-renderer-refresh.ts`, `packages/presentation/inventory/inventory-renderer-refresh.test.ts` |
...
| 295 | 1.15〜1.20 新規モブ未実装 | BEE は passive Mob として実装済み。AXOLOTL・FOX・GOAT・FROG・TADPOLE・ALLAY・WARDEN・CAMEL 等は EntityType 未定義で、水中/村/レイド/騎乗系の新規 Mob 群は未対応が残る | `packages/entity/domain/mob/entity.ts:11` |
...
| 318 | 雪バイオームでの雪積もり実装済み | WeatherService の rain/thunder を降水として扱い、loaded chunk の SNOW biome 露出地形上に world maintenance で SNOW ブロックを積もらせる。clear 時や非 SNOW biome では積雪しない | `packages/world/domain/snow-accumulation.ts`, `packages/world/application/crop-growth-maintenance.ts`, `packages/app/application/frame/frame-maintenance-simulation.ts` |
...
| 326 | エンダーパール使用時のエンダーマイト湧き実装済み | ENDER_PEARL の右クリック即時テレポート後、5% ロールで Endermite をテレポート先へ生成する。投射物飛翔・着弾イベントは #213 の未実装範囲として残る | `packages/app/application/frame/stages/interaction-item-use-handler/ender-pearl.ts`, `packages/app/application/frame/stages/interaction-ender-pearl-handler.test.ts` |
| 327 | コウモリ（BAT）Mob 実装済み | `BatDefinition` を追加し、EntityType・PASSIVE_MOBS・既存受動スポーンローテーション・ドロップなし/XPなし・翼付き Mob 描画に対応。飛行AI・洞窟暗所スポーンは未実装 | `packages/entity/domain/mob/mobs/bat.ts`, `packages/entity/domain/mob/mob-categories.ts`, `packages/rendering/infrastructure/entity/mob-geometry.ts` |
| 328 | イカ（SQUID）Mob 実装済み | `SquidDefinition` / `GlowSquidDefinition` を追加し、EntityType・INK_SAC ドロップ・PASSIVE_MOBS・既存受動スポーンローテーション・縦型胴体 + 触手 Mob 描画・Glow Squid の発光色パレット描画に対応。GLOW_INK_SAC と水中限定スポーン/泳ぎAIは未実装 | `packages/entity/domain/mob/mobs/squid.ts`, `packages/entity/domain/mob/mob-categories.ts`, `packages/rendering/infrastructure/entity/mob-geometry.ts` |
| 329 | ウィッチ（WITCH）Mob 実装済み | `WitchDefinition` を追加し、EntityType・HOSTILE_MOBS・既存敵対スポーンローテーション・ローブ型 Biped 描画に対応。ドロップは既存 ItemType の REDSTONE_DUST/GLOWSTONE_DUST/SPIDER_EYE/STICKS に限定。ポーション投擲AI・回復/耐性ポーション・専用ポーションアイテム/確率ドロップは未実装 | `packages/entity/domain/mob/mobs/witch.ts`, `packages/entity/domain/mob/mob-categories.ts`, `packages/rendering/infrastructure/entity/mob-geometry.ts` |
| 330 | ドラウンド（水中ゾンビ）Mob 実装済み | `DrownedDefinition` を追加し、EntityType・HOSTILE_MOBS・UNDEAD_MOB_TYPES・既存敵対スポーンローテーション・水没ゾンビ型 Biped 描画に対応。ドロップは既存 ItemType の ROTTEN_FLESH/RAW_COD に限定。水中限定スポーン/泳ぎAI・ゾンビ変換・TRIDENT/銅/オウムガイ殻ドロップ・トライデント投擲は未実装 | `packages/entity/domain/mob/mobs/drowned.ts`, `packages/entity/domain/mob/mob-categories.ts`, `packages/entity/domain/combat.ts`, `packages/rendering/infrastructure/entity/mob-geometry.ts` |
| 331 | 氷（ICE）ブロック基本実装済み | ICE を BlockType / codec index 88 / 初期ブロック / 衝突 / 仮タイル描画に追加し、寒冷バイオームの水面を ICE 化。滑走摩擦はゲーム物理へ接続済み。光源融解は crop maintenance 周期で blockLight > 11 の ICE を WATER に置換済み。通常破壊は無ドロップ、Silk Touch 破壊のみ ICE をドロップする | `packages/core/domain/block-type.ts`, `packages/world/domain/terrain/lake-generator.ts`, `packages/game/application/game-state-physics.ts`, `packages/world/domain/ice-melting.ts`, `packages/world/test/ice-melting.test.ts`, `packages/world/application/crop-growth-maintenance.test.ts`, `packages/world/test/block-service-silk-touch.test.ts` |
| 332 | ゾンビビレジャー Mob 実装済み / 村人キュアなし | `ZombieVillagerDefinition` を追加し、EntityType・HOSTILE_MOBS・UNDEAD_MOB_TYPES・既存敵対スポーンローテーション・大きい頭とローブ型胴体の村人ゾンビ Biped 描画に対応。ドロップと戦闘値は Zombie 相当。弱化ポーション + 金のリンゴでの村人キュア、村人感染/変換、職業保持、村人ドメインとの連携は未実装 | `packages/entity/domain/mob/mobs/zombie-villager.ts`, `packages/entity/domain/mob/mob-categories.ts`, `packages/entity/domain/combat.ts`, `packages/rendering/infrastructure/entity/mob-geometry.ts` |
...
| 334 | ハチ（BEE）Mob 実装済み | BeeDefinition を追加し、EntityType・PASSIVE_MOBS・日中スポーンローテーション・ドロップなし/XPあり・翼付き Mob 描画とインスタンス描画に対応。中立化/刺針/巣・蜂蜜・花誘導・受粉AIは未実装 | `packages/entity/domain/mob/mobs/bee.ts`, `packages/rendering/infrastructure/entity/mob-geometry.ts` |
...
| 336 | 複数ワールドの作成・一覧・削除・表示名変更が実装済み | `main-menu-handlers.ts`：listWorldMetadata（最終プレイ順）・新規作成・削除 + 確認ダイアログ・`makeOpenRenamePrompt()` による `metadata.displayName` 更新を実装 | `packages/presentation/menu/main-menu-handlers.ts`, `packages/presentation/menu/main-menu-dom.ts`, `packages/world/domain/world-metadata-model.ts` |
