...
| 279 | XP オーブランタイムと主要発生元接続は実装済み / 描画は未完了 | `DroppedXpOrbService` が XP orb の spawn・tick・pickup delay・寿命・吸引移動・距離回収を管理し、frame 回収時は mending 修理を先に通して残 XP を `XPService` に加算する。近接・弓でのモブ撃破 XP と鉱石ブロック XP は orb spawn 経路に接続済み。繁殖報酬は直接 mending/XP 加算経路のまま。描画表現は未接続 | `packages/entity/application/dropped-xp-orb-service.ts`, `packages/entity/domain/dropped-xp-orb.ts`, `packages/app/application/frame/stages/entity-update-stage.ts`, `packages/entity/test/dropped-xp-orb-service.test.ts` |
| 280 | 浮遊アイテムエンティティと XP orb ランタイムは一部実装済み | Q ドロップ、Survival ブロック破壊ドロップ、近接・弓でのモブ死亡時アイテムドロップ、死亡時インベントリ/装備散布は `DroppedItemService` + `DroppedItemRendererService` + frame auto pickup に接続済み。ブロック破壊では直接インベントリ加算を抑止し、通常ドロップ・Silk Touch・草種・作物・葉ボーナス・Fortune 追加分を world item として spawn する。XP orb は runtime/pickup/mending 経路、プレイヤー吸引、モブ/鉱石 XP 発生元 spawn を追加済みだが、描画・浮遊表現は未完了 | `packages/app/application/frame/stages/interaction-break-handler.ts`, `packages/app/application/frame/stages/interaction-mob-drops.ts`, `packages/app/application/frame/stages/interaction-melee-handler.ts`, `packages/app/application/frame/stages/interaction-bow-handler.ts`, `packages/app/application/frame/stages/entity-update-stage.ts`, `packages/rendering/infrastructure/entity/dropped-item-renderer.ts`, `packages/entity/application/dropped-xp-orb-service.ts`, `packages/game/application/game-state-service.ts` |
