...
related_docs:
  ['./05-rendering-system.md', './06-physics-system.md', '../explanations/architecture/06-effect-ts-patterns.md']
---
...
// エラー定義
const GameLoopError = Schema.TaggedError("GameLoopError")({
  reason: Schema.String
}) {}
...
const createFixedTimeStepLoop = (config: TickConfig) =>
  Effect.gen(function* () {
    ...
    const tickStream = Stream.repeatEffectOption(
      STM.commit(
        STM.gen(function* () {
          ...
          const deltaTime = Math.min(currentTime - state.lastTime, tickDuration * config.maxCatchUpTicks)
...
const createVariableTimeStepLoop = (config: FrameConfig) =>
  Effect.gen(function* () {
    ...
    const renderStream = Stream.repeatEffectOption(
      Effect.gen(function* () {
        ...
        // フレームスキップ判定
        const shouldSkip = state.consecutiveSkips < config.maxFrameSkip && deltaTime > target * 2
...
const updateGameSystems = (tickNumber: bigint, deltaTime: DeltaTime) =>
  Effect.gen(function* () {
    ...
    yield* Stream.fromIterable(phases).pipe(
      Stream.mapEffect(
        (phase) => processTickPhase({ tickNumber, deltaTime, phase }),
...
const createSystemScheduler = Effect.gen(function* () {
    ...
    const systems: SystemInfo[] = [
      {
        ...
        estimatedDuration: Schema.decodeSync(DeltaTime)(2),
      },
      {
        name: 'player',
        priority: 2,
        ...
        estimatedDuration: Schema.decodeSync(DeltaTime)(3),
      },
      {
        name: 'ai',
        priority: 2,
        dependencies: ['input'],
        canRunInParallel: true,
        estimatedDuration: Schema.decodeSync(DeltaTime)(5),
      },
      {
        name: 'physics',
        priority: 3,
        dependencies: ['player', 'ai'],
        canRunInParallel: false,
        estimatedDuration: Schema.decodeSync(DeltaTime)(8),
      },
      {
        name: 'collision',
        priority: 4,
        ...
        canRunInParallel: false,
        estimatedDuration: Schema.decodeSync(DeltaTime)(4),
      },
      {
        name: 'movement',
        priority: 5,
      ...
      },
      {
        ...
        canRunInParallel: true,
        estimatedDuration: Schema.decodeSync(DeltaTime)(6),
      },
      {
        name: 'fluids',
        priority: 6,
      ...
      },
      {
        name: 'lighting',
        priority: 7,
        dependencies: ['redstone', 'fluids'],
        canRunInParallel: true,
        estimatedDuration: Schema.decodeSync(DeltaTime)(7),
      },
      {
        name: 'particles',
        priority: 7,
      ...
      },
      {
        ...
        dependencies: [],
        canRunInParallel: true,
        estimatedDuration: Schema.decodeSync(DeltaTime)(1),
      },
      {
        name: 'network',
        priority: 9,
        dependencies: ['lighting', 'sound'],
        canRunInParallel: false,
        estimatedDuration: Schema.decodeSync(DeltaTime)(5),
      },
    ]
  ...
  const executeSystemsInOrder = (tickData: GameTickData) =>
    Effect.gen(function* () {
      // 優先度でグループ化
      const systemGroups = Array.from(systemRegistry.values()).reduce(
        (acc, system) => {
...
const performCulling = (frameData: RenderFrameData) =>
  Effect.gen(function* () {
    ...
    const visibleEntities = yield* renderingService
      .performFrustumCulling(frustum)
      .pipe(Effect.flatMap((visible) => renderingService.performOcclusionCulling(visible)))
...
const interpolatePositions = (frameData: RenderFrameData) =>
  Effect.gen(function* () {
    ...
    const interpolatedEntities = yield* Stream.fromIterable(frameData.visibleEntities).pipe(
      Stream.mapEffect((entity) => interpolateEntityPosition(entity, frameData.interpolationAlpha), { concurrency: 4 }),
      Stream.runCollect
    )
...
const PerformanceReport = Schema.Struct({
  ...
  renderBottlenecks: Schema.Array(
    Schema.Struct({
      phase: Schema.String,
      impact: Schema.Number.pipe(Schema.filter((n) => n >= 0 && n <= 1, { message: () => 'Impact must be 0-1' })),
    })
  ),
...
const DebugOverlayConfig = Schema.Struct({
  ...
  opacity: Schema.Number.pipe(Schema.filter((n) => n >= 0 && n <= 1, { message: () => 'Opacity must be 0-1' })),
  refreshRate: Schema.Number.pipe(Schema.filter((n) => n > 0, { message: () => 'Refresh rate must be positive' })),
})
...
// デバッグオーバーレイシステム
const createDebugOverlay = (config: DebugOverlayConfig) =>
  Effect.gen(function* () {
    ...
    ).pipe(Stream.schedule(Schedule.spaced(`${1000 / config.refreshRate} millis`)))
    ...
    const drawPerformanceGraph = (ctx: CanvasRenderingContext2D, report: PerformanceReport, cfg: DebugOverlayConfig) =>
      Effect.gen(function* () {
        ...
        report.frameTimeDistribution.forEach(([time, count], index) => {
          ...
          const y =
            graphY + graphHeight - (count / Math.max(...report.frameTimeDistribution.map(([, c]) => c))) * graphHeight
    ...
    return {
      ...
      updateFrameHistory,
      debugInfoStream,
      isVisible: STM.TRef.get(debugState).pipe(
        STM.map((s) => s.visible),
        STM.commit
      ),
    }
    ...
    Stream.mapEffect(
      (task) =>
        Match.value(task).pipe(
          Match.tag('Physics', (physicsTask) => schedulePhysicsTask(workerPool.physics, physicsTask)),
          Match.tag('AI', (aiTask) => scheduleAITask(workerPool.ai, aiTask)),
          Match.tag('Rendering', (renderTask) => scheduleRenderingTask(workerPool.rendering, renderTask)),
          Match.tag('ChunkGeneration', (chunkTask) => scheduleChunkTask(workerPool.chunks, chunkTask)),
          Match.exhaustive
        ),
      { concurrency: 4 }
    ),
  ...
  const parallelUpdate = (tickNumber: bigint, deltaTime: DeltaTime) =>
    Effect.gen(function* () {
      ...
      yield* Stream.fromIterable(tasks).pipe(
        Stream.mapEffect((task) => STM.TQueue.offer(taskQueue, task).pipe(STM.commit), { concurrency: 'unbounded' }),
        Stream.runDrain
      )
    ...
    const fiber = yield* Effect.fork(
      Effect.gen(function* () {
        ...
        yield* messageStream.pipe(
          Stream.mapEffect((message) => STM.TQueue.offer(resultQueue, message as WorkerResult).pipe(STM.commit)),
          Stream.runDrain
        )
      })
    )
...
// Workerタスクスケジューラ関数
const schedulePhysicsTask = (
  worker: { id: string; fiber: Fiber.Fiber<void, never> },
  task: WorkerTask & { _tag: 'Physics' }
) =>
  Effect.gen(function* () {
...
const scheduleRenderingTask = (
  worker: { id: string; fiber: Fiber.Fiber<void, never> },
  task: WorkerTask & { _tag: 'Rendering' }
) =>
  Effect.gen(function* () {
    yield* Effect.logDebug(`Scheduling rendering task for tick ${task.tickNumber}`)
  })

const scheduleChunkTask = (
  worker: { id: string; fiber: Fiber.Fiber<void, never> },
  task: WorkerTask & { _tag: 'ChunkGeneration' }
) =>
  Effect.gen(function* () {
