...
// SoundServiceエラー定義
export const SoundLoadError = Schema.TaggedError("SoundLoadError")({
  soundId: string
  message: string
  timestamp: number
})

export const AudioContextError = Schema.TaggedError("AudioContextError")({
  message: string
  timestamp: number
})
...
export const SpatialAudioProcessor = Context.GenericTag<{
  readonly applySpatialEffects: (
    source: AudioBufferSourceNode,
    soundPosition: typeof Position.Type
  ) => Effect.Effect<AudioBufferSourceNode | AudioNode, never>
  readonly applyOcclusionEffects: (
    source: AudioBufferSourceNode,
    listenerPos: typeof Position.Type,
    soundPos: typeof Position.Type
  ) => Effect.Effect<AudioBufferSourceNode | AudioNode, never>
  readonly calculateSpatialData: (
    listenerPos: typeof Position.Type,
    soundPos: typeof Position.Type
  ) => Effect.Effect<typeof SpatialAudioData.Type, never>
  readonly processAudioStream: (
...
const makeSpatialAudioProcessor = Effect.gen(function* () {
  ...
  const calculateSpatialData = (listenerPos: typeof Position.Type, soundPos: typeof Position.Type) =>
    Effect.gen(function* () {
      ...
      const attenuationFactor = Volume(Math.max(0, 1 - Brand.nominal(distance) / maxHearingDistance))
  ...
  const processAudioStream = (
    ...
    audioStream.pipe(
      Stream.map((buffer) => {
        ...
        // 空間音響効果を適用したバッファを作成
        const processedBuffer = audioContext.createBuffer(buffer.numberOfChannels, buffer.length, buffer.sampleRate)
...
const MusicEvent = Schema.Struct({
  ...
  action: Schema.Union(Schema.Literal('play'), Schema.Literal('stop'), Schema.Literal('crossfade')),
  volume: Schema.instanceOf(Volume as any),
...
export const MusicManager = Context.GenericTag<{
  ...
  readonly startMusicProcessingLoop: (queue: Queue.Queue<typeof MusicEvent.Type>) => Effect.Effect<void, never>
...
const makeMusicManager = Effect.gen(function* () {
  ...
  const startMusicProcessingLoop = (queue: Queue.Queue<typeof MusicEvent.Type>) =>
    Effect.gen(function* () {
      ...
      // ストリームを実行（バックグラウンド処理）
      yield* Stream.runDrain(audioStream).pipe(Effect.forkDaemon)
    })
...
const AmbientEvent = Schema.Struct({
  ...
  action: Schema.Union(Schema.Literal('start'), Schema.Literal('stop'), Schema.Literal('update')),
  volume: Schema.instanceOf(Volume as any),
  fadeTime: Schema.Optional(Schema.instanceOf(Duration as any)),
}).annotations({
...
export const AmbientSoundManager = Context.GenericTag<{
  readonly updateAmbientSounds: (
    context: typeof EnvironmentContext.Type
  ) => Effect.Effect<void, SoundLoadError | AudioContextError>
  readonly determineAmbientSounds: (
    context: typeof EnvironmentContext.Type
  ) => Effect.Effect<ReadonlyArray<string>, never>
  readonly processAmbientEvents: (
  ...
  readonly startAmbientProcessingLoop: (
...
const makeAmbientSoundManager = Effect.gen(function* () {
          ...
          return yield* Match.value(event.action).pipe(
            ...
            Match.when('update', () =>
              Effect.gen(function* () {
                const existingSource = activeAmbients.get(event.soundId)
                if (!existingSource) {
                  return yield* processAmbientEvents(
                    Stream.succeed({
                      ...event,
                      action: 'start',
                    })
  ...
  // 環境音処理ループ（ストリームベース）
  const startAmbientProcessingLoop = (contextStream: Stream.Stream<typeof EnvironmentContext.Type, never>) =>
    Effect.gen(function* () {
...
export const AudioSettings = Schema.Struct({
  ...
  masterVolume: Schema.transform(Schema.Number.pipe(Schema.min(0), Schema.max(1)), Schema.instanceOf(Volume as any), {
    decode: Volume,
    encode: Brand.nominal,
  }),
  musicVolume: Schema.transform(Schema.Number.pipe(Schema.min(0), Schema.max(1)), Schema.instanceOf(Volume as any), {
    decode: Volume,
    encode: Brand.nominal,
  }),
  effectsVolume: Schema.transform(Schema.Number.pipe(Schema.min(0), Schema.max(1)), Schema.instanceOf(Volume as any), {
    decode: Volume,
    encode: Brand.nominal,
  }),
  ambientVolume: Schema.transform(Schema.Number.pipe(Schema.min(0), Schema.max(1)), Schema.instanceOf(Volume as any), {
    decode: Volume,
    encode: Brand.nominal,
  }),
  uiVolume: Schema.transform(Schema.Number.pipe(Schema.min(0), Schema.max(1)), Schema.instanceOf(Volume as any), {
    decode: Volume,
    encode: Brand.nominal,
  }),
