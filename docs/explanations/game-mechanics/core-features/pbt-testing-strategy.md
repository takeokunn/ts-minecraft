...
```typescript
// ✅ PBT最適: 小さな純粋関数
export const worldToChunkCoord = (worldCoord: number): number => Math.floor(worldCoord / 16)

export const chunkToWorldCoord = (chunkCoord: number): number => chunkCoord * 16

// 明度計算（テスト可能な単位）
export const combineLightLevels = (blockLight: number, skyLight: number): number => Math.max(blockLight, skyLight)

// 距離減衰（テスト可能な単位）
export const calculateLightAttenuation = (sourceLevel: number, distance: number): number =>
  Math.max(0, sourceLevel - distance)
```
...
test.prop([fc.integer()])('chunk conversion is reversible', (worldCoord) => {
...
```typescript
test.prop([fc.integer(), fc.integer(), fc.integer()])('world generation is deterministic', (seed, x, z) => {
...
])('division properties', (dividend, divisor) => {
