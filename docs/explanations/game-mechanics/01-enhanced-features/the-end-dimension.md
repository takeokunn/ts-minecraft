...
export interface DragonAIService {
  ...
  readonly handleDamage: (dragonId: EntityId, damage: number, source: DamageSource) => Effect.Effect<DamageResult, DamageError>
...
const processDragonCombat = (dragonId: EntityId) =>
  Effect.gen(function* () {
    ...
    const nextAction = yield* pipe(
      dragon.currentPhase,
      Match.value,
      Match.when({ type: "circling" }, (phase) =>
        calculateCirclingMovement(dragon, phase)
      ),
      Match.when({ type: "charging" }, (phase) =>
        executePlayerCharge(dragon, phase)
      ),
      Match.when({ type: "perching" }, (phase) =>
        handlePerchingBehavior(dragon, phase)
      ),
      Match.when({ type: "breathing" }, (phase) =>
        executeBreathAttack(dragon, phase)
      ),
      Match.exhaustive
    )
...
const updateShulkerBullet = (bulletId: ProjectileId) =>
  Effect.gen(function* () {
    ...
    const direction = normalizeVector(
      subtractVector(targetPosition, bullet.position)
    )
...
const updateDragonBossBar = (dragonId: EntityId) =>
  Effect.gen(function* () {
    ...
    const barColor = pipe(
      healthPercentage,
      Match.value,
      Match.when(p => p > 0.75, () => "pink" as const),
      Match.when(p => p > 0.25, () => "purple" as const),
