---
title: '03 Day Night Cycle'
description: '03 Day Night Cycleに関する詳細な説明とガイド。'
...
export const GameTick = pipe(Schema.Number, Schema.int(), Schema.nonNegative(), Schema.brand('GameTick'))

export type GameTime = Brand.Brand<number, 'GameTime'> // 0-24000 ticks per day
export const GameTime = pipe(Schema.Number, Schema.int(), Schema.between(0, 24000), Schema.brand('GameTime'))
...
export const LightLevel = pipe(Schema.Number, Schema.int(), Schema.between(0, 15), Schema.brand('LightLevel'))
...
export const TimeState = Schema.Struct({
  ...
  gameTime: GameTime,
  dayPhase: DayPhase,
  dayCount: pipe(Schema.Number, Schema.int(), Schema.nonNegative()),
  sunAngle: pipe(Schema.Number, Schema.between(0, 360)), // degrees
  moonAngle: pipe(Schema.Number, Schema.between(0, 360)), // degrees
...
export const CelestialBody = Schema.Struct({
  ...
  position: Schema.Struct({
    azimuth: pipe(Schema.Number, Schema.between(0, 360)),
    elevation: pipe(Schema.Number, Schema.between(-90, 90)),
  }),
  brightness: pipe(Schema.Number, Schema.between(0, 1)),
  visible: Schema.Boolean,
  phase: Schema.optional(pipe(Schema.Number, Schema.between(0, 1))), // For moon phases
})
...
export const TimeConfig = Schema.Struct({
  ...
  dayLength: pipe(
    Schema.Number,
    Schema.positive(),
    Schema.default(() => 20 * 60)
  ), // 20 minutes in real time
  doDayLightCycle: pipe(
    Schema.Boolean,
    Schema.default(() => true)
  ),
  weatherCycle: pipe(
  ...
  moonPhaseLength: pipe(
    Schema.Number,
    Schema.positive(),
    Schema.default(() => 8)
  ), // 8 days per moon cycle
})
...
```typescript
// Time System Errors
export const TimeSystemError = Schema.TaggedError("TimeSystemError")({
  message: string
  operation: string
  timestamp: number
})

export const LightingError = Schema.TaggedError("LightingError")({
  message: string
  position?: BlockPosition
  timestamp: number
})

export const BehaviorError = Schema.TaggedError("BehaviorError")({
  message: string
  mobId?: string
  timestamp: number
})

export const TimeEventError = Schema.TaggedError("TimeEventError")({
  message: string
  phase: DayPhase
  timestamp: number
})
...
interface LightingEngine {
  readonly calculateBlockLightLevel: (position: BlockPosition, timeState: TimeState) => Effect.Effect<LightLevel, never>
  ...
  readonly renderDynamicLighting: (timeState: TimeState, renderDistance: number) => Effect.Effect<LightingData, never>
}
...
export const LightingEngineLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const calculateBlockLightLevel = (position: BlockPosition, timeState: TimeState) =>
      Effect.gen(function* () {
    ...
    const calculateSkyLightContribution = (position: BlockPosition, timeState: TimeState, world: World) =>
      Effect.gen(function* () {
    ...
    const updateSkyLight = (chunks: ReadonlyArray<Chunk>, skyLightLevel: LightLevel) =>
      Effect.gen(function* () {
        const updatedChunks = yield* Effect.forEach(chunks, (chunk) => updateChunkSkyLight(chunk, skyLightLevel), {
          concurrency: 4,
        })
    ...
    const updateChunkSkyLight = (chunk: Chunk, skyLightLevel: LightLevel) =>
      Effect.gen(function* () {
    ...
    const propagateLight = (startPosition: BlockPosition, lightLevel: LightLevel, world: World) =>
      Effect.gen(function* () {
    ...
    const calculateShadows = (sunPosition: CelestialBody, world: World, region: BoundingBox) =>
      Effect.gen(function* () {
        ...
        for (let x = region.minX; x <= region.maxX; x++) {
          for (let z = region.minZ; z <= region.maxZ; z++) {
            const shadowIntensity = yield* calculateShadowAtPosition({ x, z }, sunVector, world, region)
    ...
    const renderDynamicLighting = (timeState: TimeState, renderDistance: number) =>
      Effect.gen(function* () {
        const sunPosition = yield* TimeSystem.pipe(Effect.flatMap((ts) => ts.calculateSunPosition(timeState.gameTime)))
...
interface TimeBehaviorSystem {
  ...
  readonly calculateSpawnRates: (timeState: TimeState, biome: string) => Effect.Effect<MobSpawnRates, never>
...
export const TimeBehaviorSystemLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const updateMobBehaviors = (mobs: ReadonlyArray<MobEntity>, timeState: TimeState) =>
      Effect.gen(function* () {
        const updatedMobs = yield* Effect.forEach(mobs, (mob) => updateSingleMobBehavior(mob, timeState), {
          concurrency: 8,
        })
    ...
    const updateSingleMobBehavior = (mob: MobEntity, timeState: TimeState) =>
      Effect.gen(function* () {
    ...
    const updateVillagerSchedule = (villager: VillagerEntity, timeState: TimeState) =>
      Effect.gen(function* () {
    ...
    const triggerTimeEvents = (oldPhase: DayPhase, newPhase: DayPhase, world: World) =>
      Effect.gen(function* () {
        let updatedWorld = world
    ...
    const calculateSpawnRates = (timeState: TimeState, biome: string) =>
      Effect.gen(function* () {
    ...
    const updatePlantGrowth = (plants: ReadonlyArray<PlantEntity>, timeState: TimeState, deltaTime: number) =>
      Effect.gen(function* () {
    ...
    return {
      ...
      calculateSpawnRates,
      updatePlantGrowth,
      manageVillagerSchedules: (villagers, timeState) =>
        Effect.gen(function* () {
          return yield* Effect.forEach(villagers, (villager) => updateVillagerSchedule(villager, timeState))
        }),
    } as const
  })
)
...
interface SkyRenderingSystem {
  ...
  readonly generateStarField: (timeState: TimeState, latitude: number) => Effect.Effect<StarFieldData, never>
...
export const SkyRenderingSystemLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const renderSkybox = (timeState: TimeState, weather: WeatherState, camera: CameraState) =>
      Effect.gen(function* () {
        const timeSystem = yield* TimeSystem
        const sunPosition = yield* timeSystem.calculateSunPosition(timeState.gameTime)
        const moonPosition = yield* timeSystem.calculateMoonPosition(timeState.gameTime, timeState.dayCount)
    ...
    const generateStarField = (timeState: TimeState, latitude: number) =>
      Effect.gen(function* () {
    ...
    const calculateSkyColors = (timeState: TimeState, sunPosition: CelestialBody, weather: WeatherState) =>
      Effect.gen(function* () {
        ...
        const zenithColor = blendColors([baseColors.zenith, sunInfluence.zenith, weatherModifier.zenith])

        const horizonColor = blendColors([baseColors.horizon, sunInfluence.horizon, weatherModifier.horizon])

        const ambientColor = blendColors([baseColors.ambient, sunInfluence.ambient, weatherModifier.ambient])
    ...
    const renderClouds = (timeState: TimeState, weather: WeatherState, windVector: { x: number; z: number }) =>
      Effect.gen(function* () {
    ...
    const createAtmosphericEffects = (timeState: TimeState, viewDistance: number) =>
      Effect.gen(function* () {
...
).pipe(Layer.provide(WorldSystemLayer), Layer.provide(EventBusLayer))
```
...
const exampleDayNightCycle = Effect.gen(function* () {
  ...
  if (currentTime.dayPhase !== newTime.dayPhase) {
    const updatedWorld = yield* behaviorSystem.triggerTimeEvents(currentTime.dayPhase, newTime.dayPhase, world)
  ...
  const processingFiber = yield* Effect.fork(
    Effect.forever(
      Effect.gen(function* () {
        ...
        yield* Effect.forEach(normalTask, processLightUpdate, { concurrency: 4 })
...
```typescript
// 時間システムの品質調整
export const adaptiveTimeQuality = (performanceMetrics: PerformanceMetrics) =>
  Effect.gen(function* () {
    const timeSystem = yield* TimeSystem
...
describe('Day/Night Cycle System', () => {
  const TestDayNightLayer = Layer.mergeAll(DayNightCycleLayer, TestWorldLayer).pipe(
  ...
  it('should correctly calculate day phases', () =>
    ...
    }).pipe(Effect.provide(TestDayNightLayer), Effect.runPromise))
