...
class ConsistentHashRing {
  private ring = new Map<number, StorageNodeId>()
  private sortedHashes: number[] = []
  private virtualNodes = 150

  addNode(nodeId: StorageNodeId): void {
  ...
  removeNode(nodeId: StorageNodeId): void {
    for (let i = 0; i < this.virtualNodes; i++) {
      const hash = this.hash(`${nodeId}-${i}`)
  ...
  getNode(key: string): StorageNodeId | null {
  ...
  getReplicationNodes(key: string, count: number): StorageNodeId[] {
    ...
    while (nodes.size < count && nodes.size < this.getUniqueNodes().length) {
  ...
  private findNextIndex(hash: number): number {
  ...
  private getUniqueNodes(): Set<StorageNodeId> {
    return new Set(this.ring.values())
  }

  getNodes(): StorageNodeId[] {
