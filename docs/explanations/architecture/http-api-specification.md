...
export const WorldAPIRoutes = {
  ...
  listWorlds: {
    ...
    queryParams: Schema.Struct({
      ...
      status: Schema.Optional(
        Schema.Union(Schema.Literal('active'), Schema.Literal('inactive'), Schema.Literal('maintenance'))
      ),
    ...
    errors: Schema.Union(UnauthorizedErrorSchema, ValidationErrorSchema, ServerErrorSchema),
  },
  ...
  getWorld: {
    ...
    errors: Schema.Union(NotFoundErrorSchema, UnauthorizedErrorSchema, ServerErrorSchema),
  },
  ...
  createWorld: {
    ...
    body: Schema.Struct({
      ...
      maxPlayers: Schema.Optional(Schema.Number.pipe(Schema.int(), Schema.between(1, 100))),
      worldType: Schema.Optional(
        Schema.Union(
    ...
    response: WorldDetailSchema,
    errors: Schema.Union(ValidationErrorSchema, UnauthorizedErrorSchema, ConflictErrorSchema, ServerErrorSchema),
  },
  ...
  updateWorld: {
    ...
    body: Schema.Struct({
      name: Schema.Optional(Schema.String.pipe(Schema.minLength(1), Schema.maxLength(50))),
      description: Schema.Optional(Schema.String.pipe(Schema.maxLength(500))),
  ...
  deleteWorld: {
    ...
    errors: Schema.Union(NotFoundErrorSchema, UnauthorizedErrorSchema, ConflictErrorSchema, ServerErrorSchema),
  },
  ...
  getWorldPlayers: {
    ...
    queryParams: Schema.Struct({
      status: Schema.Optional(Schema.Union(Schema.Literal('online'), Schema.Literal('offline'), Schema.Literal('all'))),
      limit: Schema.Optional(Schema.Number.pipe(Schema.int(), Schema.between(1, 100))),
      offset: Schema.Optional(Schema.Number.pipe(Schema.int(), Schema.min(0))),
    }),
    response: Schema.Struct({
  ...
  getWorldChunks: {
    ...
    errors: Schema.Union(NotFoundErrorSchema, ValidationErrorSchema, UnauthorizedErrorSchema, ServerErrorSchema),
  },
}
...
export const WorldSummarySchema = Schema.Struct({
  ...
  gameMode: GameModeSchema,
  difficulty: DifficultySchema,
  status: Schema.Union(Schema.Literal('active'), Schema.Literal('inactive'), Schema.Literal('maintenance')),
...
export const PlayerAPIRoutes = {
  ...
  getMyProfile: {
    ...
    errors: Schema.Union(UnauthorizedErrorSchema, ServerErrorSchema),
  },
  ...
  updateMyProfile: {
    ...
    path: '/api/v1/players/me',
    body: Schema.Struct({
      displayName: Schema.Optional(Schema.String.pipe(Schema.minLength(1), Schema.maxLength(20))),
  ...
  updateMyInventory: {
    ...
    body: Schema.Struct({
      ...
      hotbarSelection: Schema.Optional(Schema.Number.pipe(Schema.int(), Schema.between(0, 8))),
    }),
    response: InventorySchema,
    errors: Schema.Union(ValidationErrorSchema, UnauthorizedErrorSchema, ConflictErrorSchema, ServerErrorSchema),
  },
  ...
  getMyStats: {
    ...
    queryParams: Schema.Struct({
      ...
      period: Schema.Optional(
        Schema.Union(Schema.Literal('day'), Schema.Literal('week'), Schema.Literal('month'), Schema.Literal('all'))
      ),
    }),
  ...
  getMyAchievements: {
    ...
    queryParams: Schema.Struct({
      status: Schema.Optional(
        Schema.Union(Schema.Literal('unlocked'), Schema.Literal('locked'), Schema.Literal('all'))
      ),
...
export const PlayerPreferencesSchema = Schema.Struct({
  ...
  theme: Schema.Union(Schema.Literal('light'), Schema.Literal('dark'), Schema.Literal('auto')),
...
export const GameSessionAPIRoutes = {
  ...
  updateMySession: {
    ...
    body: Schema.Struct({
      ...
      status: Schema.Optional(Schema.Union(Schema.Literal('playing'), Schema.Literal('idle'), Schema.Literal('afk'))),
    }),
    response: SessionSchema,
    errors: Schema.Union(NotFoundErrorSchema, ValidationErrorSchema, UnauthorizedErrorSchema, ServerErrorSchema),
  },
...
export const AuthAPIRoutes = {
  ...
  login: {
    ...
    errors: Schema.Union(ValidationErrorSchema, UnauthorizedErrorSchema, TooManyRequestsErrorSchema, ServerErrorSchema),
  },
  ...
  register: {
    ...
    body: Schema.Struct({
      username: Schema.String.pipe(Schema.minLength(3), Schema.maxLength(20), Schema.pattern(/^[a-zA-Z0-9_-]+$/)),
      email: Schema.String.pipe(Schema.email()),
      password: Schema.String.pipe(
        Schema.minLength(8),
        Schema.pattern(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]/)
      ),
      displayName: Schema.String.pipe(Schema.minLength(1), Schema.maxLength(30)),
      termsAccepted: Schema.Boolean,
      captcha: Schema.String,
    }),
    response: AuthResponseSchema,
    errors: Schema.Union(ValidationErrorSchema, ConflictErrorSchema, TooManyRequestsErrorSchema, ServerErrorSchema),
  },
  ...
  forgotPassword: {
    ...
    errors: Schema.Union(ValidationErrorSchema, TooManyRequestsErrorSchema, ServerErrorSchema),
  },
  ...
  resetPassword: {
    ...
    errors: Schema.Union(ValidationErrorSchema, UnauthorizedErrorSchema, ServerErrorSchema),
  },
      ...
      authenticate: (token) =>
        Effect.gen(function* () {
          try {
            ...
            // ユーザー情報の取得
            const user = yield* UserRepository.pipe(Effect.flatMap((repo) => repo.findById(payload.sub)))
...
export const WebSocketAPIEvents = {
  ...
  clientEvents: {
    ...
    'world:block-update': Schema.Struct({
      ...
      action: Schema.Union(Schema.Literal('place'), Schema.Literal('break')),
      timestamp: Schema.Number,
    }),

    // チャットメッセージ
    'chat:message': Schema.Struct({
      content: Schema.String.pipe(Schema.minLength(1), Schema.maxLength(500)),
      type: Schema.Optional(Schema.Union(Schema.Literal('public'), Schema.Literal('private'), Schema.Literal('team'))),
  ...
  serverEvents: {
    ...
    'chat:broadcast': Schema.Struct({
      ...
      type: Schema.Union(Schema.Literal('public'), Schema.Literal('system'), Schema.Literal('admin')),
      timestamp: Schema.Number,
    }),
    ...
    return {
      connect: (params) =>
        Effect.gen(function* () {
          ...
          yield* Ref.update(connections, (conns) => new Map([...conns, [connectionId, connection]]))

          yield* Ref.update(playerConnections, (players) => new Map([...players, [params.auth.id, connectionId]]))
      ...
      broadcast: (params) =>
        Effect.gen(function* () {
          ...
          const targetConnections = Array.from(conns.values()).filter(
            (conn) => conn.worldId === params.worldId && conn.id !== params.excludeConnectionId && conn.isAlive
          )
          ...
          yield* Effect.all(
            targetConnections.map((conn) =>
              Effect.promise(() =>
                ...
                Effect.catchAll((error) => Effect.logError(`Failed to send message to connection ${conn.id}: ${error}`))
              )
            ),
            { concurrency: 10 }
          )
      ...
      get: (params) =>
        Effect.gen(function* () {
          ...
          if (Date.now() > entry.expiresAt) {
            yield* HTTPCacheManager.pipe(Effect.flatMap((manager) => manager.invalidate(params.key)))
            return Option.none()
          }
        ...
        Effect.gen(function* () {
          ...
          yield* Effect.all(
            Array.from(keysToInvalidate).map((key) =>
              HTTPCacheManager.pipe(Effect.flatMap((manager) => manager.invalidate(key)))
            )
          )
        }),
...
export const createCacheMiddleware = <T>(params: {
  ...
  return (request: Request, response: Response, next: () => Effect.Effect<Response>) =>
    Effect.gen(function* () {
      ...
      // キャッシュから取得を試行
      const cached = yield* cacheManager
        .get({
          key: cacheKey,
