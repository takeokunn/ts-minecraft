...
const processValue = (value: number | string | boolean) =>
  pipe(
    ...
    Match.when(Match.number, (n) =>
      pipe(
        Match.value(n),
        Match.when((x) => x > 100, () => "large"),
        Match.when((x) => x > 50, () => "medium"),
        Match.orElse(() => "small")
      )
    ),
...
const categorizeAge = (age: number) =>
  pipe(
    Match.value(age),
    Match.when((a) => a < 0, () => "invalid"),
    Match.when((a) => a < 13, () => "child"),
    Match.when((a) => a < 20, () => "teenager"),
    Match.when((a) => a < 60, () => "adult"),
    Match.orElse(() => "senior")
  )
...
const handlePacket = pipe(
  ...
  Match.discriminator("protocol")(
    "HTTP",
    ({ method, url }) => `HTTP ${method} ${url}`
  ),
  Match.discriminator("protocol")(
    "WebSocket",
    ({ event, data }) => `WS Event: ${event}`
  ),
  Match.discriminator("protocol")(
    "TCP",
    ({ port }) => `TCP on port ${port}`
  ),
  Match.exhaustive
)
...
const renderPipeline = pipe(
  ...
  Match.discriminatorsExhaustive("stage")({
    ...
    error: ({ message, code }) =>
      Effect.fail(new RendererError(message, code))
  })
...
const validateUsername = (input: string) =>
  pipe(
    Match.value(input),
    Match.when(Match.nonEmptyString, (username) =>
      Effect.succeed({ username, valid: true })
    ),
    Match.orElse(() =>
      Effect.fail(new Error("Username cannot be empty"))
    )
  )
...
const canAcceptConnections = (status: ServerStatus) =>
  pipe(
    Match.value(status),
    Match.not("error", "maintenance", (safeStatus) =>
  ...
  pipe(
    ...
    Match.tag("Exiting", ({ saveProgress }) =>
      pipe(
        ...
        Match.when((p) => p >= 100, () => Effect.succeed(state)),
        Match.orElse(() =>
          Effect.succeed({
...
const hasRequiredFields = <T extends Record<string, unknown>>(
...
const validateUserInput = (input: unknown): Effect.Effect<ValidatedUser> =>
  pipe(
    ...
    Match.orElse(() =>
      Effect.fail(new ValidationError("Invalid user input"))
    )
  )
...
const pathMatcher = <T>(path: string[], predicate: (value: T) => boolean) =>
  (obj: unknown) => {
...
// 動的にパターンを構築
const createDynamicMatcher = <T extends string>(
  validValues: readonly T[]
) => {
  const matchers = validValues.map(value =>
    Match.when(value as T, () => `Matched: ${value}`)
  )

  return (input: T | unknown) =>
    matchers.reduce(
      (acc, matcher) => matcher(acc),
      Match.value(input)
    )
}
...
const autoGeneratedMatcher = (user: User) => {
  const handlers = {
    Admin: (admin: Extract<User, { _tag: "Admin" }>) =>
      `Admin with ${admin.permissions.length} permissions`,
    User: (user: Extract<User, { _tag: "User" }>) =>
      `User: ${user.email}`,
    Guest: (guest: Extract<User, { _tag: "Guest" }>) =>
      `Guest session: ${guest.sessionId}`
  }
...
const traverseTree = <T, R>(
  tree: Tree<T>,
  leafHandler: (value: T) => R,
  branchHandler: (left: R, right: R) => R
): R =>
  pipe(
    Match.value(tree),
    Match.when(
      { type: "leaf" },
      ({ value }) => leafHandler(value)
    ),
    Match.when(
      { type: "branch" },
      ({ left, right }) => branchHandler(
        traverseTree(left, leafHandler, branchHandler),
        traverseTree(right, leafHandler, branchHandler)
      )
    ),
    Match.exhaustive
  )
...
const evaluate = (
  ...
  pipe(
    ...
    Match.when(
      ...
      ({ name }) =>
        name in variables
          ? Effect.succeed(variables[name])
          : Effect.fail(new Error(`Undefined variable: ${name}`))
    ),
    Match.when(
      { type: "binary" },
      ({ op, left, right }) =>
...
const validateAndProcess = (data: unknown): Effect.Effect<ProcessedData> =>
  pipe(
    ...
    // 正常処理
    Match.orElse((d) =>
      Effect.succeed(processData(d as ValidData))
    )
  )
```
...
type Result<T, E> =
  ...
  | { _tag: "Error"; message: E }

const processWithTypeInference = <T, E>(
  result: Result<T, E>
): InferMatchResult<Result<T, E>> =>
  pipe(
    Match.value(result),
...
const validateToken = (token: string): Effect.Effect<AuthToken> =>
  pipe(
    ...
    Match.orElse(() =>
      Effect.fail(new Error("Invalid token format"))
    )
  )
