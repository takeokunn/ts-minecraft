...
export const TimeConfig = Schema.Struct({
  ticksPerSecond: pipe(Schema.Number, Schema.positive(), Schema.default(() => 20)),
  dayLength: pipe(Schema.Number, Schema.positive(), Schema.default(() => 20 * 60)), // 20 minutes in real time
  doDayLightCycle: pipe(Schema.Boolean, Schema.default(() => true)),
  weatherCycle: pipe(Schema.Boolean, Schema.default(() => true)),
  timeSpeed: pipe(Schema.Number, Schema.positive(), Schema.default(() => 1.0)),
  moonPhaseLength: pipe(Schema.Number, Schema.positive(), Schema.default(() => 8)) // 8 days per moon cycle
})
...
```typescript
// Time System Errors
export class TimeSystemError extends Schema.TaggedError("TimeSystemError")<{
  message: string
  operation: string
  timestamp: number
}> {}

export class LightingError extends Schema.TaggedError("LightingError")<{
  message: string
  position?: BlockPosition
  timestamp: number
}> {}

export class BehaviorError extends Schema.TaggedError("BehaviorError")<{
  message: string
  mobId?: string
  timestamp: number
}> {}

export class TimeEventError extends Schema.TaggedError("TimeEventError")<{
  message: string
  phase: DayPhase
  timestamp: number
}> {}
