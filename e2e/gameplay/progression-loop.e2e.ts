...
test.describe('Progression loop', () => {
  test('supports gather → craft → build → fight through the runtime loop', async ({ page }) => {
    ...
    await page.evaluate(() => (window as typeof window & { __TS_MINECRAFT_QA__?: { stageBuildSupportBlock: () => Promise<void> } }).__TS_MINECRAFT_QA__?.stageBuildSupportBlock())
    await page.waitForTimeout(200)
    ...
    await page.evaluate(() => (window as typeof window & { __TS_MINECRAFT_QA__?: { aimAtStagedZombie: () => Promise<void> } }).__TS_MINECRAFT_QA__?.aimAtStagedZombie())
    await page.waitForTimeout(100)
